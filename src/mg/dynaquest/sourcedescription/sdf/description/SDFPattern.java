...
abstract public class SDFPattern extends SDFElement {
	...
	static Logger logger = Logger.getLogger(SDFPattern.class);
	...
	public SDFPattern(SDFPattern pattern) {
        ...
        this.elements = new SDFAttributeAttributeBindingPairList(pattern.elements);
    }

    public void addAttributeAttributeBindingPair(
			SDFAttributeAttributeBindingPair attributeAttributeBindingPair) {
		this.elements.addAttributeAttributeBindingPair(attributeAttributeBindingPair);
	}


	public SDFAttributeAttributeBindingPair getAttributeAttributeBindingPair(int index) {
    ...
    public SDFAttributeAttributeBindingPair getAttributeAttributeBindingPair(SDFAttribute attribute) {
		for (int i = 0; i < elements.getAttributeAttributeBindingPairCount(); i++) {
	...
	protected static void doUnion(SDFPattern rightPattern, SDFPattern newPattern) throws SDFPatternNotCompatible {
        // Hier einfach die beiden Pattern zusammenkopieren. Probleme gibt es nur bei Attributen, die 
        // in beiden Pattern vorkommen, die müssen verträglich kombiniert werden
		for (int i=0;i<rightPattern.getAttributeAttributeBindingPairCount();i++){
            SDFAttributeAttributeBindingPair leftAap = null;
            SDFAttributeAttributeBindingPair rightAap = rightPattern.getAttributeAttributeBindingPair(i);
            if ((leftAap = newPattern.getAttributeAttributeBindingPair(rightAap.getAttribute())) != null){


                if (leftAap.getAttributeBinding()==null || rightAap.getAttributeBinding()== null || !leftAap.getAttributeBinding().equals(rightAap.getAttributeBinding())){
                    try{
                        SDFAttributeBindung mAttributeBinding = leftAap.getAttributeBinding() != null?leftAap.getAttributeBinding():rightAap.getAttributeBinding();
                        SDFAttributeBindung newAttributeBinding = null;
                        if (mAttributeBinding != null){
                            newAttributeBinding = mAttributeBinding.createMerge(leftAap.getAttributeBinding(), rightAap.getAttributeBinding());
                        }
                        // das AttributeBinding austauschen (selbe Stelle!)
                        leftAap.setAttributeBinding(newAttributeBinding);
                        logger.info("##### => SDFPAttern-OK: LinkesAttributPaar:"+leftAap+" <==> RightAttributPaar:"+newAttributeBinding);    	
                   	 
                    }catch(SDFAttributeBindingNotMergeableException e){
                		logger.info("##### => SDFPAttern-EXC: LinkesAttributPaar:"+leftAap+" <==> NewPattern:"+newPattern);    	
                        throw new SDFPatternNotCompatible(e.getMessage());
                    }
                }
            }else{
                newPattern.addAttributeAttributeBindingPair(rightPattern.getAttributeAttributeBindingPair(i));
            }
        }
	}
    ...
    public static boolean checkIfEqual(SDFPattern left, SDFPattern right) {
        // Vergleicht alle Attribute beider Pattern und gibt true, wenn absolut gleiche Namen
		...
		if(leftSet.getAttributeCount() != rightSet.getAttributeCount()) 
			return false; // schon mal unterschiedliche Mustergrössen
		// sonst......
		for (int i = 0; i < leftSet.getAttributeCount(); i++) 
		{
			logger.info("##### => CHECK: "+rightSet.getAttribute(i).toString()+" <==> "+leftSet.getAttribute(i).toString()+"("+leftSet.getAttribute(i).toString().equals(rightSet.getAttribute(i).toString())+")");    	
