 ...
 * RepositoryManagers als Liste der Konstanten zu ermitteln. RepositoryManager ermittelt die Belegungen durch
 * die Ausführung einer in Repository abgelegten Anfrage auf einer Hilfsquelle.
 * @author Jurij Henne
 *
 */
public class NecInAttrCompSource extends NecInAttrCompensation {

	
	public NecInAttrCompSource(){}

	
	public NecInAttrCompSource(SDFQuery query)
	{
    	this.setQuery(query);
	}
	...
	public ArrayList <AnnotatedPlan> compensate (ArrayList <AnnotatedPlan> toCompensate)  
	{
		...
		for(int i=0; i<toCompensate.size(); i++)
		{
			...
			for(int j=0; j<attrToCompensate.getAttributeCount(); j++)
			{
				...
				SDFConstantList constSet = rm.getSourceList(nextAttribute);  // UNIQUE-Property of this CompensateAction
