 ...
 * RepositoryManagers als Liste der Konstanten zu ermitteln.
 * @author Jurij Henne
 *
 */
public class NecInAttrCompConstant extends NecInAttrCompensation {

	
	public NecInAttrCompConstant(){}

	public NecInAttrCompConstant(SDFQuery query)
	{
    	this.setQuery(query);
	}
	...
	public ArrayList <AnnotatedPlan> compensate (ArrayList <AnnotatedPlan> toCompensate)  
	{
		...
		for(int i=0; i<toCompensate.size(); i++)
		{
			...
			for(int j=0; j<attrToCompensate.getAttributeCount(); j++)
			{
				...
				SDFConstantList constSet = rm.getConstantList(nextAttribute);  // UNIQUE-Property of this CompensateAction
