...
public class FreqCycleBuilder extends AbstractOperatorBuilder {
	...
	protected ILogicalOperator createOperatorInternal() {
		...
		BrokerDictionary.getInstance().setReadTypeForPort(name, 0,
		...
		for (int i = 1; i <= super.getInputOperatorCount(); i++) {
			...
			BrokerDictionary.getInstance().setCurrentOuputPort(name,
					dataOutPort + 1);
			BrokerDictionary.getInstance().setReadTypeForPort(name,
					dataOutPort, ReadTransaction.Continuous);
			BrokerDictionary.getInstance().setWriteTypeForPort(name,
					dataInPort, WriteTransaction.Continuous);
			BrokerDictionary.getInstance().setWriteTypeForPort(name,
					queueInPort, WriteTransaction.Timestamp);
			BrokerDictionary.getInstance().addQueuePortMapping(name,
					new QueuePortMapping(dataOutPort, queueInPort));
	...
	public FreqCycleBuilder cleanCopy() {
		return new FreqCycleBuilder();
	}
