...
public class FilterExpressionCovarianceUpdatePO<M extends IGain & IProbability & IObjectTrackingLatency & IConnectionContainer>
	...
	public FilterExpressionCovarianceUpdatePO(
			FilterExpressionCovarianceUpdatePO<M> copy) {
		super(copy);
			...
			if (variable.isSchemaVariable()
				...
				if (variable.isInList(scannedTupleIndexPath)) {
					...
					String[] restrictedVariables = car.getMetadata()
							.getRestrictedList();
					// cov =
					// scanCovHelper.getCovarianceForRestrictedVariables(cov,
					// restrictedVariables);
					// variable.bind(cov);
					variable.bind(covHelper
							.getCovarianceForRestictedAttributes(
									restrictedVariables, car.getMetadata()));
					...
					// cov =
					// predCovHelper.getCovarianceForRestrictedVariables(cov,
					// restrictedVariables);
					// variable.bind(cov);
					variable.bind(covHelper
							.getCovarianceForRestictedAttributes(
	...
	private void updateCovariance(MVRelationalTuple<M> car, double[][] restCov,
		...
		for (String attr : restrictedList) {
			// restrictedIndices[i] =
			// prob.getCovarianceIndex(restrictedList[i]);
