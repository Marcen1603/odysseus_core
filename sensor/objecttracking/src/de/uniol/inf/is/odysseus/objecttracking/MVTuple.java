...
public class MVTuple<T extends IProbability> extends Tuple<T> implements Serializable {
    ...
    public MVTuple(SDFSchema schema, String line, char delimiter) {
        super();
     ...
     */
    @SuppressWarnings("deprecation")
    public MVTuple(SDFSchema schema) {
        super();
        this.findMeasurementValuePositions(schema);
    }
    ...
    public MVTuple(MVTuple copy) {
        ...
        for (int i = 0; i < size(); i++) {
            ...
            if (attribute instanceof MVTuple) {
                setAttribute(i, new MVTuple<T>((MVTuple<T>) attribute)); // recursive
                                                                                             // copy
    ...
    private List<Object> copyList(List<Object> attribute) {
        ...
        for (Object obj : oldList) {
            if (obj instanceof MVTuple) {
                newList.add(new MVTuple((MVTuple) obj));
    ...
    // if( actualAttribute instanceof MVTuple<?>) {
    // actualList = ((MVTuple<?>)actualAttribute).getAttributes();
    // }
    // }
    //
    ...
    // return true;
    // }
    // Iterator<SDFDatatypeConstraint> i =
    // attribute.getDtConstraints().iterator();
    //
    // while (i.hasNext()) {
    ...
    // SDFDatatypeConstraints.isMeasurementValue(constraint)) {
    // return true;
    // }
    // }
    // return false;
    ...
    // * werden, die in der attrList uebergeben werden, die Reihenfolge des
    // neuen
    // * Objektes wird durch die Reihenfolge der Attribute im Array festgelegt
    // * Beispiel: attrList[1]=14,attrList[2]=12 erzeugt ein neues Objekt,
    // welches
    // * die Attribute 14 und 12 enthaelt.
    // *
    // * It is important to pass the original schema. Otherwise it is impossible
    // * to find out whether an attribute to put into the output is a
    // measurement
    // * value or not.
    // *
    ...
    // */
    // @Deprecated
    // public MVTuple restrict(int[] attrList, RealMatrix matrix,
    // RealMatrix b, SDFSchema overwriteSchema, SDFSchema originalSchema) {
    //
    // MVTuple newAttrList = null;
    //
     ...
     * @return
     */
    public MVTuple<T> restrict(int[] restrictList, RealMatrix restrictMatrix, boolean createNew) {
        if (!createNew) {
            MVTuple<T> modifiedAttrs = (MVTuple<T>) super.restrict(restrictList, createNew);
            /**
        ...
        MVTuple<T> newTuple = new MVTuple<T>(restrictList.length);
    ...
    @Override
    public MVTuple<T> clone() {
        return new MVTuple<T>(this);
    }

    public final boolean equals(MVTuple<T> o) {
        return this.compareTo(o) == 0;
    }

    public int compareTo(MVTuple<T> tuple) {
        int min = tuple.size();
    ...
    private final static Object convertAttribute(String stringValue, SDFAttribute attribute) {
        ...
        throw new IllegalArgumentException("attributes of type " + attribute.getDatatype() + " can't be used with " + Tuple.class);
    }
}
