package de.uniol.inf.is.odysseus.mining.weka;
...
public final class AbstractWeka<M extends ITimeInterval> {
	...
	public AbstractWeka(SDFSchema schema, int[] attributes) {
		this.schema = schema;
		this.wekaSchema = calcAttributeList(attributes, null);
		this.attributes = attributes;
	}

	public AbstractWeka(SDFSchema schema, int[] attributes, Map<SDFAttribute, List<String>> nominals) {
		this.schema = schema;
	...
	protected void removeFromSchema(SDFAttribute attribute) {
		...
		if (index != -1) {
			...
			this.schema = new SDFSchema(this.schema.getURI(), attributes);
		}
	}
