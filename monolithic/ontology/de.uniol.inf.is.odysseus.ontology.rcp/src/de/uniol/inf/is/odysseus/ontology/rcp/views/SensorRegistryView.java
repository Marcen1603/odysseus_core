                ...
                public void run() {
                    try {
                        ...
                        if (!SensorRegistryView.this.getTreeViewer().getTree().isDisposed()) {
                            ...
                            final Collection<Object> entities = new ArrayList<Object>();
                            entities.addAll(sensingDevices);
                            entities.addAll(featuresOfInterest);
                            SensorRegistryView.this.getTreeViewer().setInput(entities);

                            for (final SensingDevice device : sensingDevices) {
                                for (final MeasurementCapability measurementCapability : device.getHasMeasurementCapabilities()) {
