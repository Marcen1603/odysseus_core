...
public class MeasurementCapabilitiesPage extends WizardPage {
    ...
    public void createControl(final Composite parent) {
            ...
            public void widgetSelected(final SelectionEvent e) {
                ...
                if ((MeasurementCapabilitiesPage.this.attributeCapabilities != null) && (attribute != null)) {
                    for (final MeasurementCapability capability : MeasurementCapabilitiesPage.this.attributeCapabilities.get(attribute)) {
                        ...
                        for (final Condition condition : capability.getInConditions()) {
                            for (final MeasurementProperty measurementProperty : capability.getHasMeasurementProperties()) {
                                item.setText(0, capability.getForProperty().getName());
                                item.setText(1, condition.getOnProperty().getName());
                                item.setText(2, condition.toString());
                                item.setText(3, URI.create(measurementProperty.getResource()).getFragment());
                                item.setText(4, measurementProperty.getExpression());
                            }
                        }
                    }
                }
        ...
        final String[] titles = { OdysseusNLS.Property, OdysseusNLS.Condition, OdysseusNLS.Expression, OdysseusNLS.MeasurementProperty, OdysseusNLS.Expression };
        for (final String title2 : titles) {
            final TableColumn column = new TableColumn(this.tblCapabilities, SWT.NONE);
        ...
        btnAdd.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(final SelectionEvent e) {
                ...
                final MeasurementCapabiltyDialog measurementCapabilityDialog = new MeasurementCapabiltyDialog(btnAdd.getShell(), URI.create(MeasurementCapabilitiesPage.this.sensingDevicePage
                        .getSensingDeviceURI().toString() + "/" + attribute.getAttributeName()), attribute);
                ...
                if (measurementCapabilityDialog.getReturnCode() == Window.OK) {
                    final TableItem item = new TableItem(MeasurementCapabilitiesPage.this.tblCapabilities, SWT.NONE);
                    item.setText(0, measurementCapabilityDialog.getProperty().getName());
                    item.setText(1, measurementCapabilityDialog.getCondition().getOnProperty().getName());
                    item.setText(2, measurementCapabilityDialog.getCondition().toString());
                    item.setText(3, URI.create(measurementCapabilityDialog.getMeasurementProperty().getResource()).getFragment());
                    item.setText(4, measurementCapabilityDialog.getMeasurementProperty().getExpression());

                    final MeasurementCapability capability = new MeasurementCapability(URI.create(MeasurementCapabilitiesPage.this.sensingDevicePage.getSensingDeviceURI().toString() + "/"
                            + attribute.getAttributeName()), attribute.getAttributeName(), measurementCapabilityDialog.getProperty());
