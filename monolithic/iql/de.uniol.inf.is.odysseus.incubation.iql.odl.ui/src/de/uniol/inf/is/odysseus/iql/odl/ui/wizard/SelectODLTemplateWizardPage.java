...
public class SelectODLTemplateWizardPage extends WizardPage {

	private static final String DEFAULT_TEMPLATE = ODLTemplateRegistry.EMPTY_TEMPLATE_NAME;
	
	private static final ODLTemplateRegistry TEMPLATE_REGISTRY = ODLTemplateRegistry.getInstance();
	
	
	private final IODLTemplate defaultTemplate;
	...
	protected SelectODLTemplateWizardPage(String pageName) {
		super(pageName);
		
		setTitle("Template selection");
		setDescription("Choose a template for your odl script to be preentered.");
		
		defaultTemplate = determineDefaultTemplate();
	}

	private static IODLTemplate determineDefaultTemplate() {
		return TEMPLATE_REGISTRY.getTemplate(DEFAULT_TEMPLATE);
	}
	...
	public IODLTemplate getSelectedTemplate() {
	...
	private void selectTemplate(String templateName) {
		IODLTemplate template = TEMPLATE_REGISTRY.getTemplate(templateName);
	...
	private Combo createTemplateNameChooser(Composite rootComposite) {
		ODLTemplateRegistry registry = TEMPLATE_REGISTRY;
