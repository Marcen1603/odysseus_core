...
public class SaseAOBuilder extends AbstractOperatorBuilder {
	...
	private final BooleanParameter oneMatchPerInstance = new BooleanParameter(
			"OneMatchPerInstance", REQUIREMENT.OPTIONAL);

	private final IntegerParameter heartbeatrate = new IntegerParameter(
			"heartbeatrate", REQUIREMENT.OPTIONAL);
	...
	protected ILogicalOperator createOperatorInternal() {
		...
		List<IExecutorCommand> op = parser.parse(query.getValue(), getCaller(),
		...
		if (cmd instanceof CreateQueryCommand) {
			ret = ((CreateQueryCommand)cmd).getQuery().getLogicalPlan();
			if (oneMatchPerInstance.hasValue()) {
				((PatternDetectAO<?>) ret)
						.setOneMatchPerInstance(oneMatchPerInstance.getValue());
			}

			if (heartbeatrate.hasValue()) {
				((PatternDetectAO<?>) ret).setHeartbeatRate(heartbeatrate
						.getValue());
			}
