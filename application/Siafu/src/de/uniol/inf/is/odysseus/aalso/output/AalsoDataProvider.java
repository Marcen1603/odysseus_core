...
public class AalsoDataProvider extends StreamClientHandler {
	...
	/**
	 * Sets the global variables and generates the "create-query" by calling the setObjectFrequencyAndQuery- and writeCQLQuery method
	 * 
	 ...
	 * @param sensorObjectReferenceName @see {@link #sensorObjectReferenceName}
	 * @param startingPort @see {@link #startingPort}
	...
	private void writeCQLQuery() {
		try {
			...
			basePath = basePath.substring(10);//10 if running in jar file! 6 else
	...
	/**
	 * Updates Tuple and provides sensordata by iterating through the context variables of the Place object.
	 * Realizes the frequency of the SensorObject through a thread.sleep call.
	 */
	@Override
	...
	/**
	 * Read out the places infofields (context variables) and create the "CREATE STREAM" query  by identifying the correct StreamSQL DataTypes.
	 */
			...
			if (!infofields.isEmpty()) {
				...
				for (String infoKey : place.getInfoKeys()) {
					...
					if (info == null) {
						...
						String infoFlattened = info.flatten().toString();
						String[] infoSplitted = infoFlattened.split(":");
						// Identify the correct STREAM-SQL Datatype and write the concerning substring in the cQLQuery variable
						if(infoSplitted[0].equals("FloatNumber")){
