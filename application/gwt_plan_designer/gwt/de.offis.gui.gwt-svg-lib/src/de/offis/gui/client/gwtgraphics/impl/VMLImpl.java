...
import de.offis.gui.client.gwtgraphics.impl.util.VMLUtil;
...
public class VMLImpl extends SVGImpl {
    ...
    private void setXY(Element element, int xy, boolean x, boolean attached) {
        ...
        if (tagName.equals("line")) {
            ...
            if (isTextElement(element)) {
                ...
                xy = xy
                        - NumberUtil.parseIntValue(element.getStyle().getProperty(x ? "width" : "height"), 0) / 2;
            }
    ...
    private void setTextFont(Element element, boolean attached) {
        VMLUtil.getOrCreateChildElementWithTagName(element, "textpath").getStyle().setProperty(
                "font",
