package de.uniol.inf.is.odysseus.visualquerylanguage.view.position;
...
public final class SugiyamaPositioner implements INodePositioner<INodeContent> {
	...
	private static final int SPACE_HEIGHT = 100; // Vertikaler Abstand
	private HashMap< INodeView<INodeContent>, Integer> nodeLevels;
	private ArrayList< ArrayList<INodeView<INodeContent>>> layers;
	private int maxLevel = Integer.MIN_VALUE;

	private final ISymbolElementFactory<INodeContent> symbolFactory;
	private final SymbolElementInfo dummySymbolInfo;
	
	public SugiyamaPositioner( ISymbolElementFactory<INodeContent> symbolFactory ) {
	...
	@Override
	public void positionize( IGraphView<INodeContent> graph, int width, int height ) {
...
//		((Log4JLogger)logger).getLogger().setLevel(Level.TRACE);
		...
		nodeLevels = new HashMap< INodeView<INodeContent>, Integer>();
		...
		int found = 0;
		for( INodeView<INodeContent> node : graph.getViewedNodes() ) {
		...
		for( IConnectionView<INodeContent> conn : graph.getViewedConnections().toArray( new IConnectionView[0] ) ) {
			INodeView<INodeContent> start = conn.getViewedStartNode();
			INodeView<INodeContent> end = conn.getViewedEndNode();
			...
			if( endLevel > startLevel + 1 ) {
				...
				INodeView<INodeContent> startNode = start;
				for( int i = startLevel + 1; i < endLevel ; i++ ) {
					
					//unsichtbaren Knoten erzeugen
					INodeView<INodeContent> dummyNode = new DefaultNodeView<INodeContent>(null);
					...
					IConnectionView<INodeContent> dummyConnection = new DefaultConnectionView<INodeContent>(conn.getModelConnection(), startNode, dummyNode );
					final AbstractSWTConnectionSymbolElement<INodeContent> ele = (AbstractSWTConnectionSymbolElement<INodeContent>)symbolFactory.createForConnection( "Normal" ); 
				...
				IConnectionView<INodeContent> dummyConnection = new DefaultConnectionView<INodeContent>(conn.getModelConnection(), startNode, end );
				final AbstractSWTConnectionSymbolElement<INodeContent> ele = (AbstractSWTConnectionSymbolElement<INodeContent>)symbolFactory.createForConnection( "Arrow" ); 
				ele.setConnectionView( dummyConnection );
				dummyConnection.getSymbolContainer().add( ele );
				graph.insertViewedConnection( dummyConnection );

				graph.insertViewedConnection( new DefaultConnectionView<INodeContent>(conn.getModelConnection(), startNode, end) );
		...
		layers = new ArrayList< ArrayList<INodeView<INodeContent>>>();
		for( int i = 0; i < maxLevel + 1; i++ )
			layers.add( new ArrayList<INodeView<INodeContent>>() );

		for( INodeView<INodeContent> node : nodeLevels.keySet()) {
			int l = nodeLevels.get( node );
			ArrayList<INodeView<INodeContent>> n = layers.get( l );
			n.add( node );
		}
		...
		for( int layer = 0; layer < layers.size(); layer++) {
			...
			for( int index = 0; index < layers.get( layer ).size(); index++ ) {
				final INodeView<INodeContent> currNode = layers.get( layer ).get( index );
				final int currWidth = currNode.getWidth();
				
				posXRight[layer][index] = 0;
				if( layer > 0 ) {
					final int med = getMedian( currNode, layers.get( layer - 1 ), true );
					final INodeView<INodeContent> parentNode = layers.get( layer-1 ).get( med );
		...
		for( int layer = 0; layer < layers.size(); layer++) {
			...
			for( int index = layers.get( layer ).size() - 1; index >= 0; index-- ) {
				INodeView<INodeContent> currNode = layers.get( layer ).get( index );
				currNode.setPosition( new Vector(( posXRight[layer][index] + posXLeft[layer][index] ) / 2, posY ) );
			}
		}
	}
	
	private int getMedian( INodeView<INodeContent> node, ArrayList<INodeView<INodeContent>> fixedLayerNodes, boolean fromTop ) {
		// Nachfolgerknoten holen
		ArrayList<INodeView<INodeContent>> neighbours = new ArrayList<INodeView<INodeContent>>();
		Collection<? extends IConnectionView<INodeContent>> conns = (fromTop == true ) ? node.getConnectionsAsEnd() : node.getConnectionsAsStart();
		for( IConnectionView<INodeContent> conn : conns ) {
	...
	private void traverse( INodeView<INodeContent> node, int level ) {
		if( node == null ) {
		...
		final Collection<? extends IConnectionView<INodeContent>> connections = node.getConnectionsAsStart();
		
		for( IConnectionView<INodeContent> conn : connections ) {
	...
	private boolean sortByMedians( ArrayList<INodeView<INodeContent>> nodes, ArrayList<Integer> medians ) {
		...
		for( int p = 0; p < medians.size(); p++ ) {
			for( int q = 0; q < medians.size() - 1; q++) {
				if( medians.get( q ) > medians.get(q + 1)) {
					...
					medians.set( q + 1, tmp );
					INodeView<INodeContent> tmpDisplay = nodes.get( q );
