...
public class SWTDiagramFactory implements IDiagramFactory {
	...
	public <In,Out> DiagramConverterPair< In, Out > create( DiagramInfo info ) {
		...
		IStreamDiagram<?> dia = createDiagram( info.getDiagramType(), info.getParameters() );
	...
	private AbstractSWTDiagram< ? > createDiagram( String type, Parameters params ) {
		if( "list".equals(type))
			return new SWTListDiagram(composite, params);
		if( "histogram".equals( type ))
			return new SWTHistogram(composite, params);
		if( "line".equals( type ))
			return new SWTLineDiagram(composite, params );
