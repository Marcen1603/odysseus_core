...
public class SWTMainWindow extends AbstractView<IPhysicalOperator> 
	implements SWTInfoPanel.ISWTInfoPanelListener, ISelectListener<INodeView<IPhysicalOperator>> {
	...
	private final IRenderManager<IPhysicalOperator> renderManager;
	private final SWTStatusLine statusText;
	private final Label zoomText;
	private final ScrolledComposite infoScroll;
	private final Composite infoArea;
	private final Composite canvasComposite;
		
	private IGraphView<IPhysicalOperator> graphView;
	...
	public SWTMainWindow(Shell comp, IController<IPhysicalOperator> ctrl, boolean useOGL ) {
		...
		selectAllMenuItem.addSelectionListener( new SelectionAdapter() {
			...
			public void widgetSelected( SelectionEvent e ) {
				if( graphView != null ) {
					...
					renderManager.getSelector().select(graphView.getViewedNodes());
				}
			}
		});
		...
		reverseSelectMenuItem.addSelectionListener( new SelectionAdapter() {
			...
			public void widgetSelected( SelectionEvent e ) {
				if( graphView != null ) {
					Collection<INodeView<IPhysicalOperator>> selectedNodes = renderManager.getSelector().getSelected();
					...
					renderManager.getSelector().unselectAll();
					renderManager.getSelector().select( nodesToSelect);
				}
			}
		});
		...
		selectPathMenuItem.addSelectionListener( new SelectionAdapter() {
			...
			@Override
			public void widgetSelected( SelectionEvent e ) {
				if( renderManager.getSelector().getSelected().size() != 2 ) 
					return;
				INodeView<IPhysicalOperator>[] selectedNodes = renderManager.getSelector().getSelected().toArray( new INodeView[0] );				
				renderManager.getSelector().unselectAll();
				if(!((GraphSelector<IPhysicalOperator>)renderManager.getSelector()).selectPath( selectedNodes[0], selectedNodes[1] )){
		...
		resetViewMenuItem.addSelectionListener( new SelectionAdapter() {
			@Override
			public void widgetSelected( SelectionEvent e ) {
				renderManager.resetZoom();
				renderManager.resetGraphOffset();
		...
		stdZoomMenuItem.addSelectionListener( new SelectionAdapter() {
			@Override
			public void widgetSelected( SelectionEvent e ) {
				renderManager.zoom( 1.0f - renderManager.getZoomFactor() );
				updateZoomText();
			}
		});
		...
		allVisibleMenuItem.addSelectionListener( new SelectionAdapter() {
			...
			public void widgetSelected( SelectionEvent e ) {
				if( graphView != null ) {
					setVisible( graphView.getViewedNodes(), true );
					updateStatusText();
					renderManager.refreshView();
				}
			}
		...
		hideMenuItem.addSelectionListener( new SelectionAdapter() {
			@Override
			public void widgetSelected( SelectionEvent e ) {
				setVisible( renderManager.getSelector().getSelected(), false );
				renderManager.getSelector().unselectAll();
				updateStatusText();
				renderManager.refreshView();
			}
		...
		allDisableMenuItem.addSelectionListener( new SelectionAdapter() {
			...
			public void widgetSelected( SelectionEvent e ) {
				if( graphView != null )
					setEnabled( graphView.getViewedNodes(), false);
			}
		});
		...
		disableMenuItem.addSelectionListener( new SelectionAdapter() {
			@Override
			public void widgetSelected( SelectionEvent e ) {
				setEnabled( renderManager.getSelector().getSelected(), false);
			}
		});
		...
		allEnableMenuItem.addSelectionListener( new SelectionAdapter() {
			...
			public void widgetSelected( SelectionEvent e ) {
				if( graphView != null )
					setEnabled( graphView.getViewedNodes(), true);
			}
		});
		...
		enableMenuItem.addSelectionListener( new SelectionAdapter() {
			@Override
			public void widgetSelected( SelectionEvent e ) {
				setEnabled( renderManager.getSelector().getSelected(), true);
			}
		});
		
		//OPTIONEN
		MenuItem reloadModelMenuItem = new MenuItem( configSubMenu, SWT.PUSH );
		reloadModelMenuItem.setText( "Modell anlegen\tF5" );
		reloadModelMenuItem.setAccelerator( SWT.F5 );
		reloadModelMenuItem.addSelectionListener( new SelectionAdapter() {
			@Override
			public void widgetSelected( SelectionEvent e ) {
		...
		MenuItem reloadSymbolCfgMenuItem = new MenuItem( configSubMenu, SWT.PUSH );
		reloadSymbolCfgMenuItem.setText( "Konfiguration neu laden" );
		reloadSymbolCfgMenuItem.addSelectionListener( new SelectionAdapter() {
			@Override
			public void widgetSelected( SelectionEvent e ) {
				loadConfigurations();
				updateGraphDisplay();
		...
		// Graphenbereich
		Composite graphArea = new Composite(shell, 0);
		...
		canvasComposite = new Composite( graphArea, SWT.BORDER );
		...
		// Informationsbereich
		infoScroll = new ScrolledComposite(graphArea, SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL);
		...
		// Text für den aktuellen Zoomfaktor
		zoomText = new Label(statusRight, SWT.SHADOW_IN );
		zoomText.setText( "100 %" );		
		canvasComposite.addMouseWheelListener( new MouseWheelListener() {
			@Override
...
//		if( useOGL ) {
//			defaultSymbolFactory = new JOGLSymbolElementFactory<IPhysicalOperator>();
//			renderManager = new JOGLRenderManager<IPhysicalOperator>( canvasComposite, statusText, new SugiyamaPositioner(defaultSymbolFactory) );
//			renderManager.getSelector().addSelectListener( this );
//		} else {
			defaultSymbolFactory = new SWTSymbolElementFactory<IPhysicalOperator>();
			renderManager = new SWTRenderManager<IPhysicalOperator>(canvasComposite, statusText, new SugiyamaPositioner(defaultSymbolFactory) );
			renderManager.getSelector().addSelectListener( this );
//		}
		...
		if( renderManager instanceof SWTRenderManager<?> ) {
			...
			final SWTRenderManager<IPhysicalOperator> rm = (SWTRenderManager<IPhysicalOperator>)renderManager;
	...
	public void reloadModel() {
		...
		try{
			getController().refreshModel();
			connectWithModel();
			
			// Aus neues Modell die View erzeugen
			if( graphView != null ) {
				updateGraphDisplay();
				renderManager.resetPositions();
				statusText.setText( "Modell neu geladen" );
			} else {
				loadConfigurations();
				loadGraphDisplay();
				renderManager.resetZoom();
				...
				renderManager.resetPositions();
				renderManager.refreshView();
				statusText.setText( "Modell geladen" );
			}
		}catch(Exception ex){
	...
	private void disconnectWithModel() {
		if( graphView != null ) {
			for( INodeView<IPhysicalOperator> node : graphView.getViewedNodes() ) {
				if( node instanceof IOdysseusNodeView )
	...
	private void loadGraphDisplay() {
		createGraphDisplay( DEFAULT_GRAPH_FACTORY );
		this.renderManager.setDisplayedGraph( graphView );
	}
	
	private void updateGraphDisplay() {
		updateGraphDisplay( DEFAULT_GRAPH_FACTORY );
		renderManager.setDisplayedGraph( graphView );
	}
	
	@Override
	public void createGraphDisplay( IGraphViewFactory<IPhysicalOperator> graphFactory ) {
		
		if( graphFactory == null )  
			throw new IllegalArgumentException("graphFactory is null!");
		
		
		graphView = graphFactory.createGraphDisplay(getController().getModel(), symbolConfiguration,  defaultSymbolFactory );
	}
	
	@Override
	public void updateGraphDisplay( IGraphViewFactory<IPhysicalOperator> graphFactory ) {
		
		if( graphFactory == null )  
			throw new IllegalArgumentException("graphFactory is null!");
				
		graphFactory.updateGraphDisplay( getController().getModel(), symbolConfiguration,  defaultSymbolFactory, graphView );
	}
	...
	private void updateStatusText() {
		...
		if( graphView != null ) {
			int count = 0;
			for( INodeView<IPhysicalOperator> display : graphView.getViewedNodes() ) {
	...
	private void updateZoomText() {
		zoomText.setText( String.valueOf( (int)(100.0f * renderManager.getZoomFactor() )) + " %" );
	}
	
	private void showMiniMap() {
		if( !useOGL ) {
			((SWTRenderManager<IPhysicalOperator>)renderManager).showMinimap();
		}
	}
	...
	private void addInfoPanel( INodeView<IPhysicalOperator> node ) {
		if( !showInfoPanels.containsKey( node )) {
			...
			SWTInfoPanel p = new SWTInfoPanel(infoArea, (IOdysseusNodeView)node );
	...
	public void pinChanged( SWTInfoPanel sender ) {
		if( !sender.isPinned() ) {
			...
			if( showInfoPanels.containsKey( node ) ) {
				if( !renderManager.getSelector().getSelected().contains( node )) {
	...
	public void selectNode( SWTInfoPanel sender ) {
		if( renderManager.getSelector().getSelected().size() == 1 && renderManager.getSelector().getSelected().contains( sender.getNodeDisplay() ))
			return;
		
		renderManager.getSelector().unselectAll();
		renderManager.getSelector().select( sender.getNodeDisplay() );
		renderManager.refreshView();
	}
