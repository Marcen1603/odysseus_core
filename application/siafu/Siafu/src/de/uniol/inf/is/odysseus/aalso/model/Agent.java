...
package de.uniol.inf.is.odysseus.aalso.model;
...
import de.uniol.inf.is.odysseus.aalso.exceptions.InfoUndefinedException;
import de.uniol.inf.is.odysseus.aalso.exceptions.InitializationRequiredException;
...
import de.uniol.inf.is.odysseus.aalso.utils.SequentialNamer;
 ...
 * </ul>
 * 
 * @see de.uniol.inf.is.odysseus.aalso.behaviormodels.BaseAgentModel
...
public class Agent implements Trackable, Comparable<Agent> {
     ...
     *            all the agents in the simulation.
     */
    public Agent(final String name, final Position start, final String image, final World world) {
     ...
     *            agents.
     */
    public Agent(final String name, final Position start, final String image, final World world, final int zPriority) {
        basicChecks(world);
        this.name = name;
     ...
     */
    private static void basicChecks(final World thisAgentsWorld) {
        if (Agent.world == null) {
            throw new InitializationRequiredException("You need to initialize the agents.");
        }
        if (Agent.world != thisAgentsWorld) {
            throw new RuntimeException("All your users must belong to the same world");
        }
    }
     ...
     * mechanism, see {@link de.uniol.inf.is.odysseus.aalso.model.Gradient}.
     */
     ...
     */
    public void moveInDirection(final int moveDir) throws PositionUnreachableException {
        if (moveDir == -1) {
     ...
     * {@link de.uniol.inf.is.odysseus.aalso.model.Gradient}.
     * 
    ...
    public void moveTowardsPlace(final Place place) {
        try {
            ...
            System.err.println("Agent '" + this + "' can't reach '" + place + "' at '" + pos + "'");
        }
    }
     ...
     * @see #wander()
     */
    public void wanderAround(final Place place, final int radius, final int soberness) {
    ...
    public void wander(final int soberness) {
        ...
        if (tries == POSSIBLE_DIRECTIONS) {
            System.err.println("My name's " + name + " and you've got me stuck in a " + "one pixel wide room! I have rights!");
        }
     ...
     */
    @Override
    public FlatData getContext(final String ctxName) throws UnknownContextException {
    ...
    public int hashCode() {
        ...
        result = prime * result + ((previousImage == null) ? 0 : previousImage.hashCode());
