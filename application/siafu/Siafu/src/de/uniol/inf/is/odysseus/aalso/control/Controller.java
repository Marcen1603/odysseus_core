...
package de.uniol.inf.is.odysseus.aalso.control;
...
import de.uniol.inf.is.odysseus.aalso.control.progress.ConsoleProgress;
import de.uniol.inf.is.odysseus.aalso.control.progress.GUIProgress;
import de.uniol.inf.is.odysseus.aalso.control.progress.Progress;
import de.uniol.inf.is.odysseus.aalso.exceptions.GUINotReadyException;
import de.uniol.inf.is.odysseus.aalso.externalCommand.CommandListener;
import de.uniol.inf.is.odysseus.aalso.graphics.GUI;
import de.uniol.inf.is.odysseus.aalso.graphics.Markers;
import de.uniol.inf.is.odysseus.aalso.graphics.markers.Marker;
import de.uniol.inf.is.odysseus.aalso.model.SimulationData;
...
public class Controller {
    ...
    public static final String DEFAULT_GRADIENT_PATH = System.getProperty("user.home") + File.separator + ".Siafu" + File.separator + "CalculatedGradients" + File.separator;

    /** Default config file location. */
    public static final String DEFAULT_CONFIG_FILE = System.getProperty("user.home") + File.separator + ".Siafu" + File.separator + "config.xml";
    ...
    public Controller(final String configPath, final String simulationPath) {
        ...
        if (config.getBoolean("commandlistener.enable")) {
            ...
            try {
                ...
                System.err.println("The TCP port " + tcpPort + " is already in use. Is there another copy of " + "Siafu running? Consider changing the port " + "number in the config file.");
                return;
            }
        }
        ...
        if (guiUsed) {
            ...
            // work. Die.
            System.err.println("Please activate the GUI in the config file or provide a simulation at the command line.");
            System.exit(1);
        }
     ...
     */
    private static XMLConfiguration createDefaultConfigFile() {
        ...
        newConfig.setProperty("output.csv.path", System.getProperty("user.home") + File.separator + "SiafuContext.csv");
    ...
    public void addMarker(final Marker m) throws GUINotReadyException {
        if (guiUsed) {
            if (gui.canReceiveCommands()) {
                ...
                throw new GUINotReadyException("Can't add tracks until the simulation has started.");
            }
        }
    }
    ...
    public void unMarkAll() throws GUINotReadyException {
        if (guiUsed) {
            if (gui.canReceiveCommands()) {
                ...
                throw new GUINotReadyException("Can't remove tracks until the simulation " + "has started.");
            }
        }
    }
    ...
    public boolean isMarked(final Trackable t) throws GUINotReadyException {
        if (guiUsed) {
            if (!gui.canReceiveCommands()) {
                throw new GUINotReadyException("Can't check tracks until the simulation has started.");                
            }
