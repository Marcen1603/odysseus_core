...
public class GenericProvider extends AbstractDataGenerator {
	...
	public GenericProvider(final String schemaFile, final long frequency,
			final String out, final long number) throws IOException {
		this.schemaFile = schemaFile;
		this.frequency = frequency;
		this.out = out;
		this.number = number;
			...
			if ((this.number < 0) || (this.count < this.number)) {
				for (final String attribute : this.generators.keySet()) {
					if (this.generators.get(attribute) instanceof IMultiValueGenerator) {
						final double[] value = ((IMultiValueGenerator) this.generators
								.get(attribute)).nextValue();
						...
						tuple.addDouble(((ISingleValueGenerator) this.generators
								.get(attribute)).nextValue());
					}
				}
				if (this.writer != null) {
		...
		if (file == null) {
			try {
				...
				if (!schemaPath.exists()) {
					schemaPath = new File(System.getProperty("user.home")
							+ File.separator + this.schemaFile);
				}
		...
		if (file != null) {
			try {
				...
				while ((entry = reader.readLine()) != null) {
					...
					if (attributeParameter.length >= 2) {
						try {
							...
							// + "Generator");
							final Class<?> generatorClass = Activator
									.getGeneratorClass((attributeParameter[1] + "Generator")
											.toUpperCase());

							if (generatorClass != null) {
								final Constructor<?>[] constructors = generatorClass
										.getDeclaredConstructors();
								for (final Constructor<?> constructor : constructors) {
									final Class<?>[] params = constructor
											.getParameterTypes();
									...
									if (params.length == ((attributeParameter.length - 2) + 1)) {
										...
										for (int i = 2; i < attributeParameter.length; i++) {
											if (params[i - 1] == boolean.class) {
												args[i - 1] = Boolean
														.parseBoolean(attributeParameter[i]);
											} else if (params[i - 1] == byte.class) {
												args[i - 1] = Byte
														.parseByte(attributeParameter[i]);
											} else if (params[i - 1] == short.class) {
												args[i - 1] = Short
														.parseShort(attributeParameter[i]);
											} else if (params[i - 1] == int.class) {
												args[i - 1] = Integer
														.parseInt(attributeParameter[i]);
											} else if (params[i - 1] == float.class) {
												args[i - 1] = Float
														.parseFloat(attributeParameter[i]);
											} else if (params[i - 1] == double.class) {
												args[i - 1] = Double
														.parseDouble(attributeParameter[i]);
											} else if (params[i - 1] == Double[].class) {
												final String[] column = attributeParameter[i]
														.split(",");
												final Double[] value = new Double[column.length];
												for (int c = 0; c < column.length; c++) {
													value[c] = Double
															.parseDouble(column[c]);
												}
												args[i - 1] = value;
											} else if (params[i - 1] == Double[][].class) {
												final String[] row = attributeParameter[i]
														.split(";");
												if (row.length > 0) {
													String[] column = row[0]
															.split(",");
										...
										generator = (IValueGenerator) constructor
												.newInstance(args);
										config.append(String
												.format("Attribute %s initialized with %s(%s)\n",
														attributeParameter[0],
														generatorClass
																.getSimpleName(),
								...
								throw new IllegalArgumentException(
										"No value generator found");
							}
							if (generator != null) {
								generator.init();
								this.generators.put(attributeParameter[0],
										generator);
							} else {
								throw new IllegalArgumentException(
										"No value generator found");
				...
				for (final String attribute : this.generators.keySet()) {
					...
					if (this.generators.get(attribute) instanceof IMultiValueGenerator) {
						final IMultiValueGenerator generator = (IMultiValueGenerator) this.generators
								.get(attribute);
				...
				schema.insert(0,
						"Use the following statement to access the stream\nATTACH STREAM generator (");
	...
	public static void main(final String[] args) {
		...
		if (out != null) {
			System.out.println("Generator started with -p " + port + " -f "
					+ frequency + " -s " + schemaFile + " -n " + number
					+ " -o " + out);
		} else {
			System.out.println("Generator started with -p " + port + " -f "
					+ frequency + " -s " + schemaFile + " -n " + number);
		...
		try {
			if (out != null) {
				GenericProvider generator = new GenericProvider(schemaFile,
						frequency, out, number);
				generator.process_init();
				for (;;) {
					List<DataTuple> next = generator.next();
					if (next.isEmpty()) {
						break;
					}
				}
			} else {
				StreamServer genericServer = new StreamServer(port,
						new GenericProvider(schemaFile, frequency));
				genericServer.start();
			}
