...
public class VisibilityGenerator extends AbstractDataGenerator {
	...
	private ISingleValueGenerator visibility;
	...
	public void process_init() {
		// CREATE STREAM visibility (timestamp LONG, visibility INTEGER, name STRING, location INTEGER) CHANNEL localhost : 57006;
				...
				// Temperature
				visibility = new UniformDistributionGenerator(
						new ContinuousErrorModel(new DuplicateNoise(), 3), 100, 1000);
				visibility.init();
...
//		}
		...
		if (tupleCounter >= CHANGE_MARKER && !changed) {
			...
			visibility = new UniformDistributionGenerator(
					new ContinuousErrorModel(new DuplicateNoise(), 3), 20, 110);
			visibility.init();
