 ...
 */
public class SubBooleanGrid extends AbstractFunction<Boolean[][]> {

    /**
     * 
     */
    private static final long serialVersionUID = 383665331384700799L;
    ...
    public String getSymbol() {
        return "SubBooleanGrid";
    }

    @Override
    public Boolean[][] getValue() {
        final Boolean[][] grid = (Boolean[][]) this.getInputValue(0);
        ...
        if ((startX >= endX) && (startY >= endY)) {
            subgrid = new Boolean[1][1];
            startX = endX - 1;
            startY = endY - 1;
        }
        else if (startX >= endX) {
            subgrid = new Boolean[1][endY - startY];
            startX = endX - 1;
        }
        else if (startY >= endY) {
            subgrid = new Boolean[endX - startX][1];
            startY = endY - 1;
        }
        else {
            subgrid = new Boolean[endX - startX][endY - startY];
        }
