		...
		private void dumpPackage(final ByteBuffer buffer)
			...
			if ((debugChannel != null) && (debugChannel.isOpen())) {
				try {
					...
					SickConnectionImpl.LOG.severe(e.getMessage());
				}
			}
		}
		...
		private void onMessage(final String message, final long timestamp)
			...
			if (message.startsWith(SickConnectionImpl.SRA)) {
				if (SickConnectionImpl.LCM_STATE.equalsIgnoreCase(data[1])) {
					...
					SickConnectionImpl.LOG.warning(String.format("Dirtyness %s ",
							dirtyness));
				}
...
//									}
	...
	private static final Logger LOG = Logger.getLogger(SickConnection.class.getName());
    ...
    public void onMeasurement(final Measurement measurement,
		...
		if ((channel != null) && (channel.isOpen())) {
			...
			charBuffer.append(measurement.getDevice() + ",");
			charBuffer.append(measurement.getSerial() + ",");
			charBuffer.append(measurement.getStatus() + ",");
			charBuffer.append(measurement.getMessageCount() + ",");
			charBuffer.append(measurement.getScanCount() + ",");
			charBuffer.append(measurement.getPowerUpDuration() + ",");
			charBuffer.append(measurement.getTransmissionDuration() + ",");
			charBuffer.append(measurement.getScanningFrequency() + ",");
