...
import de.uniol.inf.is.odysseus.aalso.behaviormodels.BaseContextModel;
import de.uniol.inf.is.odysseus.aalso.behaviormodels.BaseWorldModel;
...
import de.uniol.inf.is.odysseus.aalso.output.AalsoOutputPrinter;
import de.uniol.inf.is.odysseus.aalso.output.CSVPrinter;
import de.uniol.inf.is.odysseus.aalso.output.NullPrinter;
import de.uniol.inf.is.odysseus.aalso.output.SimulatorOutputPrinter;
...
public class Simulation implements Runnable {
	...
	public Simulation(final String simulationPath, final Controller control) {
		...
		Thread simulation = new Thread(this, "Simulation thread");
		simulation.start();
	}
	...
	class Dialog extends JFrame {
		...
		protected void initWindow(String text) {
			...
			pfad = new JLabel(text);
			pfad.setBounds(5,10,1010,25);
			
			// Elemente dem Fenster hinzufügen:
			this.getContentPane().add(pfad);
			
			this.pack();
		}
		
		public void called(){
			pfad = new JLabel("Called");
			pfad.setBounds(5,10,1010,25);
	 ...
	 * 
	 * @see de.uniol.inf.is.odysseus.aalso.output.SimulatorOutputPrinter
	 */
	private SimulatorOutputPrinter createOutputPrinter(final String type) {
		...
		} else if(type.equalsIgnoreCase("odys")) {
			try {
				return new AalsoOutputPrinter(world, siafuConfig);
			} catch (Exception e) {
				e.printStackTrace();
				throw new RuntimeException(e);
			}
		} else {
			throw new RuntimeException("Unknown ouput type in the configuration");
		}
	}
