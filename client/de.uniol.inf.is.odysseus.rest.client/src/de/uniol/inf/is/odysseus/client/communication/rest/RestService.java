...
import de.uniol.inf.is.odysseus.client.communication.dto.*;
...
public class RestService {
    ...
    public static Map<String, Map<Integer, SocketInfo>> runQuery(String ip, String token, String query)
        ...
        try {
			...
			/*
             * WARNING Here we have a simplification that is not always true.
			 * What if more than one query was installed? Use
			 * installAndConnectQuery instead or to the desired connection
			 * manually and just install the query with installQuery
			 */
     ...
     * @param token The security token from the login
     * @param query The query to run
     * @return A map with the id of the query -> name of the operator -> the output port of this operator -> the
     * socketInformation
     * @throws RestException
     */
    @SuppressWarnings("unchecked")
     ...
     */
    @SuppressWarnings("unchecked")
    public static Collection<Double> installQuery(String ip, String token, String query) {
        ClientResource crAddQuery = new ClientResource(getHostURL(ip, SERVICE_PATH_CORE, RESOURCE_PATH_ADD_QUERY));
     ...
     * @return The outer map has the name of the operator as key, the inner map the output port of the operator
     * @throws RestException
     */
