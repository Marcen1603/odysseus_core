...
public abstract class AbstractCanvasDashboardPart extends AbstractDashboardPart implements PaintListener {
    ...
    public void drawPointer(final int x, final int y, final int radius, final double angle, final IColorSpace color) {
        ...
        final Pattern pattern = new Pattern(this.gc.getDevice(), 0, 0, (2 * scale) - 4, (2 * scale) - 4, this.toColor(complements[0]), this.toColor(complements[1]));
        this.gc.setBackgroundPattern(pattern);
        this.gc.fillArc((-1 * scale) + 2, (-1 * scale) + 2, (2 * scale) - 4, (2 * scale) - 4, 0, 360);
    ...
    public void drawXAxis(final int x, final int y, final int width, final double from, final double to, final IColorSpace color) {
        ...
        for (int i = 0; (i / scale) < (width - 5); i += ticksize) {
            ...
            this.gc.drawText(value + "", (int) (tickX - (extent.x / 2)), y + 6, true);
        }
    ...
    public void drawYAxis(final int x, final int y, final int height, final double from, final double to, final IColorSpace color) {
        final Color tmpForegroundColor = this.gc.getForeground();
        final Color tmpBackgroundColor = this.gc.getBackground();
        this.gc.setForeground(this.toColor(color));
        this.gc.setBackground(this.toColor(color));
        ...
        for (int i = 0; (i / scale) < (height - 5); i += ticksize) {
            ...
            this.gc.drawText(value + "", (int) (x - extent.x - 6), (int) (tickY - (extent.y / 2)), true);
        }
        final Path arrow = new Path(this.getGC().getDevice());
        arrow.moveTo(x, y);
    ...
    public void drawGrid(final int x, final int y, final int width, final int height, final double fromX, final double toX, final double fromY, final double toY, final IColorSpace color) {
        final Color tmpForegroundColor = this.gc.getForeground();
        final Color tmpBackgroundColor = this.gc.getBackground();
        this.gc.setForeground(this.toColor(color));
        this.gc.setBackground(this.toColor(color));
        ...
        for (int i = 0; (i / scaleX) < (width - 5); i += ticksizeX) {
            ...
            this.gc.drawText(value + "", (int) (tickX - (extent.x / 2)), (y + height) + 6, true);
        }
    ...
    public void drawRoundRectangle(final Coordinate corner, final int width, final int height, final int arcWidth, final int arcHeight, final IColorSpace color) {
        final Color tmpColor = this.gc.getForeground();
        this.gc.setForeground(this.toColor(color));
        this.gc.drawRoundRectangle((int) corner.x, (int) corner.y, width, height, arcWidth, arcHeight);
        this.gc.setForeground(tmpColor);
    }

    public void drawArc(final Coordinate center, final int radius, final int start, final int end, final IColorSpace color) {
        final Color tmpColor = this.gc.getForeground();
        this.gc.setForeground(this.toColor(color));
    ...
    public void fillRoundRectangle(final Coordinate corner, final int width, final int height, final int arcWidth, final int arcHeight, final IColorSpace color) {
        final Color tmpColor = this.gc.getBackground();
        this.gc.setBackground(this.toColor(color));
        this.gc.fillRoundRectangle((int) corner.x, (int) corner.y, width, height, arcWidth, arcHeight);
        this.gc.setBackground(tmpColor);
    }

    public void fillArc(final Coordinate center, final int radius, final int start, final int end, final IColorSpace color) {
        final Color tmpColor = this.gc.getBackground();
        this.gc.setBackground(this.toColor(color));
    ...
    public void fillArc(final Coordinate center, final int radius, final int start, final int end, final IColorSpace color1, final IColorSpace color2) {
        final Color tmpColor = this.gc.getBackground();
        final Pattern pattern = new Pattern(this.gc.getDevice(), (int) (center.x - radius), (int) (center.y - radius), 2 * radius, 2 * radius, this.toColor(color1), this.toColor(color2));
        this.gc.setBackgroundPattern(pattern);
        this.gc.fillArc((int) (center.x - radius), (int) (center.y - radius), 2 * radius, 2 * radius, start, end);
        pattern.dispose();
        this.gc.setBackgroundPattern(null);
    ...
    public void fillRectangle(final Coordinate corner, final int width, final int height, final IColorSpace color1, final IColorSpace color2) {
        final Color tmpColor = this.gc.getBackground();
        final Pattern pattern = new Pattern(this.gc.getDevice(), (int) corner.x, (int) corner.y, width, height, this.toColor(color1), this.toColor(color2));
        this.gc.setBackgroundPattern(pattern);
        this.gc.fillRectangle((int) corner.x, (int) corner.y, width, height);
        pattern.dispose();
        this.gc.setBackgroundPattern(null);
    ...
    public void fillRoundRectangle(final Coordinate corner, final int width, final int height, final int arcWidth, final int arcHeight, final IColorSpace color1, final IColorSpace color2) {
        final Color tmpColor = this.gc.getBackground();
        final Pattern pattern = new Pattern(this.gc.getDevice(), (int) corner.x, (int) corner.y, width, height, this.toColor(color1), this.toColor(color2));
        this.gc.setBackgroundPattern(pattern);
        this.gc.fillRoundRectangle((int) corner.x, (int) corner.y, width, height, arcWidth, arcHeight);
    ...
    public void fillRectangle(final Coordinate corner, final int width, final int height, final IColorSpace color1, final IColorSpace color2, final boolean vertical) {
        final Color tmpBackgroundColor = this.gc.getBackground();
        final Color tmpForegroundColor = this.gc.getForeground();
        this.gc.setForeground(this.toColor(color1));
        this.gc.setBackground(this.toColor(color2));
        this.gc.fillGradientRectangle((int) corner.x, (int) corner.y, width, height, vertical);
        this.gc.setBackground(tmpBackgroundColor);
        this.gc.setForeground(tmpForegroundColor);
    }

    public void fillRectangle(final Coordinate corner, final int width, final int height, final int[] percents, final IColorSpace[] colors, final boolean vertical) {
        final Color tmpBackgroundColor = this.gc.getBackground();
        if ((colors == null) || (colors.length == 1) || (percents == null || percents.length != colors.length - 1)) {
            ...
            for (int i = 0; i < percents.length; ++i) {
                ...
                if (vertical) {
                    final int gradientHeight = (((percents[i]) * height) / 100) - pos;
                    this.gc.fillGradientRectangle((int) corner.x, (int) (corner.y + pos), width, gradientHeight, vertical);
                    pos += gradientHeight;
                }
                else {
                    final int gradientWidth = (((percents[i]) * width) / 100) - pos;
                    this.gc.fillGradientRectangle((int) (corner.x + pos), (int) corner.y, gradientWidth, height, vertical);
                    pos += gradientWidth;
                }
            }
            if (vertical && (pos < height)) {
                this.gc.setBackground(tmpBackgroundColor);
                this.gc.fillRectangle((int) corner.x, (int) (corner.y + pos), width, height - pos);
            }
            if (!vertical && (pos < width)) {
                this.gc.setBackground(tmpBackgroundColor);
                this.gc.fillRectangle((int) (corner.x + pos), (int) corner.y, width - pos, height);
            }
            this.gc.setForeground(tmpForegroundColor);
        }
        this.gc.setBackground(tmpBackgroundColor);
    ...
    public void fillRectangle(final Coordinate corner, final int width, final int height, final int[] percents, final IColorSpace[] colors) {
        final Color tmpBackgroundColor = this.gc.getBackground();
        if (colors.length == 1) {
            if (colors[0] != null) {
                this.gc.setBackground(this.toColor(colors[0]));
            ...
            for (int i = 0; i < percents.length; ++i) {
                ...
                double cathetusY = Math.abs(d * Math.cos(Math.PI / 2.0 - angle));
                ...
                path.lineTo((float) (corner.x + x2 + 0.5), (float) (corner.y + y2 + 0.5));
                path.lineTo((float) (corner.x + x4 + 0.5), (float) (corner.y + y4 + 0.5));
                if (x4 > 0) {
                ...
                final Pattern pattern = new Pattern(this.gc.getDevice(), 0, 0, patternSize, patternSize, color, lastColor);
                this.gc.setBackgroundPattern(pattern);
                this.gc.fillPath(path);
                pattern.dispose();
                path.dispose();
    ...
    public void fillPath(final Path path, final IColorSpace color1, final IColorSpace color2) {
        final Color tmpColor = this.gc.getBackground();
        final float[] bounds = new float[4];
        path.getBounds(bounds);
        final Pattern pattern = new Pattern(this.gc.getDevice(), bounds[0], bounds[1], bounds[2], bounds[3], this.toColor(color1), this.toColor(color2));
        this.gc.setBackgroundPattern(pattern);
        this.gc.drawPath(path);
        pattern.dispose();
        this.gc.setBackgroundPattern(null);
    ...
    public int getXTickSize(final double from, final double to, final int width) {
        ...
        int ticksize = (int) (Math.abs(to - from) / ((width / (Math.max(extentTo.x, extentFrom.x)))));
        final int digits = ticksize == 0 ? 1 : (1 + (int) Math.floor(Math.log10(Math.abs(ticksize))));
    ...
    public int getYTickSize(final double from, final double to, final int height) {
        final Coordinate extentTo = this.textExtent(to + " ");
        final Coordinate extentFrom = this.textExtent(from + " ");
        int ticksize = (int) (Math.abs(to - from) / ((height / (Math.max(extentTo.y, extentFrom.y)))));
        final int digits = ticksize == 0 ? 1 : (1 + (int) Math.floor(Math.log10(Math.abs(ticksize))));
        if (ticksize > (Math.pow(10, digits) / 2)) {
            ticksize = (int) Math.pow(10, digits);
        }
