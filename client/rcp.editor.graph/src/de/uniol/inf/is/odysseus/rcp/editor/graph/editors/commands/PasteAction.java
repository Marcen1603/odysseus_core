...
public class PasteAction extends WorkbenchPartAction {
	...
	private Command createPasteCopyCommand() {
		Object cb = Clipboard.getDefault().getContents();
		if (!(cb instanceof List)) {
			return null;
		}
		if (!(((List<?>) cb).get(0) instanceof OperatorNode)) {
			return null;
		}
		@SuppressWarnings("unchecked")
		List<OperatorNode> nodes = (List<OperatorNode>) cb;
		return new OperatorNodePasteCommand(nodes);
	}
