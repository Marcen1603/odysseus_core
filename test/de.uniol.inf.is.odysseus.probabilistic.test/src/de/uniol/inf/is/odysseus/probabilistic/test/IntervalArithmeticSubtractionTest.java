...
public class IntervalArithmeticSubtractionTest {
    @Parameters(name = "{index}: {0} - {1} = {2}")
    public static Collection<Object[]> data() {

        return Arrays.asList(new Object[][] { //
                { Interval.of(1.0, 3.0), Interval.of(3.0, 5.0), Interval.of(-4.0, 0.0) }, //

                { Interval.of(-1.0, -3.0), Interval.of(3.0, 5.0), Interval.of(-8.0, -4.0) }, //
                { Interval.of(-3.0, -1.0), Interval.of(3.0, 5.0), Interval.of(-8.0, -4.0) }, //

                { Interval.of(-1.0, 3.0), Interval.of(-3.0, 5.0), Interval.of(-6.0, 6.0) },//
        });
    }
    ...
    public void testSubtractInterval() {
        whenSubtract(left, right);

        thenResultIs(result);
    }

    private void whenSubtract(Interval left, Interval right) {

        System.out.print(String.format("%s - %s == ", left, right));
        final Stopwatch timer = Stopwatch.createStarted();

        tmpResult = left.subtract(right);
