...
public class P2PNewPlugIn implements BundleActivator {
	...
	public void start(BundleContext bundleContext) throws Exception {
		...
		PeerID peerID = IDFactory.newPeerID(PeerGroupID.defaultNetPeerGroupID, PEER_NAME.getBytes());
		configureNetwork(manager.getConfigurator(), peerID);

		PeerGroup netPeerGroup = manager.startNetwork();
		PeerGroup subGroup = createSubGroup(netPeerGroup, SUBGROUP_ID, SUBGROUP_NAME);
		LOG.debug("JXTA-Network started. Peer is in group '{}'", subGroup);

		peerDiscoveryThread = new PeerDiscoveryThread(subGroup.getDiscoveryService(), 3 * 1000);
	...
	private static PeerGroup createSubGroup(PeerGroup parentPeerGroup, PeerGroupID subGroupID, String subGroupName) throws PeerGroupException, IOException, Exception {
		PeerGroup subGroup = parentPeerGroup.newGroup(subGroupID, parentPeerGroup.getAllPurposePeerGroupImplAdvertisement(), subGroupName, "");
		return subGroup;
	}
}
