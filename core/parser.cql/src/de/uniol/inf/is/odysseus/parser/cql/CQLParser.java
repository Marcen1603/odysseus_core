...
import de.uniol.inf.is.odysseus.sla.Window;
import de.uniol.inf.is.odysseus.sla.factories.MetricFactory;
import de.uniol.inf.is.odysseus.sla.factories.PenaltyFactory;
import de.uniol.inf.is.odysseus.sla.factories.ScopeFactory;
import de.uniol.inf.is.odysseus.sla.factories.UnitFactory;
...
public class CQLParser implements NewSQLParserVisitor, IQueryParser {
	...
	public Object visit(ASTDropRoleStatement node, Object data)
		...
		String rolename = ((ASTIdentifier) node.jjtGetChild(0)).getName();
		try {
			IRole role = UserManagement.getUsermanagement().findRole(rolename,
					caller);
			if (role != null) {
	...
	public Object visit(ASTGrantStatement node, Object data)
		...
		IRole role = UserManagement.getUsermanagement().findRole(userName,
				caller);
		if (role != null) {
			for (IPermission action : operations) {
	...
	public Object visit(ASTCreateSLAStatement node, Object data)
		...
		for (int i = 4; i < node.jjtGetNumChildren(); i++) {
			ServiceLevel sl = (ServiceLevel) node.jjtGetChild(i).jjtAccept(
					this, null);
			sl.setSla(sla);
	...
	public Object visit(ASTStreamToStatement node, Object data)
		...
		if (sink.getClass()
				.getCanonicalName()
				.equals("de.uniol.inf.is.odysseus.database.logicaloperator.DatabaseSinkAO")) {
