 ...
 * 
 */
public class OdysseusScriptContentFormatter extends ContentFormatter {

	Map<String, IOdysseusScriptFormattingStrategy> odysseusStrategies = new HashMap<>();	
	...
	/**
	 * Creates a new content formatter. The content formatter operates by
	 * default in the partition-aware mode. There are no preconfigured
	 * formatting strategies. Will use the default document partitioning if not
	 * further configured.
	 */
	public OdysseusScriptContentFormatter() {
	...
	public void setFormattingStrategy(IFormattingStrategy strategy, String contentType) {
		...
		if (strategy instanceof IOdysseusScriptFormattingStrategy) {
			odysseusStrategies.put(contentType, (IOdysseusScriptFormattingStrategy) strategy);
		}
		//****** end - changed for Odysseus *****		
	...
	public void format(IDocument document, IRegion region) {
		...
		//****** start - changed for Odysseus *****		
		for (Map.Entry<String, IOdysseusScriptFormattingStrategy> strat : odysseusStrategies.entrySet()) {
			strat.getValue().setDocument(document);			
		}
		//****** end - changed for Odysseus *****
	...
	private void format(TypedPosition[] ranges) {
		for (int i = 0; i < ranges.length; i++) {
			...
			if (s != null) {
				// start - changed for Odysseus
				if(s instanceof IOdysseusScriptFormattingStrategy){
					((IOdysseusScriptFormattingStrategy)s).setRegion(ranges[i]);
				}
				// end - changed for Odysseus
				format(s, ranges[i]);
			}
