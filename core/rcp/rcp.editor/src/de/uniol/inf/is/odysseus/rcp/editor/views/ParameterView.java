...
public class ParameterView extends ViewPart implements IViewPart,
		ISelectionListener, IParameterView, IDataDictionaryListener {
		...
		if (selection instanceof IStructuredSelection) {
			...
			if (structSelection.size() == 1) {
				...
				if (selectedObject instanceof OperatorEditPart) {
					Operator selectedOperator = (Operator) ((OperatorEditPart) selectedObject)
							.getModel();
	...
	private void createControls() {
		...
		for (IParameter<?> parameter : parameters) {
			...
			if (ParameterEditorRegistry.getInstance().exists(editorName)) {
				IParameterEditor editor = ParameterEditorRegistry.getInstance()
						.create(editorName);
	...
	private void updateErrorList() {
		...
		List<Exception> errors = selectedOperator.getOperatorBuilder()
				.getErrors();
		if (errors.size() == 0)
