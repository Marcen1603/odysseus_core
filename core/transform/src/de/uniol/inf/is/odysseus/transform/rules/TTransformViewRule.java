...
public class TTransformViewRule extends AbstractTransformationRule<AccessAO> {
	...
	public void execute(AccessAO accessAO, TransformationConfiguration transformConfig) {
		...
		ILogicalOperator view = DataDictionary.getInstance().getStreamForTransformation(accessAO.getSource().getURI(), getCaller());
		...
		try {
			roots = transformation.transform(cPlan, transformConfig, getCaller());
