 ...
 * können objektrelational sein. Dazu wird das <code>foreach</code>-Konstrukt
 * genutzt. Die Zählvariable ist vom Typ <code>TupleInfo</code>. Dort sind
 * während der Iteration alle notwendigen Informationen enthalten. Über
 * unterschiedliche Konstruktoren kann bestimmt werden, wie sich der Iterator
 * verhalten soll.
 * <p>
...
public class TupleIterator implements Iterable<TupleInfo>, Iterator<TupleInfo> {
     ...
     * Iteration erfolgt über den angegeben vollständigen Tupel. Das Tupel
     * kann objektrelational sein. Dabei beginnt die Iteration bei dem angegeben
    ...
    /**
     * Konstruktor. Erstellt eine neue <code>TupleIterator</code>-Instanz. Die
     * Iteration erfolgt über den angegeben vollständigen Tupel. Von Anfang
     * bis Ende. Auf die Tiefe wird keine Rücksicht genommen. Das Tupel kann
     ...
     * maximal die angegebene Tiefe berücksichtigt. Die Tiefenangabe ist
     * relativ zur Schemawurzel. Das Tupel kann objektrelational sein. Die
     * Iteration erfolgt über den <b>ganzen</b> Tupel. <br>
     * <br>
    ...
    private void reset(TupleIndexPath tupleStart) {
        ...
        if (start != null) {
            ...
            // e.index = tupleStart.getTupleIndices().get(tupleStart.getLength()
            // - 1).toInt();
            e.index = 0;
            ...
            if (entry.index == size) {
                ...
                if (index.getAttribute().getDatatype().isMultiValue()) {
                    if (index.getAttribute().getDatatype().hasSchema()) {
                        ...
                        schemaIndices.push(new SchemaIndex(0, index.getAttribute())); // FIXME:
                                                                                      // Ich
                                                                                      // hoffe,
                                                                                      // dass
                                                                                      // das
