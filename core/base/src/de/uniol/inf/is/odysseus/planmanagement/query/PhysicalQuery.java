...
public class PhysicalQuery implements IPhysicalQuery{
	...
	private SLA sla;
	private QueryBuildConfiguration qbConfig;
	 ...
	 */
	public PhysicalQuery(List<IPhysicalOperator> physicalPlan, QueryBuildConfiguration qbConfig) {
		id = idCounter++;
	...
	public PhysicalQuery(ILogicalQuery query, ArrayList<IPhysicalOperator> physicalPlan) {
		id = idCounter++;
		this.query = query;		
		this.qbConfig = query.getBuildParameter();
	...
	public List<IPhysicalOperator> setRoots(List<IPhysicalOperator> roots) {
		...
		this.roots = roots;
		QueryBuildConfiguration parameter = null;
		if (qbConfig != null){
			parameter = qbConfig;
		}
		...
		if (parameter != null) {
			...
			if (parameter.getDefaultRoot() != null) {
				defaultRoot = parameter.getDefaultRoot();
				...
				for (IPhysicalOperator oldRoot : this.roots) {
					...
					if (defaultRoot != null && defaultRoot.isSink()
							&& oldRoot.isSource()) {
						IPhysicalOperator cloned = parameter
...
//		}
	...
	@Override
	public void createAndAddMonitoringData(@SuppressWarnings("rawtypes") IPeriodicalMonitoringData item,
			long period) {
