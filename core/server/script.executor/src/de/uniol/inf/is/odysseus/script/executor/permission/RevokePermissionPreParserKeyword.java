 ...
 */
public class RevokePermissionPreParserKeyword extends AbstractPreParserKeyword {

	public static final String NAME = "REVOKEPERMISSIONS";
	...
	public List<IExecutorCommand>  execute(Map<String, Object> variables, String parameter, ISession caller, Context context, IServerExecutor executor) throws OdysseusScriptException {
		...
		for (int i=1;i<para.length-1;i++) {
			operations.add(para[i]);
		}
		List<String> object = new ArrayList<>();
		object.add(para[para.length-1]);
			
		cmds.add(new RevokePermissionCommand(userName, operations, object, caller));
		return cmds;
	}
