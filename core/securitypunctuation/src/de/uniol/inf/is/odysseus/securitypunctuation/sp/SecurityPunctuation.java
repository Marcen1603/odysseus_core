...
public class SecurityPunctuation extends AbstractSecurityPunctuation {
	 ...
	 * Falls beides nicht zutrifft wird null zur�ckgegeben.
	 */
	@Override
	...
	/**
	 * Vereinigt zwei SP und gibt null zur�ck, falls die Bedingungen f�r eine Vereinigung nicht erf�llt sind
	 */
	@Override
	public ISecurityPunctuation union(ISecurityPunctuation sp2) {
		if(sp2 instanceof SecurityPunctuation) {			
			...
			if(sp2.getIntegerAttribute("sign") == 1) {				
				...
				// kleineren W�hlen --> Bereich wird gr��er
				...
				// gr��eren W�hlen --> Bereich wird gr��er
				...
				newSP.setAttribute("sign", 1); //Sign = 0 macht nur Sinn f�r Tuple, die noch unterwegs sind und vorrangegangene Tupel einschr�nken k�nnen
				newSP.setAttribute("mutable", 1); //Falls weitere Tupel ankommen die mutable = 2 haben, soll auch die Verarbeitung m�glich sein.
	...
	public ISecurityPunctuation intersect(ISecurityPunctuation sp2) {		
		if(sp2 instanceof SecurityPunctuation) {
			...
			// gr��eren W�hlen --> Bereich wird kleiner
			if(newSP.getLongAttribute("tupleStartTS") < sp2.getLongAttribute("tupleStartTS")) {
				newSP.setAttribute("tupleStartTS", sp2.getLongAttribute("tupleStartTS"));
			} 
				
			// kleineren W�hlen --> Bereich wird kleiner
			if(newSP.getLongAttribute("tupleEndTS") > sp2.getLongAttribute("tupleEndTS")) {
				newSP.setAttribute("tupleEndTS", sp2.getLongAttribute("tupleEndTS"));
			} 
			...
			newSP.setAttribute("sign", 1); //Sign = 0 macht nur Sinn f�r Tuple, die noch unterwegs sind und vorrangegangene Tupel einschr�nken k�nnen
			newSP.setAttribute("mutable", 0); //Da intersect z.B. beim Join ausgef�hrt wird und die SP danach nicht mehr ge�ndert werden k�nnen sollen.
			if(sp2.getLongAttribute("ts") > newSP.getLongAttribute("ts")) {
				newSP.setAttribute("ts", sp2.getLongAttribute("ts"));
			} 
			return newSP;
