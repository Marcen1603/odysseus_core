...
/**
 * Initializes metadata (of type {@link IProbability}) of a scan tuple
 * (of type {@link MVRelationalTuple}) by initializing metadata of each car tuple.
 * <p>
 ...
 * 
*/
public class CovarianceInitiator<M extends IProbability> 
	extends AbstractMetadataUpdater<M, MVRelationalTuple<M>>
{
	...
	private ArrayList<int[]> mvPaths  = null;
	
	/**
	 * Creates a CovarianceInitiator object and initializes the metadata used
	 * by {@link #updateMetadata(MVRelationalTuple)}.
	 * <p>
	 * As basis for initializing the first car list schema in scanSchema is
	 * used as there is only one car schema for all cars:
	 * <p>
	 ...
	 * <p>
	 * To find the car list schema in the first place {@link
	 * #POS_OF_LIST_IN_SCAN} is used:
	 * <p>
	 * scanSchema.attributes[POS_OF_LIST_IN_SCAN].attributes[0]
	 * <p>
	 * The first car schema is then searched for measurement value attributes
	 * by invoking {@link SDFDatatypes#isMeasurementValue(de.uniol.inf.is.odysseus.sourcedescription.sdf.schema.SDFDatatype)}
	 * for each attribute or subattribute. The paths (ArrayList of int[]) of
	 * the measurement value attributes found are those used by {@link
	 * #updateMetadata(MVRelationalTuple)}. They are set relative to the car
	 * schema rather than to the scan schema.
	 * <p>
	 * Finally, each row of the covariance matrix used by {@link
	 * #updateMetadata(MVRelationalTuple)} is set to the covariance list of the
	 * corresponding measurement value attribute in the schema (see {@link
	 * SDFAttribute#getCovariance()}).
	 * 
	 * @param scanSchema the scan schema
	 */
	...
	private void initMetadataItself(SDFAttributeList scanSchema)
	{
		...
		SDFAttributeList firstCarSchema = OrAttributeResolver.getSubSchema(scanSchema, new int[] { CovarianceInitiator.POS_OF_LIST_IN_SCAN, 0 });
		// TODO wrong path
		
		// get measurement value paths
		this.mvPaths = CovarianceInitiator.getPathsOfMeasurements(firstCarSchema);
		
		// initialize covariance matrix:
		// number of mv attributes * number of mv attributes
		this.covMatrix = new double[this.mvPaths.size()][this.mvPaths.size()];
		...
		for (int[] mvPath : this.mvPaths)
		{
			// and get the corresponding measurement value attribute
			Object mvObject = OrAttributeResolver.getSubSchema(firstCarSchema, mvPath);
	 ...
	 * (of type {@link MVRelationalTuple}) by initializing metadata of each car tuple.
	 * <p>
	 * The metadata used to update scanTuple is being set before. See {@link
	 * #CovarianceInitiator(SDFAttributeList)} for details.
	 * <p>
	 * In order to find the car tuples, {@link #POS_OF_LIST_IN_SCAN} is used.
	 * 
	 * @param scanTuple scan tuple of which metadata (M) should be initialized
	 * 
	 */
	@Override
	...
	private void initMetadataOfScanTuple(MVRelationalTuple<M> scanTuple)
	{
		...
		MVRelationalTuple<M> listTuple = scanTuple.<MVRelationalTuple<M>>getAttribute(CovarianceInitiator.POS_OF_LIST_IN_SCAN);
		// TODO class cast exception
		...
		{
			MVRelationalTuple<M> carTuple = listTuple.<MVRelationalTuple<M>>getAttribute(indexOfCurrCar);
			// TODO class cast exception
	...
	private void initMetadataOfCarTuple(MVRelationalTuple<M> carTuple)
	{
		...
		carTuple.getMetadata().setAttributePaths(this.mvPaths);
