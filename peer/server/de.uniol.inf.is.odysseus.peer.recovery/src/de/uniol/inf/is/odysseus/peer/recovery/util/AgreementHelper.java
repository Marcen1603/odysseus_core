...
public class AgreementHelper {
	 ...
	 * for
	 */
	private static Map<PeerID, List<ID>> cRecoveryPeers = new HashMap<PeerID, List<ID>>();

	/**
	 * Failed peers and the (maybe multiple) shared queries other peers have
	 * detected earlier and want to do the recovery for
	 */
	private static Map<PeerID, List<ID>> cOtherRecoveryPeers = new HashMap<PeerID, List<ID>>();
	 ...
	 */
	public static boolean decideToRecover(PeerID failedPeer, ID sharedQuery,
			PeerID sender) {
		...
		if (!cRecoveryPeers.containsKey(failedPeer)) {
			...
			// later
			saveToOtherRecoveryPeersList(failedPeer, sharedQuery);
			return false;
		} else {
			// See, if we want to do the recovery for this query
			List<ID> queries = cRecoveryPeers.get(failedPeer);
			if (!queries.contains(sharedQuery)) {
		...
		if (!thisPeerHasHigherNumber(sender)) {
			...
			removeBackupQueueEntry(failedPeer, sharedQuery);
	...
	private static void saveToOtherRecoveryPeersList(PeerID failedPeer,
		...
		if (!cOtherRecoveryPeers.containsKey(failedPeer)) {
			sharedQueryIds.add(sharedQuery);
		} else {
			sharedQueryIds = cOtherRecoveryPeers.get(failedPeer);
			sharedQueryIds.add(sharedQuery);
		}
		cOtherRecoveryPeers.put(failedPeer, sharedQueryIds);
	}
	...
	private static boolean otherPeerDoesRecovery(PeerID failedPeer, ID sharedQueryId) {
		...
		if (cOtherRecoveryPeers.containsKey(failedPeer)) {
			List<ID> sharedQueryIds = cOtherRecoveryPeers.get(failedPeer);
