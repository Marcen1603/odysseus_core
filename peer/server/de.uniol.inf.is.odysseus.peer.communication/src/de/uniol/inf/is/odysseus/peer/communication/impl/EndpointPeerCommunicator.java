...
public class EndpointPeerCommunicator implements IPeerCommunicator, EndpointListener {
	...
	public void activate() {
		Thread waitingThread = new Thread(new Runnable() {
			...
			@Override
			public void run() {
				while (jxtaServicesProvider == null || !jxtaServicesProvider.isActive()) {
					waitSomeTime();
				}
				jxtaServicesProvider.getEndpointService().addIncomingMessageListener(EndpointPeerCommunicator.this, COMMUNICATION_SERVICE_ID, null);
			}
	...
	public void send(PeerID destinationPeer, IMessage message) throws PeerCommunicationException {
		...
		if (messenger == null) {
			...
			messenger = jxtaServicesProvider.getEndpointService().getMessenger(addr);
			if (messenger == null) {
	...
	public void processIncomingMessage(Message message, EndpointAddress srcAddr, EndpointAddress dstAddr) {
		try {
			...
			if (data != null && data.length >= 4) {
				...
				LOG.warn("Got message with too few bytes from pid {}", pid);
			}
		} catch (Throwable t) {
