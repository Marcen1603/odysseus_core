...
public class InstructionHandler {
	...
	public static void handleInstruction(
			InactiveQueryInstructionMessage instruction, PeerID senderPeer) {
		...
		case InactiveQueryInstructionMessage.INITIATE_LOADBALANCING:
			...
			if (status == null) {
				status = new InactiveQuerySlaveStatus(
						InactiveQuerySlaveStatus.INVOLVEMENT_TYPES.VOLUNTEERING_PEER,
						InactiveQuerySlaveStatus.LB_PHASES.WAITING_FOR_ADD,
						senderPeer, lbProcessId,
						new InactiveQueryMessageDispatcher(peerCommunicator,
								lbProcessId));
		...
		case InactiveQueryInstructionMessage.REPLACE_SENDER:
			...
			if (status == null) {
				status = new InactiveQuerySlaveStatus(
						InactiveQuerySlaveStatus.INVOLVEMENT_TYPES.PEER_WITH_SENDER_OR_RECEIVER,
						InactiveQuerySlaveStatus.LB_PHASES.WAITING_FOR_FINISH,
						senderPeer, lbProcessId,
						new InactiveQueryMessageDispatcher(peerCommunicator,
								lbProcessId));
			...
			if (status.getPhase()
					.equals(InactiveQuerySlaveStatus.LB_PHASES.WAITING_FOR_FINISH)
