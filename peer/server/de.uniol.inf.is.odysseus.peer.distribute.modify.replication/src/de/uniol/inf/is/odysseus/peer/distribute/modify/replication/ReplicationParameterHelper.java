...
public class ReplicationParameterHelper implements IModificationParameterHelper {
	...
	public ReplicationParameterHelper(List<String> replicationParameters) {
		...
		Preconditions.checkNotNull(replicationParameters,
				"Replication parameters must be not null!");

		this.mReplicationParameters = ImmutableList
				.copyOf(replicationParameters);
	...
	public int determineDegreeOfModification()
		...
		// Preconditions
		if (this.mReplicationParameters.size() < ReplicationParameterHelper.PARAMETER_MIN_INDEX_DEGREE_OF_REPLICATION + 1) {

			throw new QueryPartModificationException(
					"Replication parameters must at least contain one value!");
		...
		return ModificationHelper.determineDegreeOfModification(
				degreeParameter,
	...
	public IPair<Optional<ILogicalOperator>, Optional<ILogicalOperator>> determineStartAndEndPoints(
		...
		// Preconditions
		if (this.mReplicationParameters.size() < ReplicationParameterHelper.PARAMETER_MAX_INDEX_DEGREE_OF_REPLICATION + 1) {
