package de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.inactivequery.protocol;
...
import de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.inactivequery.OsgiServiceManager;
import de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.inactivequery.communicator.InactiveQueryCommunicatorImpl;
import de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.inactivequery.communicator.InactiveQueryHelper;
import de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.inactivequery.communicator.InactiveQueryMessageDispatcher;
import de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.inactivequery.messages.InactiveQueryResponseMessage;
import de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.inactivequery.status.InactiveQueryMasterStatus;
import de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.inactivequery.status.InactiveQueryMasterStatus.LB_PHASES;
...
public class ResponseHandler {
	 ...
	 * @param senderPeer
	 */
	public static void handlePeerResonse(InactiveQueryResponseMessage response, PeerID senderPeer) {
		...
		InactiveQueryMasterStatus status = (InactiveQueryMasterStatus) LoadBalancingStatusCache.getInstance().getStatusForLocalProcess(loadBalancingProcessId);

		InactiveQueryCommunicatorImpl communicationListener = InactiveQueryCommunicatorImpl.getInstance();
		...
		case InactiveQueryResponseMessage.ACK_LOADBALANCING:
			...
			if (status.getPhase().equals(InactiveQueryMasterStatus.LB_PHASES.INITIATING)) {
				...
				if (isMaster && remainingQueries.size() <= 0) {
					...
					// Deregister As Master, update other Peers List and send to
					// new Master.
		...
		case InactiveQueryResponseMessage.SUCCESS_INSTALL_QUERY:
			...
			if (status.getPhase().equals(InactiveQueryMasterStatus.LB_PHASES.COPYING_QUERY) && !status.isLocked()) {
				status.lock();
		...
		case InactiveQueryResponseMessage.SUCCESS_DUPLICATE_SENDER:
			...
			if (status.getPhase().equals(InactiveQueryMasterStatus.LB_PHASES.RELINKING_SENDERS) && !status.isLocked()) {
	 ...
	 */
	public static void handleError(InactiveQueryMasterStatus status, InactiveQueryCommunicatorImpl communicationListener) {
		InactiveQueryMessageDispatcher dispatcher = status.getMessageDispatcher();
	 ...
	 */
	public static void loadBalancingSuccessfullyFinished(InactiveQueryMasterStatus status) {
