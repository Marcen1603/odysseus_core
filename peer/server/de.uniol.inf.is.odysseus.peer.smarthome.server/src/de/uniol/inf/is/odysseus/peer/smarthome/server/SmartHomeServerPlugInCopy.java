...
public class SmartHomeServerPlugInCopy implements BundleActivator {

	private static final Logger LOG = LoggerFactory
			.getLogger(SmartHomeServerPlugInCopy.class);
	...
	private static void printLocalPeerID() {
		try {
			LOG.debug("SmartDevice PeerID: "
					+ SmartHomeServerPlugInCopy.p2pNetworkManager.getLocalPeerID()
							.intern().toString());
		} catch (Exception ex) {
	...
	public static boolean isLocalPeer(PeerID peerID) {
		if (SmartHomeServerPlugInCopy.getP2PNetworkManager() == null
	...
	public static boolean isLocalPeer(String peerIDString) {
		String str1 = SmartHomeServerPlugInCopy.p2pNetworkManager.getLocalPeerID()
				.intern().toString();
		...
		public void receivedMessage(IPeerCommunicator communicator,
			...
			if (message instanceof SmartDeviceConfigurationRequestMessage) {
				try {
					...
					try {
						SmartHomeServerPlugInCopy.getPeerCommunicator().send(
	...
	public static class P2PDictionaryListener implements IP2PDictionaryListener {
		...
		public void sourceImportRemoved(IP2PDictionary sender,
			...
			LOG.debug("you must remove logicRule who depends on sourceName:"
					+ sourceName + "!!!");
			LOG.debug("-----");
			removeSourceNeededForImport(sourceName);
			stopQuery(sourceName);
	...
	public static void removeAllLogicRules(SmartDevice remoteSmartDevice) {
		for (Sensor sensor : remoteSmartDevice.getConnectedSensors()) {
			...
			ListIterator<String> iterator = new ArrayList<String>(sensor
					...
					.listIterator(sensor
							.getViewForActivityInterpreterQueries().size());
		...
		LogicExecutionObserver.getInstance()
				.removeLogicRules(remoteSmartDevice);
	}
	...
	private static void stopQuery(String queryName) {
		...
		ServerExecutorService.getServerExecutor().removeQuery(queryName+"_query",
				SessionManagementService.getActiveSession());
		...
		if (isLogicExecutionPossible(possibleActivityName, sensor, actor,
			...
			try {
				...
				for (Entry<String, String> entry : logicRuleQueries.entrySet()) {
					...
					LogicExecutionObserver.getInstance().addRunningLogicRule(
	...
	private static boolean logicRuleIsCurrentlyRunning(
		...
		return LogicExecutionObserver.getInstance().isLogicRuleRunning(
				possibleActivityName, sensor, actor, smartDeviceWithSensor);
	}

	public static class LogicExecutionObserver {
		private static LogicExecutionObserver instance;

		public static LogicExecutionObserver getInstance() {
			if (instance == null) {
				instance = new LogicExecutionObserver();
			}
			return instance;
		}
		...
		public void removeLogicRules(SmartDevice smDevice) {
			...
			for(int i=0;i<getRuleContainer().size();i++){
				...
				if(devCon.getSmartDevice().getPeerID().equals(smDevice.getPeerID())){
