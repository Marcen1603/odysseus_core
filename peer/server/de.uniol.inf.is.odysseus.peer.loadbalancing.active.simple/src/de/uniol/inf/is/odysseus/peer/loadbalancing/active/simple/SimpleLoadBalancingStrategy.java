...
/**
 * The {@link SimpleLoadBalancingStrategy} defines a threshold for the load (CPU
 * and storage). If the current load is above this threshold, it takes the first
 * query it gets from {@link IExecutor} and allocates it using the allocator set
 * by {@link #setAllocator(ILoadBalancingAllocator)}.
 * 
...
public class SimpleLoadBalancingStrategy implements ILoadBalancingStrategy, ILoadBalancingListener {
	 ...
	 * It will call
	 * {@link ILoadBalancingCommunicator#initiateLoadBalancing(PeerID, int)} if
	...
	private class SimpleLBThread extends Thread implements IPeerLockContainerListener {
		...
		/**
		 * True, if
		 * {@link ILoadBalancingCommunicator#initiateLoadBalancing(PeerID, int)}
		 * has been called.
		 */
		 ...
		 * 
		 * @param currentUsage
		 *            The current {@link IResourceUsage}.
		 * @param lastUsage
		 *            The last known {@link IResourceUsage}.
		...
		private boolean hasUsageChanged(IResourceUsage currentUsage, IResourceUsage lastUsage) {
			...
			return currentUsage.getCpuMax() != lastUsage.getCpuMax() || currentUsage.getCpuFree() != lastUsage.getCpuFree() || currentUsage.getMemMaxBytes() != lastUsage.getMemMaxBytes() || currentUsage.getMemFreeBytes() != lastUsage.getMemFreeBytes();
		 ...
		 * @param usage
		 *            The current {@link IResourceUsage}.
		 * @return True, if there is more CPU in use as determined by
		 *         {@link #mCPUThreshold} or if there is more memory in use as
		 *         determined by {@link #mMemThreshold}.
		 ...
		 * 
		 * @return A pair of {@link ILogicalQuery} and the corresponding query
		 *         ID.
		 * @throws InterruptedException
		 *             if any error occurs.
		 */
		 ...
		 * 
		 * @param query
		 *            The {@link ILogicalQuery} to be allocated.
		 * @return The ID of the peer, to which <code>query</code> has been
		 *         allocated to.
		 * @throws InterruptedException
		 *             if any error occurs.
		 */
		...
		/**
		 * Updates the resource usage and checks, if the threshold has been
		 * reached. <br />
		 * Initializes load balancing, if that happens.
		 * 
		 * @param usage
		 *            The new {@link IResourceUsage}.
		 * @throws InterruptedException
		 *             if any error occurs
		 */
			...
			if (this.loadBalancingNeeded(usage)) {
				...
				if (lock.requestLocalLock()) {
					try {
						...
						// Additional boolean to prevent multiple parallel
		 ...
		 * 
		 * @param allocator
		 *            An implementation of {@link ILoadBalancingAllocator}.
		 * @param communicator
		 *            An implementation of {@link ILoadBalancingCommunicator}.
		 */
	...
	private static boolean hasRealSinks(int queryID) {
		...
		Collection<ILogicalOperator> relativeSinks = LogicalQueryHelper.getRelativeSinksOfLogicalQueryPart(copy);
	...
	private static ILogicalQueryPart getCopyOfQueryPart(ILogicalQueryPart part) {
		...
		Map<ILogicalQueryPart, ILogicalQueryPart> copies = LogicalQueryHelper.copyQueryPartsDeep(partsList);
