...
import de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.paralleltrack.messages.ParallelTrackResponseMessage;
import de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.paralleltrack.status.ParallelTrackMasterStatus;
import de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.paralleltrack.status.ParallelTrackMasterStatus.LB_PHASES;
...
public class ResponseHandler {
	 ...
	 * @param senderPeer
	 */
	public static void handlePeerResonse(ParallelTrackResponseMessage response, PeerID senderPeer) {
		...
		ParallelTrackMasterStatus status = (ParallelTrackMasterStatus) LoadBalancingStatusCache.getInstance().getStatusForLocalProcess(loadBalancingProcessId);
		...
		case ParallelTrackResponseMessage.ACK_LOADBALANCING:
			...
			if (status.getPhase().equals(ParallelTrackMasterStatus.LB_PHASES.INITIATING)) {
				...
				ParallelTrackHelper.relinkQueryPart(modifiedQueryPart, status);
				...
				//If Shared Query ID is null  this is no shared Query.
				else {
					...
					dispatcher.sendAddQuery(status.getVolunteeringPeer(), pqlFromQueryPart, communicationListener, null, null,queryName,transCfgName,metaAttributes);
				}
			}
			break;
		...
		case ParallelTrackResponseMessage.SUCCESS_INSTALL_QUERY:
			...
			if (status.getPhase().equals(ParallelTrackMasterStatus.LB_PHASES.COPYING)) {
		...
		case ParallelTrackResponseMessage.SUCCESS_DUPLICATE:
			...
			if (status.getPhase().equals(ParallelTrackMasterStatus.LB_PHASES.RELINKING_SENDERS)) {
			...
			if (status.getPhase().equals(ParallelTrackMasterStatus.LB_PHASES.RELINKING_RECEIVERS)) {
					...
					if (!queryPart.getOperators().asList().isEmpty()) {
						...
						for (ILogicalOperator operator : queryPart.getOperators()) {
							if (operator instanceof JxtaReceiverAO) {
								...
								dispatcher.sendDeleteOperator(true, receiver.getPeerID(), receiver.getPipeID(), communicationListener);
							}
							if (operator instanceof JxtaSenderAO) {
								JxtaSenderAO sender = (JxtaSenderAO) operator;
								dispatcher.sendDeleteOperator(false, sender.getPeerID(), sender.getPipeID(), communicationListener);
							}
						}
					}
