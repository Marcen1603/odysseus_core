...
import de.uniol.inf.is.odysseus.peer.loadbalancing.dynamic.communicator.paralleltrack.messages.ParallelTrackInstructionMessage;
...
public class InstructionHandler {
	 ...
	 */
	public static void handleInstruction(ParallelTrackInstructionMessage instruction, PeerID senderPeer) {
		...
		case ParallelTrackInstructionMessage.INITIATE_LOADBALANCING:
			...
			if (status == null) {
				status = new ParallelTrackSlaveStatus(ParallelTrackSlaveStatus.INVOLVEMENT_TYPES.VOLUNTEERING_PEER, ParallelTrackSlaveStatus.LB_PHASES.WAITING_FOR_ADD, senderPeer, lbProcessId, new ParallelTrackMessageDispatcher(peerCommunicator, lbProcessId));
		...
		case ParallelTrackInstructionMessage.ADD_QUERY:
			...
			if (status.getPhase().equals(ParallelTrackSlaveStatus.LB_PHASES.WAITING_FOR_ADD)) {
				...
				try {
					Collection<Integer> queryIDs = LoadBalancingHelper.installAndRunQueryPartFromPql(Context.empty(), instruction.getPQLQuery(),instruction.getQueryName(),instruction.getTransCfgName(),instruction.getMetaDataTypeNames());
		...
		case ParallelTrackInstructionMessage.COPY_RECEIVER:
			...
			if (status == null) {
				status = new ParallelTrackSlaveStatus(ParallelTrackSlaveStatus.INVOLVEMENT_TYPES.PEER_WITH_SENDER_OR_RECEIVER, ParallelTrackSlaveStatus.LB_PHASES.WAITING_FOR_SYNC, senderPeer, lbProcessId, new ParallelTrackMessageDispatcher(peerCommunicator, lbProcessId));
			...
			if (status.getPhase().equals(ParallelTrackSlaveStatus.LB_PHASES.WAITING_FOR_SYNC) && !status.isReceiverPipeKnown(instruction.getNewPipeId())) {

				LOG.debug("Installing pipe " + instruction.getNewPipeId());

				status.addReplacedReceiverPipe(instruction.getNewPipeId(), instruction.getOldPipeId());
				...
				dispatcher = status.getMessageDispatcher();
				try {
					ParallelTrackHelper.findAndCopyLocalJxtaOperator(status, isSender, instruction.getNewPeerId(), instruction.getOldPipeId(), instruction.getNewPipeId());
		...
		case ParallelTrackInstructionMessage.COPY_SENDER:
			...
			if (status.getPhase().equals(ParallelTrackSlaveStatus.LB_PHASES.WAITING_FOR_SYNC) && !status.isSenderPipeKnown(instruction.getNewPipeId())) {

				LOG.debug("Installing pipe " + instruction.getNewPipeId());

				status.addReplacedSenderPipe(instruction.getNewPipeId(), instruction.getOldPipeId());
				if (status.getVolunteeringPeer() == null) {
					status.setVolunteeringPeer(LoadBalancingHelper.toPeerID((instruction.getNewPeerId())));
				}
		...
		case ParallelTrackInstructionMessage.DELETE_RECEIVER:
			...
			// Sync is finished (or we would not be here). So we can stop
			// spamming SYNC_FINISHED Messages now.
