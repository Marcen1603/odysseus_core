package de.uniol.inf.is.odysseus.rcp.resource;
...
public class ResourceManager {
	...
	public void load( Device device, IResourceConfiguration configuration ) {
		...
		for( String key : resources.keySet()) {
			...
			URL fileToLoad = resources.get( key );
	...
	// schief gegangen ist.
	private static Image loadImage( Device device, URL imageURL ) {
		try {
...
//			URL imageURL = resourceBundle.getEntry(filename);
			...
			logger.debug( "Imageresource loaded:" + imageURL );
			
			return image;
		} 
		catch( Exception ex ) {
			logger.error( "Error during loading image: " + imageURL, ex );
			return null;
		}
	}}
