...
import de.uniol.inf.is.odysseus.compiler.internal.BasicResourceResolver;

public class BundleJavaManager extends ForwardingJavaFileManager<JavaFileManager> implements Constants {

	private CompiledCode compiledCode;
	private Map<BundleWiring, ClassLoader> classLoader = new HashMap<>();

	static Logger log = LoggerFactory.getLogger(BundleJavaManager.class);

	public BundleJavaManager(Bundle bundle, JavaFileManager javaFileManager, List<String> options, CompiledCode compiledCode)
			throws IOException {
		...
		log.debug("Initializing compilation in OSGi for bundle " + _bundle.getSymbolicName() + "-"
				+ _bundle.getVersion());
		...
		classLoader.put(_bundleWiring, _classLoader);
		...
		log.debug("Dependent BundleWirings included in this BundleJavaManager " + "context: ");
		...
		for (BundleWire bundleWire : providedWires) {
			...
			log.debug("\t" + curBundle.getSymbolicName() + "-" + curBundle.getVersion());

			_bundleWirings.add(providerWiring);
		}

		_bundleWirings.addAll(getWirings(bundle));

		for (BundleWiring w:_bundleWirings){
			classLoader.put(w, w.getClassLoader());
		}
	...
	public static List<BundleWiring> getWirings(Bundle startBundle) {
		List<BundleWiring> ret = new ArrayList<>();
	...
	public String inferBinaryName(Location location, JavaFileObject file) {
		if ((location == StandardLocation.CLASS_PATH) && (file instanceof BundleJavaFileObject)) {
			...
			log.debug("Infering binary name from " + bundleJavaFileObject);
	...
	public Iterable<JavaFileObject> list(Location location, String packageName, Set<Kind> kinds, boolean recurse)
		...
		if (location == StandardLocation.CLASS_PATH) {
			log.debug("List available sources for {location=" + location + ", packageName=" + packageName + ", kinds="
		...
		if (packageName.startsWith(JAVA_PACKAGE) || (location != StandardLocation.CLASS_PATH)
			...
			Iterable<JavaFileObject> localJavaFileObjects = _javaFileManager.list(location, packagePath, kinds,
					recurse);

			for (JavaFileObject javaFileObject : localJavaFileObjects) {
				if (location == StandardLocation.CLASS_PATH) {
					log.debug("\t" + javaFileObject);
				}
	...
	private void list(String packagePath, Kind kind, int options, BundleWiring bundleWiring,
			List<JavaFileObject> javaFileObjects) {

		ResourceResolver resourceResolver = getResourceResolver();

		Collection<String> resources = resourceResolver.resolveResources(bundleWiring, packagePath,
				STAR.concat(kind.extension), options);
		...
		for (String resourceName : resources) {
			...
			if (javaFileObject == null) {
				log.debug("\tCould not create JavaFileObject for {" + resourceURL + "}");
	...
	private void listFromDependencies(Set<Kind> kinds, boolean recurse, String packagePath,
			List<JavaFileObject> javaFileObjects) {
