...
public class SetOwnerGraphVisitor<T extends IOwnedOperator> implements
	...
	private boolean onlyOpsWithoutOwner = false;
	...
	/**
	 * Adds the owner to every operator in the plan.
	 * 
	 * @param owner
	 *            added owner
	 * @param onlyOpsWithoutOwner
	 *            if true only operators without owner get a new owner.
	 */
	public SetOwnerGraphVisitor(IOperatorOwner owner,
			boolean onlyOpsWithoutOwner) {
		this.visited = new IdentityArrayList<T>();
		this.owner = owner;
		this.onlyOpsWithoutOwner = onlyOpsWithoutOwner;
	}
	...
	public void nodeAction(T op) {
		if (!this.visited.contains(op)) {
			if (owner != null) {
				if ((this.onlyOpsWithoutOwner && !op.hasOwner())
						|| !this.onlyOpsWithoutOwner) {
					op.addOwner(this.owner);
				}
			} else {
				op.removeAllOwners();
			}
