...
public class NioTcpConnection implements IConnection, ReadWriteSelectorHandler {
    ...
    public NioTcpConnection(final SocketChannel socketChannel, final SelectorThread selector, final IAccessConnectionListener<ByteBuffer> listener) throws IOException {
    ...
    private void requestWrite() throws IOException {
        this.selector.addChannelInterest(this.channel, SelectionKey.OP_WRITE, new CallbackErrorHandler() {
    ...
    public void close() {
        try {
            this.selector.removeChannelInterest(this.channel, SelectionKey.OP_READ, new CallbackErrorHandler() {
                @SuppressWarnings("unused")
                public void handleError(final Exception ex) {
                    NioTcpConnection.this.listener.socketException(ex);
                }
            });
            this.selector.removeChannelInterest(this.channel, SelectionKey.OP_WRITE, new CallbackErrorHandler() {
                @SuppressWarnings("unused")
                public void handleError(final Exception ex) {
                    NioTcpConnection.this.listener.socketException(ex);
                }
