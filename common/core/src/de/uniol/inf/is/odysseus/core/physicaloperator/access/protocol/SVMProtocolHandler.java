 ...
 * 
 */
public class SVMProtocolHandler<T extends Tuple<?>> extends LineProtocolHandler<T> {

    private final Logger LOG = LoggerFactory.getLogger(SVMProtocolHandler.class);
    ...
    public SVMProtocolHandler(ITransportDirection direction, IAccessPattern access, IDataHandler<T> dataHandler) {
    ...
    protected void init(Map<String, String> options) {
        ...
        if (options.containsKey(DELIMITER)) {
            ...
            delimiter = options.containsKey(SVM_DELIMITER) ? options.get(SVM_DELIMITER).toCharArray()[0] : " ".toCharArray()[0];
        }
        // only calc once
        delimiterString = Character.toString(delimiter);
        if (options.containsKey(SVM_FLOATING_FORMATTER)) {
            floatingFormatter = new DecimalFormat(options.get(SVM_FLOATING_FORMATTER));
        }
        if (options.containsKey(SVM_NUMBER_FORMATTER)) {
            numberFormatter = new DecimalFormat(options.get(SVM_NUMBER_FORMATTER));
        }
        if (options.containsKey(SVM_WRITE_METADATA)) {
            writeMetadata = Boolean.parseBoolean(options.get(SVM_WRITE_METADATA));
        }
        ...
        if (options.containsKey(TEXT_DELIMITER)) {
            ...
            textDelimiter = options.containsKey(SVM_TEXT_DELIMITER) ? options.get(SVM_TEXT_DELIMITER).toCharArray()[0] : "'".toCharArray()[0];
        }
    }
        ...
        if (object.getAttributes().length > 0) {
            for (int i = 0; i < object.getAttributes().length; ++i) {
                ...
                if (curAttribute instanceof Number) {
                    if ((((Number) curAttribute).doubleValue() != 0.0) && (i > 0)) {
                        if (i > 0) {
                        ...
                        if ((curAttribute instanceof Double || curAttribute instanceof Float) && floatingFormatter != null) {
                            retBuff.append(floatingFormatter.format(curAttribute));
                        }
                    ...
                    if (textDelimiter != 0 && curAttribute instanceof String) {
                        retBuff.append(textDelimiter).append(curAttribute.toString()).append(textDelimiter);
                    }
                    else {
    ...
    public IProtocolHandler<T> createInstance(ITransportDirection direction, IAccessPattern access, Map<String, String> options, IDataHandler<T> dataHandler) {
        SVMProtocolHandler<T> instance = new SVMProtocolHandler<T>(direction, access, dataHandler);
