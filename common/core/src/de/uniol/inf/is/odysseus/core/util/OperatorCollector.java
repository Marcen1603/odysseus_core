...
public class OperatorCollector {
	...
	private static void collectRecursively(IPhysicalOperator op, List<IStatefulPO> ops) {
		...
		if (op instanceof ISubscriber) {
			...
			ISubscriber<? extends IPhysicalOperator, ISubscription<? extends IPhysicalOperator>> sink = (ISubscriber<? extends IPhysicalOperator, ISubscription<? extends IPhysicalOperator>>) op;
	 ...
	 */
	public static List<ControllablePhysicalSubscription<? extends IPhysicalOperator>> collectSubcriptions(
			List<IPhysicalOperator> roots) {
		List<ControllablePhysicalSubscription<? extends IPhysicalOperator>> subs = new ArrayList<>();
		for (IPhysicalOperator root : roots) {
	...
	private static void collectSubscriptionsRecursively(IPhysicalOperator op,
			List<ControllablePhysicalSubscription<? extends IPhysicalOperator>> subs) {
		if (op instanceof ISubscriber) {
			@SuppressWarnings("unchecked")
			ISubscriber<? extends IPhysicalOperator, ISubscription<? extends IPhysicalOperator>> sink = (ISubscriber<? extends IPhysicalOperator, ISubscription<? extends IPhysicalOperator>>) op;
			for (ISubscription<? extends IPhysicalOperator> sub : sink.getSubscribedToSource()) {
				if (sub instanceof ControllablePhysicalSubscription) {
					subs.add((ControllablePhysicalSubscription<? extends IPhysicalOperator>) sub);
				}
				collectSubscriptionsRecursively(sub.getTarget(), subs);
			}
		}
	}
