...
public class ClassFieldAccessorCache {
	...
	// for ( AcceptsReadAccessor target :
	// entry.getValue().getReadAccessorTargets() ) {
	...
	public ClassObjectType getClassObjectType(ClassObjectType objectType) {
		// always lookup the class, as the ClassObjectType might refer to the
	...
	public BaseClassFieldReader getReadAcessor(ClassFieldReader reader) {
		...
		return cache.getReadAccessor(new AccessorKey(className, fieldName,
				AccessorKey.AccessorType.FieldAccessor), cls);
	}
	...
	public BaseClassFieldWriter getWriteAcessor(ClassFieldWriter writer) {
		...
		// get the ReaderAccessor for this key
		return cache.getWriteAccessor(new AccessorKey(className, fieldName,
				AccessorKey.AccessorType.FieldAccessor), cls);
	}
	...
	public CacheEntry getCacheEntry(Class cls) {
		...
		ClassLoader cl = this.classLoader != null ? this.classLoader : (cls
				.getClassLoader() != null) ? cls.getClassLoader() : ClassLoader
	...
	public static class CacheEntry {
		...
		public CacheEntry(ClassLoader parentClassLoader) {
			if (parentClassLoader == null) {
				throw new RuntimeDroolsException(
						"ClassFieldAccessorFactory cannot have a null parent ClassLoader");
			}
			this.byteArrayClassLoader = new ByteArrayClassLoader(
					parentClassLoader);
		}
		...
		public BaseClassFieldReader getReadAccessor(AccessorKey key, Class cls) {
			...
			if (reader == null) {
				reader = ClassFieldAccessorFactory.getInstance()
						.getClassFieldReader(cls, key.getFieldName(), this);
		...
		public BaseClassFieldWriter getWriteAccessor(AccessorKey key, Class cls) {
			...
			if (reader == null) {
				reader = ClassFieldAccessorFactory.getInstance()
						.getClassFieldWriter(cls, key.getFieldName(), this);
				this.writeCache.put(key, reader);
			}
		...
		public ClassObjectType getClassObjectType(Class cls,
			...
			ClassObjectTypeKey key = new ClassObjectTypeKey(cls, objectType
					.isEvent());
			...
			if (existing != null) {
				...
				objectType.setClassType(cls); // most likely set, but set
												// anyway.
