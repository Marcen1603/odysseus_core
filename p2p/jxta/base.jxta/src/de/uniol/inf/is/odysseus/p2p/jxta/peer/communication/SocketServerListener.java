...
public class SocketServerListener implements ISocketServerListener {
    ...
    public void run() {
        ...
        try {
            ...
            serverSocket = new JxtaServerSocket(PeerGroupTool.getPeerGroup(), getServerPipeAdvertisement(), 10);
                        ...
                        try {
                            ...
                            while (it.hasNext()) {
                                ...
                                if (namespace != null && namespace.trim().length() > 0) {
                                    ...
                                    IMessageHandler h = messageHandlers.get(namespace);
                                    if (h != null) {
                                        logger.debug("Handle meassage in namespace " + namespace + " with " + h);
                                        h.handleMessage(msg, namespace);
                                    } else {
                                        logger.warn("No handler found for " + namespace);
                                    }
                                }
                            }
    ...
    @Override
    public synchronized void registerMessageHandler(List<IMessageHandler> messageHandler) {
    ...
    @Override
    public synchronized void registerMessageHandler(IMessageHandler messageHandler) {
        messageHandlers.put(messageHandler.getInterestedNamespace(), messageHandler);
    }
