...
public class MessageTool {
    ...
    public static void sendMessage(PeerGroup netPeerGroup, PipeAdvertisement adv, Message message, int maxRetries) {
        new MessageSender(netPeerGroup, adv, message, maxRetries).start();
    }

    @SuppressWarnings("rawtypes")
    public static PipeAdvertisement createResponsePipeFromMessage(String namespace, Message msg, int number) {
        MessageElement advElement = msg.getMessageElement(namespace, "pipeAdv" + number);
        XMLDocument theDocument = null;
        try {
            theDocument = (XMLDocument) StructuredDocumentFactory.newStructuredDocument(advElement.getMimeType(), advElement.getStream());
        } catch (IOException e1) {
            e1.printStackTrace();
        }
        PipeAdvertisement pipeAdv = (PipeAdvertisement) AdvertisementFactory.newAdvertisement(theDocument);

        return pipeAdv;
    }

    @SuppressWarnings("rawtypes")
    public static PeerAdvertisement createPeerAdvertisementFromMessage(String namespace, Message msg) {
        ...
        PeerAdvertisement peerAdv = (PeerAdvertisement) AdvertisementFactory.newAdvertisement(theDocument);
    ...
    public static PipeAdvertisement createPipeAdvertisementFromXml(String xmlAdv) {
        ...
        try {
            xml = (XMLDocument) StructuredDocumentFactory.newStructuredDocument(MimeMediaType.XMLUTF8, sr);
        } catch (IOException e1) {
            e1.printStackTrace();
        }
    ...
    public static Message createOdysseusMessage(OdysseusMessageType type, Map<String, Object> messageElems) {
        String namespace = type.name();
        return createSimpleMessage(namespace, messageElems);
    }

    @SuppressWarnings("rawtypes")
    public static Message createSimpleMessage(String namespace, Map<String, Object> messageElems) {
        ...
        for (Entry entry : messageElems.entrySet()) {
            ...
            if (value instanceof String) {
                response.addMessageElement(namespace, new StringMessageElement((String) elem, (String) value, null));
            } else if (value instanceof PeerAdvertisement) {
                TextDocumentMessageElement peerAdv = new TextDocumentMessageElement("peerAdv", (XMLDocument) ((PeerAdvertisement) value).getDocument(MimeMediaType.XMLUTF8), null);
                response.addMessageElement(namespace, peerAdv);
            }

            else if (value instanceof PipeAdvertisement) {
                TextDocumentMessageElement responsePipeAdv = new TextDocumentMessageElement("pipeAdv" + pipeCounter++, (XMLDocument) ((PipeAdvertisement) value).getDocument(MimeMediaType.XMLUTF8),
                        null);
...
class MessageSender extends Thread {
    ...
    public MessageSender(PeerGroup netPeerGroup, PipeAdvertisement pipeAdv, Message message, int maxRetries) {
