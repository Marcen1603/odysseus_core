...
/**
 * abstract rule for the AccessAO (PULL)
 * 
 * @author MarcPreuschaft
 *
 * @param <T>
 */
public abstract class AbstractCAccessGenericAORule <T extends AccessAO> extends AbstractCOperatorRule<AccessAO>{
	
	public AbstractCAccessGenericAORule(String name) {
		super(name);
	}
	...
	public boolean isExecutable(AccessAO logicalOperator,
			...
			if (Constants.GENERIC_PULL.equalsIgnoreCase(logicalOperator.getWrapper())) {
				return true;
			} else {
				return false;
			}
