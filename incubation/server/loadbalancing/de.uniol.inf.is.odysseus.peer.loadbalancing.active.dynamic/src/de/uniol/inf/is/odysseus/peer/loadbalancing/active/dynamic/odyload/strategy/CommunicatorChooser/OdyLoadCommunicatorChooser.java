...
public HashMap<Integer,ILoadBalancingCommunicator> chooseCommunicators(List<Integer> queryIds,IServerExecutor executor, ILoadBalancingCommunicatorRegistry communicatorRegistry, ISession session) {
		...
		for (int queryId : queryIds) {
			...
			if(!hasStatefulOperator(queryId,executor,session)) {
	...
	private static boolean isActive(int queryId,IServerExecutor executor) {
	...
	private static boolean hasStatefulOperator(int queryId,IServerExecutor executor, ISession session) {
		HashMap<Integer,IPhysicalQuery> queries = getPhysicalQueries(executor, session);
	...
	private static HashMap<Integer, IPhysicalQuery> getPhysicalQueries(IServerExecutor executor, ISession session) {
