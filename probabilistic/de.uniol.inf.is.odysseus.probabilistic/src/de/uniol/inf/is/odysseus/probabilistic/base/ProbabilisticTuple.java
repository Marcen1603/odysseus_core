...
public class ProbabilisticTuple<T extends IMetaAttribute> extends Tuple<T> {
     ...
     * @param b
     */
    public ProbabilisticTuple(final ProbabilisticTuple<T> copy, final Object[] newAttrs, final NormalDistributionMixture[] newDistrs, final boolean b) {
     ...
     * @param distribution
     */
    public final void setDistribution(final int pos, final NormalDistributionMixture distribution) {
     ...
     */
    @Override
    public ProbabilisticTuple<T> restrict(final int attr, final boolean createNew) {
     ...
     */
    @Override
    public ProbabilisticTuple<T> restrict(final int[] attrList, final boolean createNew) {
     ...
     */
    public ProbabilisticTuple<T> restrict(final int attr, final RealMatrix restrictMatrix, final boolean createNew) {
        return this.restrict(new int[] { attr }, new RealMatrix[] { restrictMatrix }, createNew);
    }
     ...
     * @return The restricted tuple
     */
    public ProbabilisticTuple<T> restrict(final int[] attrList, final RealMatrix[] restrictMatrix, final boolean createNew) {
        final int[] newDistrDimensions = new int[this.distributions.length];
     ...
     * @return
     */
    private IStreamObject<T> processMergeInternal(ProbabilisticTuple<T> left, ProbabilisticTuple<T> right) {

        NormalDistributionMixture[] newDistributions = mergeDistributions(left != null ? left.getDistributions() : null, right != null ? right.getDistributions() : null, left.getAttributes().length);

        Object[] newAttributes = mergeAttributes(left != null ? left.getAttributes() : null, right != null ? right.getAttributes() : null, left.getDistributions().length);

        ProbabilisticTuple<T> r = new ProbabilisticTuple<T>(newAttributes, newDistributions, left.requiresDeepClone() || right.requiresDeepClone());
        return r;
    }
     ...
     * @return
     */
    private Object[] mergeAttributes(Object[] leftAttributes, Object[] rightAttributes, int offset) {
     ...
     * @return
     */
    private NormalDistributionMixture[] mergeDistributions(NormalDistributionMixture[] leftDistributions, NormalDistributionMixture[] rightDistributions, int offset) {
        NormalDistributionMixture[] newDistributions = new NormalDistributionMixture[leftDistributions.length + rightDistributions.length];
...
//            System.arraycopy(leftDistributions, 0, newDistributions, 0, leftDistributions.length);
        ...
        if (rightDistributions != null) {
             ...
             newDistributions[leftDistributions.length + i] =
             rightDistributions[i].clone();
             }
//            System.arraycopy(rightDistributions, 0, newDistributions, leftDistributions.length, rightDistributions.length);
    ...
    public static void main(final String[] args) {
        final NormalDistributionMixture mixture1 = new NormalDistributionMixture(new double[] { 1.0, 2.0 }, new CovarianceMatrix(new double[] { 1.0, 0.0, 1.0 }));
        mixture1.setAttributes(new int[] { 0, 4 });
        final NormalDistributionMixture mixture2 = new NormalDistributionMixture(new double[] { 3.0 }, new CovarianceMatrix(new double[] { 2.0 }));
        mixture2.setAttributes(new int[] { 2 });

        final ProbabilisticContinuousDouble probAttr11 = new ProbabilisticContinuousDouble(0);
        final ProbabilisticContinuousDouble probAttr12 = new ProbabilisticContinuousDouble(0);
        final ProbabilisticContinuousDouble probAttr21 = new ProbabilisticContinuousDouble(1);

        final Object[] attrs = new Object[] { probAttr21, 1.0, probAttr11, "String", probAttr12 };
        ...
        tuple.setDistributions(new NormalDistributionMixture[] { mixture1, mixture2 });
        System.out.println(tuple);
        for (NormalDistributionMixture dist : tuple.distributions) {
            System.out.println(dist);
        }
        final ProbabilisticTuple<IMetaAttribute> newTuple = tuple.restrict(new int[] { 0, 1, 2 }, true);
        System.out.println(tuple);
        for (NormalDistributionMixture dist : tuple.distributions) {
            System.out.println(dist);
        }
        ...
        ProbabilisticTuple<IMetaAttribute> mergeTuple = (ProbabilisticTuple<IMetaAttribute>) tuple.process_merge(tuple, newTuple, Order.LeftRight);
