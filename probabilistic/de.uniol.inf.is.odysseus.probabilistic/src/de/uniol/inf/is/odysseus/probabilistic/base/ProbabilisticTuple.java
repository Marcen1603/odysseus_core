...
public class ProbabilisticTuple<T extends IMetaAttribute> extends Tuple<T> {
	...
	public ProbabilisticTuple(ProbabilisticTuple<T> copy, Object[] newAttrs,
			boolean b) {
		super(copy);
	}
	...
	public ProbabilisticTuple(ProbabilisticTuple<T> copy) {
		...
		for (int i = 0; i < this.attributes.length; i++) {
			if ((this.attributes[i] != null)
					&& (this.attributes[i].getClass() == ProbabilisticContinuousDouble.class)) {
				ProbabilisticContinuousDouble attribute = (ProbabilisticContinuousDouble) this.attributes[i];
				int index = continuousAttributes.indexOf(attribute
						.getDistribution());
				if (index >= 0) {
					this.attributes[i] = new ProbabilisticContinuousDouble(
	...
	public ProbabilisticTuple<T> restrict(int attr, boolean createNew) {
		...
		return (ProbabilisticTuple<T>) super.restrict(attr, createNew);
	}

	@Override
	public ProbabilisticTuple<T> restrict(int[] attrList, boolean createNew) {
		// TODO Auto-generated method stub
		return (ProbabilisticTuple<T>) super.restrict(attrList, createNew);
	}

	@SuppressWarnings("unused")
	private ProbabilisticTuple<T> restrictCreation(boolean createNew,
			Object[] newAttrs) {
		if (createNew) {
			ProbabilisticTuple<T> newTuple = new ProbabilisticTuple<T>(this,
					newAttrs, false);
			return newTuple;
		}
		return this;
