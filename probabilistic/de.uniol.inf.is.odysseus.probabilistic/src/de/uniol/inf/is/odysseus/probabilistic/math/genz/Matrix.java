...
public class Matrix {
	...
	public void setSubRow(int row, int from, Matrix values) {
		...
		for(int i = 0 ; i < values.getRowDimension() ; i++){
			setElement(row, from + i, values.get(i + 1));
		}
	}
	...
	public void setSubCol(int col, int from, Matrix values) {
		...
		for(int i = 0 ; i < values.getColumnDimension() ; i++){
			setElement(from + i, col, values.get(i + 1));
		}
	}
	...
	public Matrix getSubVector(int from, int to) {
		...
		if(isRowVector()){
			return getSubRow(0+1 , from, to);
		} else {
			return getSubColumn(0+1, from, to); 
		}
	}
...
//				has no factors other than 1 and itself.
		ArrayList<Integer> prim = Util.generatePrimes(n);
