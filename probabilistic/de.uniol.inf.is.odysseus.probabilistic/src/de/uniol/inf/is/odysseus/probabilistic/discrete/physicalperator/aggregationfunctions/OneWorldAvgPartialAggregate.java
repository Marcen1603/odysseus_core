...
public class OneWorldAvgPartialAggregate<T> implements IPartialAggregate<T> {
	...
	/**
	 * Default constructor with approximation error (\f$ \epsilon \f$) and constant upper probability bound (\f$ \theta = 1/e \f$).
	 * 
	 * @param epsilon
	 *            Approximation error
	 * @param datatype
	...
	/**
	 * Gets the smallest even integer that is greater or equal to \f$ 5 \ln(2 P/\epsilon) \f$.
	 * 
			...
			for (int j = 1; j <= this.storeIndex; j++) {
				if (j != i) {
					if (product == null) {
						...
						product = product.multiply((new Polynomial(1.0, 0)).substract(new Polynomial(this.probabilityStore[j - 1], 1)));
					}
				} else {
	...
	private boolean checkClaim28(final double[] p, final double[] a, final double z0) {
		...
		for (int i = 1; i <= this.k0; i++) {
			for (double z = 0; z <= z0; z += 0.01) {
				...
				if (!result) {
					OneWorldAvgPartialAggregate.LOG.error("Error at (i=" + i + ",z=" + z + "): " + (p[i - 1] * Math.pow(z, i)) + " <= " + ((p[0] * z) * Math.pow(this.theta, (i - 1))));
					return result;
				}
			}
		}
		for (int i = 1; i < this.k1; i++) {
			for (int z = 0; z <= z0; z++) {
				...
				if (!result) {
					OneWorldAvgPartialAggregate.LOG.error("Error at (i=" + i + ",z=" + z + "): " + (a[i] * Math.pow(z, i)) + " <= " + (a[0] * Math.pow(this.theta, i)));
					return result;
				}
			}
		}
	...
	private boolean checkTheorem23(final double rho, final double mean) {
		...
		if (this.pBig[0] >= Math.E) {
			...
			if (!result) {
				OneWorldAvgPartialAggregate.LOG.error("Error at: " + (((rho * this.aBig[0]) / this.pBig[0]) * (1 - ((2 * Math.log(this.pBig[0])) / this.pBig[0]))) + " <= " + mean + " " + mean + " <= " + (((rho * this.aBig[0]) / this.pBig[0]) * (1 + (1 / (this.pBig[0] - 1)))));
			}
		} else {
			for (double delta = 0.0001; delta < 1.0; delta += 0.0001) {
				final double lhs = ((rho * this.aBig[0]) / this.pBig[0]) * (delta / Math.log(1 / (1 - delta))) * (1 - Math.pow(1 - delta, this.pBig[0]));
				result &= (lhs <= mean);
			}
		}
		OneWorldAvgPartialAggregate.LOG.trace("Check pass " + result);
	...
	private boolean checkLemma29(final Polynomial gTilde, final double z0) {
		...
		if (!result) {
			OneWorldAvgPartialAggregate.LOG.error("Error at (z=" + z0 + "): " + (lhs.evaluate(z0)) + " <= " + (gTilde.evaluate(z0)) + " && " + gTilde.evaluate(z0) + " <= " + rhs.evaluate(z0));
			return result;
		}
	...
	private boolean checkLemma210(final double[] p, final double l, final double z0) {
		...
		if (!result) {
			OneWorldAvgPartialAggregate.LOG.error("Error at (l=" + l + ",z=" + z + "): gTildek0(z) (" + rhs + ") <= product(sum(1/j! (-Pi z^i)^j/i^j)) (" + p[0] + "(" + z0 + ")=" + lhs + ")");
			return result;
		}

		result &= lhs <= ((1 + this.epsilon) * rhs);
		if (!result) {
			OneWorldAvgPartialAggregate.LOG.error("Error at (z=" + z + "): product(sum(1/j! (-Pi z^i)^j/i^j)) (" + lhs + ") <= (1 + epsilon)gTildek0(z) (" + rhs + ")");
			return result;
		}
	...
	private boolean checkProof27(final double[] p, final double[] a, final double z0) {
		...
		if (!result) {
			OneWorldAvgPartialAggregate.LOG.error("Error at: " + rhs + " == " + (p[0] * Math.exp(-p[0] * z0)) + " -> " + (Math.abs(rhs - (p[0] * Math.exp(-p[0] * z0)))));
			return result;
		}
		result &= lhs <= rhs;
		if (!result) {
