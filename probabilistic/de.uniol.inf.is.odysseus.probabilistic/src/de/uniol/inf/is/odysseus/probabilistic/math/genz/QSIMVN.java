...
public final class QSIMVN {
	...
	public static QSIMVNResult qsimvn(int m, Matrix r, Matrix a, Matrix b) {
		...
		double nv = max(new double[] { m / ns, 1 }); // double nv = max( [ m/ns
														// 1 ] );
		...
		for (int i = 1; i <= ns; i++) { // for i = 1 : ns
			...
			if (Math.abs(d) > 0) {
				e = Math.abs(d)
						* Math.sqrt(1 + Math.pow((e / d), 2 * (i - 2) / i));
			} else {
				if (i > 1) {
					e = e * Math.sqrt((i - 2) / i);
				}
	...
	private static Matrix rand(int n) {
		...
		// TODO schauen ob diese methode �quivalent zu mathlabs rand(n) ist.
	...
	public static ChlrdrResult chlrdr(Matrix r, Matrix a, Matrix b) {
		...
		for (int k = 1; k <= n; k++) {
			...
			if (ckk > ep * k) {
				...
				if (Math.abs(dem) > ep) {
					y.set(k,
							(exp(Math.pow(-am, 2) / 2) - exp(Math.pow(-bm, 2) / 2))
									/ (sqtp * dem)); // TODO Math.pow(-bm, 2/2)
														// ODER Math.pow(-bm,
														// 2/2)
				} else {
					if (am < -10) {
	...
	public static void main(String[] args) {
		...
		Matrix R = new Matrix(new double[][] { { 15, 3, 2, 1 },
				{ 3, 5, -1, 1 }, { 2, -1, 4, 2 }, { 1, 1, 2, 5 } });
