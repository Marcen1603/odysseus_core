 ...
 */
public class Evidence<T extends Comparable<T>> implements Serializable,
		Cloneable, Comparable<Evidence<T>>, Set<T> {
	...
	public Evidence(T event) {
		this.events.add(event);
	}

	public Evidence(T... events) {
		for (T event : events) {
			this.events.add(event);
		}
	}

	public Evidence(Collection<T> events) {
		for (T event : events) {
			this.events.add(event);
		}
	}
	...
	public Evidence<T> intersection(final Evidence<T> other) {
		final Evidence<T> intersection = new Evidence<T>(getEvents());
	...
	public List<Evidence<T>> powerSet() {
		...
		for (final T event : this) {
			...
			final Evidence<T> singleton = new Evidence<T>(
