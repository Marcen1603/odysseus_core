 ...
 * 
{
    ...
    "name": "gameTime",
    "parameters": {
        "startts": 10753295594424116,
        "endts": 14879639146403495
    }
};
 * 
...
public class GameTimeSportsQLParser implements ISportsQLParser {
	...
	public ILogicalQuery parse(SportsQLQuery sportsQL)	throws SportsQLParseException {
		...
		MapAO newtsMap = OperatorBuildHelper.createMapAO(newtsMapExpressions, gameTimeSelect, 0, 0);
		...
		gameTimeMapExpressions.add(OperatorBuildHelper.createExpressionParameter("ToLong(("+ATTRIBUTE_NEW_TS+" - ToLong("+ATTRIBUTE_NEW_TS+")) * 60)", "second",newtsMap));
		gameTimeMapExpressions.add(OperatorBuildHelper.createExpressionParameter("ToLong(((("+ATTRIBUTE_NEW_TS+" - ToLong("+ATTRIBUTE_NEW_TS+")) * 60) - ToLong(("+ATTRIBUTE_NEW_TS+" - ToLong("+ATTRIBUTE_NEW_TS+")) * 60))*1000)", ATTRIBUTE_MILLISECOND,newtsMap));

		MapAO gameTimeMap = OperatorBuildHelper.createMapAO(gameTimeMapExpressions, newtsMap, 0, 0);
		...
		ChangeDetectAO gameTimeChangeDetect = OperatorBuildHelper.createChangeDetectAO(OperatorBuildHelper.createAttributeList(gameTimeChangeDetectAttributes, gameTimeMap),0, gameTimeMap);
		allOperators.add(gameTimeChangeDetect);			
		
		// 5. Finish		
		return OperatorBuildHelper.finishQuery(gameTimeChangeDetect, allOperators, sportsQL.getName());		
	}
