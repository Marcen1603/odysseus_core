...
import de.uniol.inf.is.odysseus.sports.distributor.helper.DistributionConfigBuildHelper.PARSER;
...
public class SportsQLDistributorRegistry {
	...
	static Logger logger = LoggerFactory.getLogger(SportsQLDistributorRegistry.class);
	
	private static Map<String, ISportsQLDistributor> sportsQLDistributorMap = Maps.newHashMap();
	...
	public static void registerSportsQLDistributor(ISportsQLDistributor distributor) {
		if (!sportsQLDistributorMap.containsKey(distributor.getDistributorName()
				.toLowerCase())) {
			sportsQLDistributorMap.put(distributor.getDistributorName().toLowerCase(),
					distributor);
		} else {
			logger.debug("Distributor " + distributor.getDistributorName().toLowerCase()
					+ " already added");
		}
		
	}

	public static void unregisterSportsQLDistributor(ISportsQLDistributor distributor) {
		if (sportsQLDistributorMap.containsKey(distributor.getDistributorName()
				.toLowerCase())) {
			sportsQLDistributorMap.remove(distributor.getDistributorName().toLowerCase());
		}
	}
	...
	if(p2pDictionary.getRemotePeerIDs().size() > 0){
		if (!sportsQLDistributorMap.containsKey(type+"_"+game+"_"+name)) {
			logger.info("Distributor for: " + type+"_"+game+"_"+name + " is not valid. You get the default distributor!");
			ISportsQLDistributor distributor = sportsQLDistributorMap.get("default");
			return distributor;
		}else{
			ISportsQLDistributor distributor = sportsQLDistributorMap.get(type+"_"+game+"_"+name);
			return distributor;
		}
	}else{
	//no peer found --> no distribution 
		ISportsQLDistributor distributor = sportsQLDistributorMap.get("nodistribution");
		return distributor;
	}		
	}
}
