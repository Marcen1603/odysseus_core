...
import de.uniol.inf.is.odysseus.pnapproach.id.physicalopertor.relational.join.helper.ResultAwarePNTransferFunction;
import de.uniol.inf.is.odysseus.pnapproach.id.sweeparea.DefaultPNIDSweepArea;

/**
 * Dieser Join merkt sich die erzeugten Ergenisse und schreibt nur dann ein
 * negatives Wildcardelement in den Ausgabedatenstrom,wenn das entsprechen positive Element
 * auch Ergebnisse produziert hat.
 * 
 * @author abolles
 *
 ...
 * @param <T> Datentyp
 */
public class ResultAwareJoinPNIDPO<M extends IPosNeg, T extends IMetaAttributeContainer<M>> extends
	...
	public ResultAwareJoinPNIDPO(IDataMergeFunction<T> dataMerge,
		...
		this.areas = areas;
		this.nareas = nareas;
		this.results = new HashMap<List<Long>, HashMap<List<Long>, PointInTime>>();
	...
	public void process_next_internal(T object, int port){
		...
		synchronized(this.nareas){
			synchronized(this.areas){
				...
				while(negatives.hasNext()){
					...
					List<PointInTime> resultTimestamps = new ArrayList<PointInTime>();
					Iterator<T> removedElems = areas[otherport].insertAndReturnRemoved(neg).iterator();
					while(removedElems.hasNext()){
						T removed = removedElems.next();
						resultTimestamps.addAll(this.removeJoinResultsForElement(removed));
					}
					
					if(!resultTimestamps.isEmpty()){
						...
						// falls es einen Zeitstempel gibt, der vor dem Zeitstempel in newMData liegt,
						// so muss das Result auf jeden Fall in die SweepArea der TransferFunction
						// eingefügt werden, auch wenn falsch geschedulete Ergebnisse durch neg entfernt
						// wurden.
						Collections.sort(resultTimestamps);
						for(PointInTime p: resultTimestamps){
							// Es gibt einen, der davor liegt, also
							// füge neg ein.
							if(p.before(neg.getMetadata().getTimestamp())){
								this.transferFunction.transferForce(neg);
								break;
							}
							else if(p.after(neg.getMetadata().getTimestamp())){
								break;
							}
						}
					}
		...
		if(object.getMetadata().getElementType() == ElementType.POSITIVE){
			...
			while (qualifies.hasNext()) {
				...
				// store it under results
				this.addJoinResult(object, next, newElement.getMetadata().getTimestamp());
			}
		}
		else{
			if(lastTs[otherport].after(object.getMetadata().getTimestamp())){
				synchronized(this.nareas){
					synchronized(this.areas){
						List<PointInTime> resultTimestamps = new ArrayList<PointInTime>();
						Iterator<T> removedElems = areas[port].insertAndReturnRemoved(object).iterator();
						while(removedElems.hasNext()){
							T removed = removedElems.next();
							resultTimestamps.addAll(this.removeJoinResultsForElement(removed));
						}
						...
						if(!resultTimestamps.isEmpty()){
							...
							for(PointInTime p: resultTimestamps){
								// Es gibt einen, der davor liegt, also
								// füge neg ein.
								if(p.before(object.getMetadata().getTimestamp())){
									this.transferFunction.transferForce(object);
									break;
								}
								else if(p.after(object.getMetadata().getTimestamp())){
									break;
								}
							}
						}
	...
	/**
	 * Stores the information that left has been joined with right
	 */
	private synchronized void addJoinResult(T left, T right, PointInTime timestamp){
		synchronized(this.results){
			HashMap<List<Long>, PointInTime> results_of_left = this.results.get(left.getMetadata().getID());
			if(results_of_left == null){
				results_of_left = new HashMap<List<Long>, PointInTime>();
			}
			
			// DEBUG
	//		if(results_of_left.get(right.getMetadata().getID()) != null){
	//			System.out.println("Kollision in results_of_left");
	//		}
			results_of_left.put(right.getMetadata().getID(), timestamp);
			
			this.results.put(left.getMetadata().getID(), results_of_left);
			
			HashMap<List<Long>, PointInTime> results_of_right = this.results.get(right.getMetadata().getID());
			if(results_of_right == null){
				results_of_right = new HashMap<List<Long>, PointInTime>();
			}
			
			// DEBUG
	//		if(results_of_right.get(left.getMetadata().getID()) != null){
	//			System.out.println("Kollision in results_of_right");
	//		}
			results_of_right.put(left.getMetadata().getID(), timestamp);
			this.results.put(right.getMetadata().getID(), results_of_right);
		}
	}
	
	/**
	 * If {removed} has been joined with other elements, this information will
	 * be deleted from the results table
	 * 
	 *  @return True, if {removed} has produced join results, false otherwise
	 */
	private synchronized List<PointInTime> removeJoinResultsForElement(T removed){
		List<PointInTime> timestamps = new ArrayList<PointInTime>();
		synchronized(this.results){
			HashMap<List<Long>, PointInTime> results_of_removed = this.results.remove(removed.getMetadata().getID());
			if(results_of_removed != null && !results_of_removed.isEmpty()){
				for(List<Long> id : results_of_removed.keySet()){
					PointInTime timestamp = this.results.get(id).remove(removed.getMetadata().getID());
