...
/**
 * Dieser JoinPNPO merkt sich nicht, welche Ergebnisse werzeugt wurden.Ein
 * negatives Element erhält immer eine Wildcard-ID und wird in den
 ...
 *            Datentyp
 */
public class JoinPNPO<M extends IPosNeg, T extends IMetaAttributeContainer<M>>
		extends AbstractBinaryPNPipe<T, T> {
	private static final Logger logger = LoggerFactory
			.getLogger(JoinPNPO.class);
	...
	private void process_next_internal(T object, int port) {
		...
		synchronized (this.nareas) {
			synchronized (this.areas) {
				Iterator<T> negatives = this.nareas[otherport]
						.extractElementsBefore(new PointInTime(object.getMetadata()
								.getTimestamp().getMainPoint() + 1));
				while (negatives.hasNext()) {
					T neg = negatives.next();
					areas[otherport].insert(neg, null);

					// das negative Element joint jetzt mit allen Elementen der
					// anderen Seite,
					// mit denen auch das entsprechende positive Element gejoint
					// hat.
					Iterator<T> negQualifies = this.areas[port].queryCopy(neg,
							otherorder);
		...
		if (object.getMetadata().getElementType() == ElementType.POSITIVE) {
			...
			// Falls im anderen Strom schon neuere Elemente aufgetaucht sind, so
			// kann
			// das negative direkt verarbeitet werden und muss nicht erst in der
			// negative
			// SA gespeichert werden.
			if (lastTs[otherport].afterOrEquals(object.getMetadata()
					.getTimestamp())) {
				synchronized (this.nareas) {
					synchronized (this.areas) {
						areas[port].insert(object, null);

						// das negative Element joint jetzt mit allen Elementen
						// der anderen Seite,
						// mit denen auch das entsprechende positive Element
						// gejoint hat.
						Iterator<T> negQualifies = this.areas[otherport]
								.queryCopy(object, order);
						while (negQualifies.hasNext()) {
							T next = negQualifies.next();
							T newElement = merge(object, next, order);
							transferFunction.transfer(newElement);
	 ...
	 * @param object
	 * @param port
	 * @return The element with the minimal timestamp of both pre areas (index
	 *         0) and its port (index 1)
	 */
	private Object[] preStore(T object, int port) {
		...
		// System.out.println("Prearas JoinPNPO: " + this + ", left: " +
		// this.preareas[0].size() + ", right: " + this.preareas[1].size());

		if (left_min != null && right_min != null) {

			if (left_min.getMetadata().getTimestamp().before(
					right_min.getMetadata().getTimestamp())) {
				nextElem[0] = this.preareas[0].poll();
				nextElem[1] = 0;
				this.lastPort = 0;
			} else if (right_min.getMetadata().getTimestamp().before(
					left_min.getMetadata().getTimestamp())) {
				nextElem[0] = this.preareas[1].poll();
				nextElem[1] = 1;
				this.lastPort = 1;
			} else {
				// Die Zeitstempel sind gleich, also schreibe zuerst das
				// negative heraus.
				...
				// beide elemente gleich und beide elemente positiv, also mache
				// ein lookahead
				else {
					PointInTime peekTs = this.preareas[0].peek().getMetadata()
							.getTimestamp();
					...
					while (counter < this.lookahead
							&& (iter_right.hasNext() || iter_left.hasNext())) {
						if (iter_right.hasNext() && iter_left.hasNext()) {
							left = iter_left.next();
							right = iter_right.next();
							if (left.getMetadata().getTimestamp().after(peekTs)
									|| right.getMetadata().getElementType() == ElementType.NEGATIVE) {
								usePort = 1;
								break;
							} else if (right.getMetadata().getTimestamp()
									.after(peekTs)
									|| left.getMetadata().getElementType() == ElementType.NEGATIVE) {
								usePort = 0;
								break;
							}
						} else if (!iter_left.hasNext()
								&& iter_right.hasNext()
								&& iter_right.next().getMetadata()
										.getTimestamp().equals(peekTs)) {
							usePort = 1;
							break;
						} else if (!iter_right.hasNext()
								&& iter_left.hasNext()
								&& iter_left.next().getMetadata()
										.getTimestamp().equals(peekTs)) {
							usePort = 0;
							break;
						}
		...
		// falls der Port, auf dem etwas null ist, auch done ist, kann der
		// andere rausgeschrieben werden
	...
	private T merge(T left, T right, Order order) {
		if (logger.isTraceEnabled()) {
			logger.trace("ResultAwareJoinPNIDPO (" + hashCode()
					+ ") start merging: " + left + " AND " + right);
		}
		T mergedData;
		M mergedMetadata;
	...
	protected void process_done(int port) {
		...
		if (this.isDone[otherport]) {
			while (!this.preareas[port].isEmpty()
					&& !this.preareas[otherport].isEmpty()) {
				T left_min = this.preareas[0].peek();
				T right_min = this.preareas[1].peek();
		...
		// der andere port ist noch nicht fertig
			...
			// falls die liste port, welcher ja done ist, leer ist, kann die
			// andere auch geleert werden
			if (this.preareas[port].isEmpty()) {
				while (!this.preareas[otherport].isEmpty()) {
					this.process_next_internal(this.preareas[otherport].poll(),
							0);
				}
			}
		}
		...
		// falls der andere port auch done ist, kann es sein, dass durch den
		// vorherigen
		// aufruf dieser methode, noch negative elemente in der eigenen SA
		// vorhanden sind.
		...
		//					
		// // das negative Element joint jetzt mit allen Elementen der anderen
		// Seite,
		// // mit denen auch das entsprechende positive Element gejoint hat.
		// Iterator<T> negQualifies = this.areas[otherport].queryCopy(neg,
		// order);
		// while(negQualifies.hasNext()){
		// T next = negQualifies.next();
	...
	protected void process_done() {
		...
		System.out.println("Falsch erzeugte Ergebnisse: "
				+ this.transferFunction.getWrongResultCount());
	 ...
	 * 
	 * @param factory
	 *            - {@link MemoryUsageMonitoringDataFactory}, die ein
	 *            MetadataItem erzeugt welches auf die SweepAreas zugreifen
	 *            kann.
	 */
	// public void addMemoryUsageMetadataItem(MemoryUsageMonitoringDataFactory
	// factory) {
	// IMonitoringData<Integer> item = factory.createPNJoinMemoryHandler(this,
	// preareas, areas, nareas);
