 ...
 *
 */
public class CIELCHToRGBFunction extends AbstractFunction<double[]> {
    /**
     *
     */
    private static final long serialVersionUID = 5101008620656321363L;
    private static final SDFDatatype[][] ACC_TYPES = new SDFDatatype[][] { SDFDatatype.NUMBERS, SDFDatatype.NUMBERS, SDFDatatype.NUMBERS };

    /**
     *
     */
    public CIELCHToRGBFunction() {
        super("CIELCHToRGB", 3, CIELCHToRGBFunction.ACC_TYPES, SDFDatatype.VECTOR_DOUBLE);
    }
    ...
    public double[] getValue() {
        ...
        final double C = this.getNumericalInputValue(1).doubleValue();
        final double H = this.getNumericalInputValue(2).doubleValue();

        final RGB colorSpace = (new CIELCH(L, C, H)).toRGB();
