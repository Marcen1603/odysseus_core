...
public abstract class AbstractEuclideanDistanceFunction extends AbstractFunction<Double> {
    ...
    public AbstractEuclideanDistanceFunction(final SDFDatatype[][] accTypes) {
        super("EuclideanDistance", 2, accTypes, SDFDatatype.DOUBLE);
    }
    ...
    protected final static Double getValueInternal(final RealMatrix a, final RealMatrix b) {
        return Double.valueOf(a.walkInOptimizedOrder(new RealMatrixPreservingVisitor() {
            ...
            public void visit(final int row, final int column, final double value) {
                final double diff = value - b.getEntry(row, column);
                this.distance += diff * diff;
            }

            @Override
            public double end() {
                return FastMath.sqrt(this.distance);
            }
