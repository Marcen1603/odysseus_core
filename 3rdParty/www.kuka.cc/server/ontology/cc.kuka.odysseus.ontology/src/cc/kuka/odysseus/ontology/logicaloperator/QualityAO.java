...
public class QualityAO extends UnaryLogicalOp {
	 ...
	 * Property1_1Expr2]), DOUBLE.MAX), eif((Condition2_1) AND (Condition2_2),
	 * sMax([Property2_1Expr1, Property2_2Expr2]),DOUBLE.MAX))
	 *
	 *
			...
			if (!sensingDevices.isEmpty()) {
				...
				for (final MeasurementCapability measurementCapability : measurementCapabilities) {
					...
					final Map<String, StringBuilder> measurementPropertyExpression = QualityAO
							.getMeasurementPropertyExpressions(measurementCapability);
				...
				for (final String measurementPropertyName : measurementPropertyNames) {
					if (attributeMeasurementPropertyExpression.containsKey(measurementPropertyName)) {
						...
						for (final StringBuilder conditionExpression : conditionMeasurementPropertyMapping.keySet()) {
							...
							expression.append("toDouble(eif(");
							expression.append(conditionExpression);
							expression.append(",");
							expression.append("toDouble(max(");
							expression.append(conditionMeasurementPropertyMapping.get(conditionExpression));
							expression.append("))");
							expression.append(",");
							expression.append("toDouble(0.0)");
							expression.append("))");
						}
						expression.insert(0, "toDouble(max([");
						expression.append("]))");
						exprs[i] = new SDFExpression(expression.toString(), null, MEP.getInstance());
					} else {

						exprs[i] = new SDFExpression("toDouble(-1.0)", null, MEP.getInstance());
					}
					i++;
				}
			} else {
	...
	private static StringBuilder getConditionExpression(final MeasurementCapability measurementCapability) {
		...
		for (final Condition condition : conditions) {
			final List<String> observerAttributeUris = Activator.getSensorOntologyService().attributes(condition);
	 ...
	 */
	private static Map<String, StringBuilder> getMeasurementPropertyExpressions(
		...
		for (final MeasurementProperty measurementProperty : measurementProperties) {
			if (measurementProperty != null) {
				...
				if (!measurementPropertyExpressions
						.containsKey(URI.create(measurementProperty.resource()).getFragment())) {
				...
				if (measurementProperty.expression() != null) {
					// Inject the observed attribute name for evaluation during
					// processing
					expressionBuilder.append(String.format("toDouble(" + measurementProperty.expression() + ")",
							measurementCapability.name()));
				} else {
					expressionBuilder.append("toDouble(0.0)");
				}
			}
		}
