...
public class RInsertSensingDevicesRule extends AbstractRewriteRule<QualityAO> {
                        ...
                        if (!observerAttributes.isEmpty()) {
                            ...
                            if (!inSchema) {
                                ...
                                frequencies.put(observerAttributes.get(0).getSourceName(),
                                        new Double(FastMath.max(frequencies.get(observerAttributes.get(0).getSourceName()).doubleValue(), frequency)));
                            }
                        }
                    }
    ...
    private void concatConditionObserverStreams(final ILogicalOperator operator, final Map<String, List<SDFAttribute>> observers, final Map<String, Double> frequencies) {
        ...
        for (final String sourceName : observers.keySet()) {
            ...
            // TODO Set frequency
            final WindowAO windowAO = RInsertSensingDevicesRule.insertWindowAO(projectAO, frequencies.get(sourceName).doubleValue());
    ...
    private static MapAO insertMapAO(final QualityAO parent) {
        ...
        for (int i = 0; i < parent.getOutputSchema().size(); i++) {
            final SDFExpression expr = new SDFExpression(parent.getExpressions()[i].getMEPExpression().toString(),parent.getExpressions()[i].getAttributeResolver(), MEP.getInstance());
