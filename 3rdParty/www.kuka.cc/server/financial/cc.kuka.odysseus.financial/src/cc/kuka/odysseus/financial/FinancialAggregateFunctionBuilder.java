...
import cc.kuka.odysseus.financial.physicaloperator.relational.RelationalNetPresentValue;
...
public class FinancialAggregateFunctionBuilder extends AbstractAggregateFunctionBuilder {
    ...
    {
        FinancialAggregateFunctionBuilder.names.add(FinancialAggregateFunctionBuilder.NPV);
    }
    ...
    public final Collection<String> getFunctionNames() {
        return FinancialAggregateFunctionBuilder.names;
    }
    ...
    public final IAggregateFunction<Tuple<?>, Tuple<?>> createAggFunction(final AggregateFunction key, final SDFSchema schema, final int[] pos, final boolean partialAggregateInput,
        ...
        if (key.getName().equalsIgnoreCase(FinancialAggregateFunctionBuilder.NPV)) {
            aggFunc = RelationalNetPresentValue.getInstance(pos[0], pos[1], partialAggregateInput);
        }
        else {
