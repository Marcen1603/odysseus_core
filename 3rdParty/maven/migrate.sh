...
echo "cp -uR $partyDir/odysseus-parent ."
...
for f in $folders; do
	...
	if [ $isin = 0 ]; then
		...
		if [ -d $partyDir/$f/odysseus-$artifactId ]; then
		    echo cp -u $partyDir/$f/odysseus-$artifactId/pom.xml $f/odysseus-$artifactId/
		else
		    echo "Generate pom"
...
rm -rf scheduler/odysseus-scheduler
rm -rf scheduler/odysseus-scheduler-singlethreadscheduler
...
cp -u $partyDir/scheduler/odysseus-scheduler/pom.xml scheduler/odysseus-scheduler/
cp -u $partyDir/scheduler/odysseus-scheduler-singlethreadscheduler/pom.xml scheduler/odysseus-scheduler-singlethreadscheduler
