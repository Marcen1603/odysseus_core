...
public final class GridUtil {
	...
	public static final double FREE = MIN_VALUE;
	public static final double UNKNOWN =  0.5;
	public static final double OBSTACLE = MAX_VALUE;
	 ...
	 */
	public static boolean isInGrid(double x, double y, double width,
			double height, Coordinate coordinate) {
		return ((coordinate.x >= x) && (coordinate.x <= x + width)
				&& (coordinate.y >= y) && (coordinate.y <= y + height));
	}

	/**
	 * Checks weather the given {@link Coordinate Coordinates} are in the same
	 * grid cell
	 * 
	 * @param x
	...
	public static boolean isInSameGridCell(double x, double y, double cellsize,
		...
		return (Math.ceil((coordinate1.x - x) / cellsize) == Math
				.ceil((coordinate2.x - x) / cellsize))
				&& (Math.ceil((coordinate1.y - y) / cellsize) == Math
						.ceil((coordinate2.y - y) / cellsize));
	}
	 ...
	 * @return true if the coordinate is in the grid cell else false
	 */
	public static boolean isInGridCell(double x, double y, double cellsize,
			Coordinate coordinate) {
		return ((coordinate.x >= x) && (coordinate.x <= x + cellsize)
				&& (coordinate.y >= y) && (coordinate.y <= y + cellsize));
	}
	 ...
	 */
	private static double getDenominator(double x1, double y1, double x2,
			double y2, Coordinate from, Coordinate to) {
	 ...
	 */
	private static double getNumeratorA(double x1, double y1, double x2,
	 ...
	 */
	private static double getNumeratorB(double x1, double y1, double x2,
			double y2, double x3, double y3) {
		return ((x2 - x1) * (y1 - y3)) - ((y2 - y1) * (x1 - x3));
	}

	/**
	 * Checks weather the given grid cell intersects with a line of the polygon
	 ...
	 * @param to
	 *            The second {@link Coordinate} of the polygon
	 * @return true if the line formed by the coordinates intersects with the
	 *         grid cell else false
	 */
	public static boolean intersects(double x, double y, double cellsize,
			Coordinate from, Coordinate to) {
		// Check bottom left to top left segment for intersection
		double denominator = getDenominator(x, y, x, y + cellsize, from, to);
		double numeratorA = getNumeratorA(x, y, x, y + cellsize, from.x,
				from.y, to.x, to.y);
		double numeratorB = getNumeratorB(x, y, x, y + cellsize, from.x, from.y);
		if ((denominator != 0.0)
				&& (isIntersection(denominator, numeratorA, numeratorB))) {
			return true;
		}

		// Check top left to top right segment for intersection
		denominator = getDenominator(x, y + cellsize, x + cellsize, y
				+ cellsize, from, to);
		numeratorA = getNumeratorA(x, y + cellsize, x + cellsize, y + cellsize,
				from.x, from.y, to.x, to.y);
		numeratorB = getNumeratorB(x, y + cellsize, x + cellsize, y + cellsize,
				from.x, from.y);
		...
		denominator = getDenominator(x + cellsize, y + cellsize, x + cellsize,
				y, from, to);
		numeratorA = getNumeratorA(x + cellsize, y + cellsize, x + cellsize, y,
				from.x, from.y, to.x, to.y);
		numeratorB = getNumeratorB(x + cellsize, y + cellsize, x + cellsize, y,
				from.x, from.y);
		if (isIntersection(denominator, numeratorA, numeratorB)) {
			return true;
		}
	...
	/**
	 * Checks if the given numerators and the denominator allows an intersection
	 * between the two line segments which is true if A/d and B/d are both
	 ...
	 * @return
	 */
	private static boolean isIntersection(double denominator,
			double numeratorA, double numeratorB) {
	 ...
	 * @return The bounding box as an array of four double values with 0: minX,
	 *         1:minY, 2:maxX, 3:maxY
	 */
	 ...
	 */
	public static boolean isInPolygon(Coordinate coordinate,
			Coordinate[] polygon) {
