...
//	private static final Logger LOG = LoggerFactory.getLogger(ChoroplethLayer.class);
	...
	public ChoroplethLayer(ScreenTransformation transformation, SDFAttribute sdfAttribute, int geoStreamPosition, int valueStreamPosition) {
	...
	public ChoroplethLayer(String name, ScreenTransformation transformation, SDFAttribute sdfAttribute, int geoStreamPosition, int valueStreamPosition) {
		this(transformation, sdfAttribute, geoStreamPosition, valueStreamPosition);
		this.name = name;
	}
	...
	private void drawGeometryAndValue(GeometryAndValue geometryAndValue, GC gc) {
		if (geometryAndValue.getGeometry() instanceof Polygon) {
			drawPolygonAndValue((Polygon)geometryAndValue.getGeometry(), geometryAndValue.getValue(), gc);
		}else if (geometryAndValue.getGeometry() instanceof GeometryCollection) {
			drawGeometryCollectionAndValue((GeometryCollection) geometryAndValue.getGeometry(), geometryAndValue.getValue(), gc);
		}
	}
	private void drawGeometryCollectionAndValue(GeometryCollection geometryCollection, Integer value, GC gc) {
		for (int i = 0; i < geometryCollection.getNumGeometries(); i++) {
			drawGeometryAndValue( new GeometryAndValue(geometryCollection.getGeometryN(i), value) , gc);
		}
...
//		this.style.draw(gc, list);	
	...
	public void addGeometryAndValue(Geometry geometry, Integer value) {
		synchronized (geometryAndValueList) {
			this.geometryAndValueList.offer(new GeometryAndValue(geometry, value));
		}
	}
