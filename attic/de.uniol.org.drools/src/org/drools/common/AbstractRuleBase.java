 ...
 */
abstract public class AbstractRuleBase implements InternalRuleBase,
		Externalizable {
	// ------------------------------------------------------------
	// Instance members
	// ------------------------------------------------------------
	...
	/**
	 * This lock is used when adding to, or reading the
	 * <field>statefulSessions</field>
	 */
	...
	public AbstractRuleBase(final String id,
		...
		this.rootClassLoader = new CompositeClassLoader(this.config
				.getClassLoader());
		...
		this.classFieldAccessorCache = new ClassFieldAccessorCache(
	...
	public void writeExternal(final ObjectOutput out) throws IOException {
		...
		// Rules must be restored by an ObjectInputStream that can resolve using
		// a given ClassLoader to handle seaprately by storing as
		// a byte[]
	...
	public void readExternal(final ObjectInput in) throws IOException,
		...
		this.rootClassLoader = new CompositeClassLoader(droolsStream
		...
		for (final Object object : this.pkgs.values()) {
			((Package) object).getDialectRuntimeRegistry().onAdd(
					this.rootClassLoader);
		}
		...
		Class cls = null;
		try {
			cls = droolsStream.getParentClassLoader().loadClass(
					droolsStream.readUTF());
		...
		for (final Object object : this.pkgs.values()) {
			...
			((Package) object).getClassFieldAccessorStore()
					.setClassFieldAccessorCache(this.classFieldAccessorCache);
		...
		// read globals
		Map<String, String> globs = (Map<String, String>) droolsStream
				.readObject();
		populateGlobalsMap(globs);

		this.partitionIDs = (List<RuleBasePartitionId>) droolsStream
				.readObject();
	 ...
	 */
	private void populateGlobalsMap(Map<String, String> globs)
			throws ClassNotFoundException {
		this.globals = new HashMap<String, Class<?>>();
		for (Map.Entry<String, String> entry : globs.entrySet()) {
			this.globals.put(entry.getKey(), this.rootClassLoader
	...
	private void populateTypeDeclarationMaps() throws ClassNotFoundException {
		...
		for (Package pkg : this.pkgs.values()) {
			for (TypeDeclaration type : pkg.getTypeDeclarations().values()) {
				type.setTypeClass(this.rootClassLoader.loadClass(type
						.getTypeClassName()));
	...
	public StatefulSession newStatefulSession() {
		return newStatefulSession(
				new SessionConfiguration(this.rootClassLoader),
	...
	public Process[] getProcesses() {
		return (Process[]) this.processes.values().toArray(
				new Process[this.processes.size()]);
	}
	...
	public void addPackages(final Collection<Package> newPkgs) {
		try {
			lock();
			// we need to merge all byte[] first, so that the root classloader
			// can resolve classes
			for (Package newPkg : newPkgs) {
				...
				pkg.getDialectRuntimeRegistry().merge(
						newPkg.getDialectRuntimeRegistry(),
						this.rootClassLoader);
			}

			// now iterate again, this time onBeforeExecute will handle any
			// wiring or cloader re-creating that needs to be done as part of
			// the merge
			for (Package newPkg : newPkgs) {
				Package pkg = this.pkgs.get(newPkg.getName());

				// this needs to go here, as functions will set a java dialect
				// to dirty
				if (newPkg.getFunctions() != null) {
					for (Map.Entry<String, Function> entry : newPkg
				...
				// with the classloader recreated for all byte[] classes, we
				// should now merge and wire any new accessors
				pkg.getClassFieldAccessorStore().merge(
						newPkg.getClassFieldAccessorStore());
			}

			for (Package newPkg : newPkgs) {
				Package pkg = this.pkgs.get(newPkg.getName());

				// we have to do this before the merging, as it does some
				...
				try {
					...
					if (newPkg.getTypeDeclarations() != null) {
						...
						for (TypeDeclaration type : newPkg
								.getTypeDeclarations().values()) {
							lastType = type;
							type.setTypeClass(this.rootClassLoader
									.loadClass(type.getTypeClassName()));
							// @TODO should we allow overrides? only if the
							// class is not in use.
							if (!this.classTypeDeclaration.containsKey(type
									.getTypeClass())) {
								// add to rulebase list of type declarations
								this.classTypeDeclaration.put(type
										.getTypeClass(), type);
							}
						}
					}
	 ...
	 * into the package).
	 */
	private void mergePackage(final Package pkg, final Package newPkg) {
		// Merge imports
		...
		try {
			...
			if (newPkg.getGlobals() != null
					&& newPkg.getGlobals() != Collections.EMPTY_MAP) {
				Map<String, String> globals = pkg.getGlobals();
				// Add globals
				for (final Map.Entry<String, String> entry : newPkg
						.getGlobals().entrySet()) {
					...
					if (globals.containsKey(identifier)
						...
						pkg.addGlobal(identifier, this.rootClassLoader
								.loadClass(type));
						// this isn't a package merge, it's adding to the
						// rulebase, but I've put it here for convienience
						this.globals.put(identifier, this.rootClassLoader
								.loadClass(type));
					}
				}
			}
		} catch (ClassNotFoundException e) {
			throw new RuntimeDroolsException("Unable to resolve class '"
					+ lastType + "'");
		}
		...
		if (newPkg.getTypeDeclarations() != null) {
			...
			for (TypeDeclaration type : newPkg.getTypeDeclarations().values()) {
				// @TODO should we allow overrides? only if the class is not in
				// use.
		...
		if (newPkg.getRuleFlows() != null) {
			final Map flows = newPkg.getRuleFlows();
			for (final Iterator iter = flows.values().iterator(); iter
					.hasNext();) {
		...
		// // this handles re-wiring any dirty Packages, it's done lazily to
		// allow incremental
		// // additions without incurring the repeated cost.
		// if ( this.reloadPackageCompilationData == null ) {
		// this.reloadPackageCompilationData = new
		// ReloadPackageCompilationData();
		// }
		// this.reloadPackageCompilationData.addDialectDatas(
		// pkg.getDialectRuntimeRegistry() );
	}
	...
	public void addRule(final Package pkg, final Rule rule)
	...
	protected abstract void addRule(final Rule rule)
			throws InvalidPatternException;
		...
		try {
			...
			if (pkg == null) {
				throw new IllegalArgumentException("Package name '"
						+ packageName + "' does not exist for this Rule Base.");
			}
			...
			for (final Iterator it = this.pkgs.values().iterator(); it
					.hasNext();) {
				final org.drools.rule.Package pkgref = (org.drools.rule.Package) it
						.next();
				if (pkgref != pkg) {
			...
			for (final Iterator it = pkg.getGlobals().keySet().iterator(); it
					.hasNext();) {
			...
			for (final Iterator iter = flows.keySet().iterator(); iter
					.hasNext();) {
	...
	public void removeRule(final String packageName, final String ruleName) {
		try {
			...
			this.reloadPackageCompilationData.addDialectDatas(pkg
	...
	public void removeRule(final Package pkg, final Rule rule) {
		try {
			lock();
		...
		synchronized (this.pkgs) {
			...
			if (!pkg.getFunctions().containsKey(functionName)) {
				throw new IllegalArgumentException("function name '"
						+ packageName + "' does not exist in the Package '"
						+ packageName + "'.");
			}
	...
	public InternalWorkingMemory[] getWorkingMemories() {
		...
		try {
			statefuleSessionLock.lock();
			copyOfMemories = new InternalWorkingMemory[this.statefulSessions
					.size()];
	...
	public void addEventListener(final RuleBaseEventListener listener) {
		// no need for synchonization or locking because eventSupport is
		// thread-safe
	...
	public static interface RuleBaseAction extends Externalizable {
