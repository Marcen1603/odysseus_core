...
public class AccessAOBuilder extends AbstractOperatorBuilder {
	...
	protected ILogicalOperator createOperator(List<ILogicalOperator> inputOps) {
		...
		if (DataDictionary.getInstance().containsView(sourceName)) {
			if (host.hasValue() || type.hasValue() || port.hasValue() || attributes.hasValue()) {
				throw new IllegalArgumentException("view " + sourceName + " already exists");
			}
			return DataDictionary.getInstance().getView(sourceName);
		}
		if (!(host.hasValue() && type.hasValue() && port.hasValue() && attributes.hasValue())) {
			throw new IllegalArgumentException("missing information for the creation of a new source. expecting host, port, type and attributes.");
		}
