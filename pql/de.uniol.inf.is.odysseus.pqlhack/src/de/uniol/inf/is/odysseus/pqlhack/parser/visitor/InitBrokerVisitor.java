...
/**
 * This visitor sets the queue port mappings in the broker operator. This means,
 * the visitor traverses the whole query plan. Each time a new ASTNode is
 * visited the preceding (parent) node is passed. If the next (child) node is a
 * Broker-Node, the broker dictionary get a new mapping from queue in-port to
 * data out-port. The port numbers are incremented each time a mapping is found.
 * Since the operators are not subscribed to each other by this visitor, the
 * order of subscription in the next visitor must be the same order as the
 * mappings are detected here. I assume that this will hold automatically since
 * the AST will not be changed.
 * 
 * @author André Bolles
 * 
 */
	...
	public Object visit(ASTBrokerOp broker, Object data) {
		...
		if (data != null && data instanceof ASTAlgebraOp) {
			...
			if (broker.hasQueue()) {
				...
				// by CREATE BROKER
				int curIn = BrokerDictionary.getInstance().getCurrentInputPort(
						broker.getName());
				int curOut = BrokerDictionary.getInstance()
						.getCurrentOutputPort(broker.getName());

				QueuePortMapping qpm = new QueuePortMapping(curIn, curOut);

				BrokerDictionary.getInstance().setCurrentInputPort(
						broker.getName(), curIn + 1);
				BrokerDictionary.getInstance().setCurrentOuputPort(
						broker.getName(), curOut + 1);

				BrokerDictionary.getInstance().addQueuePortMapping(
						broker.getName(), qpm);
			}

			// if the broker has no queue statement, then the current output
			// port must be incremented
			// the parent node is an algebra, the broker is the input for the
			// algebra op.
			else {
				int curOut = BrokerDictionary.getInstance()
						.getCurrentOutputPort(broker.getName());
			...
			// if there are child operators, than increment the no of input
			// ports by
			// the no of child ops.
			int curIn = BrokerDictionary.getInstance().getCurrentInputPort(
					broker.getName());
			BrokerDictionary.getInstance().setCurrentInputPort(
					broker.getName(), curIn + broker.getNoOfChildOps());
		}
