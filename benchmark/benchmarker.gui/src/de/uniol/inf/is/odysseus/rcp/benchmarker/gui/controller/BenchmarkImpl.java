...
import de.uniol.inf.is.odysseus.benchmarker.impl.AvgBenchmarkMemUsageListener;
import de.uniol.inf.is.odysseus.benchmarker.impl.Benchmark;
import de.uniol.inf.is.odysseus.benchmarker.impl.BenchmarkSink;
import de.uniol.inf.is.odysseus.benchmarker.impl.IBenchmarkResultFactory;
import de.uniol.inf.is.odysseus.benchmarker.impl.IntegrationPipe;
...
public class BenchmarkImpl implements IErrorEventListener, IBenchmark, IEventListener {
	...
	public Collection<IBenchmarkResult<ILatency>> runBenchmark() throws BenchmarkException {
		...
		try {
			...
			parameters.add(new ParameterTransformationConfiguration(trafoConfig));
			...
			for (Pair<String, String> query : getQueries()) {
				...
				if (parserId.equalsIgnoreCase(SCRIPT_PARSER)) {
					try {
						QueryTextParser.getInstance().parseAndExecute(queryString);
					} catch (QueryTextParseException e) {
						throw new BenchmarkException(e);
					}
				} else {
					executor.addQuery(queryString, parserId, user, parameters.toArray(new IQueryBuildSetting[0]));
				}
			}
			int i = 0;
	...
	private void createNexmarkSources() {
		...
		for (String s : q) {
			try {
				this.executor.addQuery(s, "CQL", user, new ParameterTransformationConfiguration(
