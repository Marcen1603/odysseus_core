...
private static int count = 0;
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		openBenchmark(HandlerUtil.getActiveWorkbenchWindow(event).getActivePage()
				, new BenchmarkParam(BenchmarkIdHolder.INSTANCE.generateNextId()));
		return null;
	}
	...
	private static IEditorPart openBenchmark(IWorkbenchPage page, BenchmarkParam benchmarkParam) {
		try {
			...
			return (BenchmarkEditorPart) page.openEditor(new BenchmarkEditorInput(benchmarkParam), BenchmarkEditorPart.ID);
