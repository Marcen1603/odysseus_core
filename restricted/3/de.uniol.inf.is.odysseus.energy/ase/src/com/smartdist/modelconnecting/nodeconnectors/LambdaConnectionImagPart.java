...
public class LambdaConnectionImagPart extends NodeConnector {
	private Node myNode = null;
	@SuppressWarnings("unused")
	private double myTrueValue = 0;
	private FourWireNetwork myNetwork = null;

	public LambdaConnectionImagPart(FourWireNetwork network, Node myNode) {
		this.myNode = myNode;
...
//		this.myTrueValue = tempNeutralCurrent.getImag();
//
	...
	public double getModelSensitivity(FourWireNetwork network,
		...
		if (element instanceof TuneableEightTerminalElement){
			if (network.isIncident(this.myNode, (TuneableEightTerminalElement)element)){
				...
				for (int i=0;i<4;i++){
					for (int p=0;p<4;p++){
						sensitivity += dY[4*here+p][4*here+i].getReal()*expansionPoint[4*myNodeNumber+i].getImag();
						sensitivity += dY[4*here+p][4*there+i].getReal()*expansionPoint[4*remoteNodeNumber+i].getImag();
						sensitivity += dY[4*here+p][4*here+i].getImag()*expansionPoint[4*myNodeNumber+i].getReal();
						sensitivity += dY[4*here+p][4*there+i].getImag()*expansionPoint[4*remoteNodeNumber+i].getReal();
					}
...
//					sensitivity += dY[4*here+FourWireNetwork.phaseN][4*there+i].getImag()*expansionPoint[4*remoteNodeNumber+i].getReal();
		...
		if (element instanceof TuneableFourTerminalElement){
			if (this.myNode.myFourTerminalElements.contains(element)){
				...
				System.out.println("StarConnectionImagPart doesn't support tuning of FourTerminalElements yet!");
			}
		}	
		return sensitivity;
	}
