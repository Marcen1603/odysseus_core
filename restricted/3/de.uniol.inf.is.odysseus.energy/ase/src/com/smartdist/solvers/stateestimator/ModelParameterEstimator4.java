...
public class ModelParameterEstimator4 {
	...
	public void tuneModel(FourWireNetwork network, TuneableEightTerminalElement[] tuneables, EstimationResultSet expansionPoint, Inputs inputs, double[] meanValues){
		...
		double[] oldTheoreticals = new double[numberOfConnectors];
		for (int j=0;j<numberOfConnectors;j++){
			oldTheoreticals[j] = inputs.getConnector(j).myTheroreticalValue(expansionPoint.voltages);
		}
		...
		for (int i=0;i<numberOfTuneables;i++){
			...
			for (int j=0;j<numberOfConnectors;j++){
				hSensitivity[j][i] = (inputs.getConnector(j).myTheroreticalValue(expansionPoint.voltages)-oldTheoreticals[j])/stepSize;//.myJacobianRow(expansionPoint.voltages);
			}
