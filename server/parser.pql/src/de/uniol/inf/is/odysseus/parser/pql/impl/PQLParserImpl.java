...
public class PQLParserImpl implements PQLParserImplConstants {
	...
	static private ILogicalOperator createOperator(String identifier, Map<String, Object> parameters,
			List<InputOperatorItem> inputOps) {
		IOperatorBuilder builder = OperatorBuilderFactory.createOperatorBuilder(identifier.toUpperCase(), getUser(),
				getDataDictionary(), getContext(), getServerExecutor());
	...
	static final public List<IExecutorCommand> query() throws ParseException {
				...
				if (!namedOpParameters.get(queryName).isEmpty()) {
					{
						if (true) {
							if (true) {
								if (true)
									throw new QueryParseException(
											"query parameters can only be defined for root operators, not for: "
													+ queryName);
							}
						}
					}
		...
		if (roots.size() > 0) {
			...
			// System.err.println(walker2.prefixWalk(topOperator, new
	...
	static final public void namedStream(Map<String, ILogicalOperator> namedOps,
			Map<String, Map<String, Object>> namedOpParameters, Map<String, IExecutorCommand> commands)
...
//		  op = rename;
	...
	static final public Object parameterValue() throws ParseException {
		...
		case CHAR_LITERAL:
			...
			value = t.image.substring(1, t.image.length() - 1).replace("\u005c\u005c\u005c'", "\u005c'")
					.replace("\u005c\u005c\u005c"", "\u005c"");
			break;
		case BOOLEAN:
	...
	static final public List<InputOperatorItem> operatorList(Map<String, ILogicalOperator> namedOps)
			throws ParseException {
	...
	static final public PredicateItem predicate() throws ParseException {
		...
		{
			if (true)
				return new PredicateItem(predicateType.image,
						predicate.image.substring(1, predicate.image.length() - 1));
		}
		throw new Error("Missing return statement in function");
	}
	...
	private static void jj_la1_init_0() {
		jj_la1_0 = new int[] { 0x40, 0x40000, 0xf00000, 0x2000000, 0x40000, 0x40, 0x2000000, 0xc220, 0x8000040,
				0x2000000, 0x800c260, 0x2000000, 0x800c260, 0x8000, 0x2000000, 0x8000, 0x8040, };
	}
