...
public class SpatialIsWithinDistance extends AbstractFunction<Boolean> {
	...
	public SpatialIsWithinDistance() {
		super("SpatialIsWithinDistance", 3, accTypes, SDFDatatype.BOOLEAN);
	}
	...
	public Boolean getValue() {
		...
		// ((Geometry)this.getInputValue(0)).toString() + " " +
		// ((Geometry)this.getInputValue(1)).toString() + " " +
		// ((Double)this.getInputValue(2)) );
		return ((Geometry) this.getInputValue(0)).isWithinDistance(
				(Geometry) this.getInputValue(1),
				(Double) this.getInputValue(2));
	}
