...
public class RSwitchSelectionUnionRule extends AbstractSwitchSelectionRule<UnionAO> {

	@Override
	public void execute(UnionAO union, RewriteConfiguration config) {
		for (SelectAO sel : getAllOfSameTyp(new SelectAO())) {
			if (isValidSelect(sel, union)) {
				...
				Collection<ILogicalOperator> toUpdate = RelationalRestructHelper.switchOperator(sel, union, toInsert);
