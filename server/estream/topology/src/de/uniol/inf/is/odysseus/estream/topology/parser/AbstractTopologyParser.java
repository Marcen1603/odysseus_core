...
/**
 * An abstract parser to parse JSON-Topology
 * 
 * @author Jens Plümer
 *
 */
public abstract class AbstractTopologyParser<T extends IStreamObject<? extends IMetaAttribute>> {

	// private static final Logger LOGGER =
	// LoggerFactory.getLogger(AbstractCOSEMParser.class);
	 ...
	 * 
	 * @param key
	 *            is the name of a schema attribute that corresponds to a field name
	 *            (e.g. a xml or json token)
	 * @param value
	 *            is the corresponding value for the given key
	 */
	...
	/**
	 * Returns the current string representation and should be called only if one
	 * smart meter was processed completely.
	 * 
	...
	/**
	 * Closes the input stream reader and signals that the parser reached the end of
	 * the stream.
	 */
