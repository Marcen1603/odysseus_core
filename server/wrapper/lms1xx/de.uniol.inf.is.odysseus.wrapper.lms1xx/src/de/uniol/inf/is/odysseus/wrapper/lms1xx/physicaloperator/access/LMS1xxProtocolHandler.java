 ...
 */
public class LMS1xxProtocolHandler extends LineProtocolHandler<KeyValueObject<? extends IMetaAttribute>> {
    /** The logger. */
    private static final Logger LOG = LoggerFactory.getLogger(LMS1xxProtocolHandler.class);
     ...
     * @param dataHandler
     */
    public LMS1xxProtocolHandler(final ITransportDirection direction, final IAccessPattern access, final IDataHandler<KeyValueObject<? extends IMetaAttribute>> dataHandler) {
    ...
    public IProtocolHandler<KeyValueObject<? extends IMetaAttribute>> createInstance(final ITransportDirection direction, final IAccessPattern access, final Map<String, String> options,
        ...
        final LMS1xxProtocolHandler instance = new LMS1xxProtocolHandler(direction, access, dataHandler);
        ...
        if (!(caller instanceof AbstractFileHandler)) {
            ...
            try {
                ...
                if (this.username.equalsIgnoreCase("maintainer")) {
                    this.send(LMS1xxConstants.SET_ACCESS_MODE_MAINTAINER_COMMAND.getBytes(this.charset));
                }
                else if (this.username.equalsIgnoreCase("client")) {
                    this.send(LMS1xxConstants.SET_ACCESS_MODE_CLIENT_COMMAND.getBytes(this.charset));
                }
                else if (this.username.equalsIgnoreCase("service")) {
                    this.send(LMS1xxConstants.SET_ACCESS_MODE_SERVICE_COMMAND.getBytes(this.charset));
                }
                Thread.sleep(1000);
                ...
                Thread.sleep(1000);
                this.send(LMS1xxConstants.STOP_SCAN_COMMAND.getBytes(this.charset));
                Thread.sleep(1000);
                this.send(LMS1xxConstants.START_SCAN_COMMAND.getBytes(this.charset));
            }
    ...
    public KeyValueObject<? extends IMetaAttribute> getNext() throws IOException {
        ...
        if (line.startsWith("2")) {
            ...
            this.buffer.put(encoder.encode(CharBuffer.wrap(line.substring(1, line.length() - 1))));
        ...
        catch (LMS1xxReadErrorException | LMS1xxLoginException | LMS1xxUnknownMessageException e) {
            LMS1xxProtocolHandler.LOG.error(e.getMessage(), e);
    ...
    protected void init(final Map<String, String> options) {
        ...
        if (options.containsKey(LMS1xxProtocolHandler.BYTEORDER)) {
            this.setByteOrder(options.get(LMS1xxProtocolHandler.BYTEORDER));
        }
        if (options.containsKey(LMS1xxProtocolHandler.USERNAME)) {
            this.setUsername(options.get(LMS1xxProtocolHandler.USERNAME));
        }
        if (options.containsKey(LMS1xxProtocolHandler.PASSWORD)) {
            this.setPassword(options.get(LMS1xxProtocolHandler.PASSWORD));
        }
    }
    ...
    private KeyValueObject<? extends IMetaAttribute> parse() throws LMS1xxReadErrorException, LMS1xxLoginException, LMS1xxUnknownMessageException {
    ...
    private KeyValueObject<? extends IMetaAttribute> parseLMS1xxScanData(final String[] data) {
        ...
        if (hasPosition == 1) {
            ...
            position[0][1] = Float.intBitsToFloat(((Long) Long.parseLong(data[pos++], 16)).intValue());
            position[1][1] = Float.intBitsToFloat(((Long) Long.parseLong(data[pos++], 16)).intValue());
            position[2][1] = Float.intBitsToFloat(((Long) Long.parseLong(data[pos++], 16)).intValue());
        ...
        event.put(LMS1xxProtocolHandler.TIMESTAMP_ATTRIBUTE, calendar.getTimeInMillis());
        event.put(LMS1xxProtocolHandler.VERSION_ATTRIBUTE, measurement.getVersion());
        event.put(LMS1xxProtocolHandler.DEVICE_ATTRIBUTE, measurement.getDevice());
        event.put(LMS1xxProtocolHandler.SERIAL_ATTRIBUTE, measurement.getSerial());

        event.put(LMS1xxProtocolHandler.STATUS_ATTRIBUTE, measurement.getStatus());
        event.put(LMS1xxProtocolHandler.MESSAGE_COUNT_ATTRIBUTE, measurement.getMessageCount());
        event.put(LMS1xxProtocolHandler.SCAN_COUNT_ATTRIBUTE, measurement.getScanCount());
        event.put(LMS1xxProtocolHandler.POWERUP_DURATION_ATTRIBUTE, measurement.getPowerUpDuration());
        event.put(LMS1xxProtocolHandler.TRANSMISSION_DURATION_ATTRIBUTE, measurement.getTransmissionDuration());
        event.put(LMS1xxProtocolHandler.INPUT_STATUS_ATTRIBUTE, measurement.isInputStatus());
        ...
        if (measurement.get8BitSamples() != null) {
            ...
            final double[][] distance8Bit2 = new double[measurement.get8BitSamples().length][2];
            final double[] remission8Bit2 = new double[measurement.get8BitSamples().length];
            ...
            event.put(LMS1xxProtocolHandler.DISTANCE_8BIT_ATTRIBUTE, distance8Bit);
            event.put(LMS1xxProtocolHandler.REMISSION_8BIT_ATTRIBUTE, remission8Bit);
            event.put(LMS1xxProtocolHandler.DISTANCE2_8BIT_ATTRIBUTE, distance8Bit2);
            event.put(LMS1xxProtocolHandler.REMISSION2_8BIT_ATTRIBUTE, remission8Bit2);
        }
        if (measurement.get16BitSamples() != null) {
            final double[][] distance16Bit = new double[measurement.get16BitSamples().length][2];
            final double[] remission16Bit = new double[measurement.get16BitSamples().length];
            final double[][] distance16Bit2 = new double[measurement.get16BitSamples().length][2];
            final double[] remission16Bit2 = new double[measurement.get16BitSamples().length];
            ...
            event.put(LMS1xxProtocolHandler.DISTANCE_16BIT_ATTRIBUTE, distance16Bit);
            event.put(LMS1xxProtocolHandler.REMISSION_16BIT_ATTRIBUTE, remission16Bit);
            event.put(LMS1xxProtocolHandler.DISTANCE2_16BIT_ATTRIBUTE, distance16Bit2);
            event.put(LMS1xxProtocolHandler.REMISSION2_16BIT_ATTRIBUTE, remission16Bit2);
        }
    ...
    private void send(final byte[] message) throws IOException {
        ...
        LMS1xxProtocolHandler.LOG.debug("SICK: Send message {}", decoder.decode(ByteBuffer.wrap(message)));
    }

    private void checkOverflow(final int size) {
        if ((size + this.buffer.position()) >= this.buffer.capacity()) {
            final ByteBuffer newBuffer = ByteBuffer.allocate((this.buffer.limit() + size + this.buffer.position()) * 2);
