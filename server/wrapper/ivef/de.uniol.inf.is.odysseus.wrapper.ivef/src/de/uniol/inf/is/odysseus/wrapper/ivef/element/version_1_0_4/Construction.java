...
import de.uniol.inf.is.odysseus.wrapper.ivef.element.version_1_0_4.UnType;

/**
 * @author msalous
 *
 */
public class Construction implements IIvefElement { 
	...
	public void fillMap(KeyValueObject<? extends IMetaAttribute> map) {
		if(m_unTypePresent)
			m_unType.fillMap(map);
		if(m_hullColorPresent)
			map.addAttributeValue("construction_hullColor", m_hullColor);
		if(m_hullTypePresent)
			map.addAttributeValue("construction_hullType", m_hullType);
		if(m_deadWeightPresent)
			map.addAttributeValue("construction_deadWeight", m_deadWeight);
		if(m_grossWeightPresent)
			map.addAttributeValue("construction_grossWeight", m_grossWeight);
		if(m_lengthPresent)
			map.addAttributeValue("construction_length", m_length);
		if(m_lloydsShipTypePresent)
			map.addAttributeValue("construction_lloydsShipType", m_lloydsShipType);
		if(m_yearOfBuildPresent)
			map.addAttributeValue("construction_yearOfBuild", m_yearOfBuild);
		if(m_maxAirDraughtPresent)
			map.addAttributeValue("construction_maxAirDraught", m_maxAirDraught);
		if(m_maxDraughtPresent)
			map.addAttributeValue("construction_maxDraught", m_maxDraught);
		if(m_maxPersonsOnBoardPresent)
			map.addAttributeValue("construction_maxPersonsOnBoard", m_maxPersonsOnBoard);
		if(m_maxSpeedPresent)
			map.addAttributeValue("construction_maxSpeed", m_maxSpeed);
		if(m_widthPresent)
			map.addAttributeValue("construction_width", m_width);
	}
}