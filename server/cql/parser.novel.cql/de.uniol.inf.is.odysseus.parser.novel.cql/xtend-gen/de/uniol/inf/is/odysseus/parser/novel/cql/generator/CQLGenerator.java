...
public class CQLGenerator implements IGenerator2 {
   ...
   */
  private Map<SimpleSelect, Map<String, List<String>>> queryAttributes = CollectionLiterals.<SimpleSelect, Map<String, List<String>>>newHashMap();
  
  /**
   * Contains all selected attributes for each registered query
   */
  private Map<SimpleSelect, List<String>> projectionAttributes = CollectionLiterals.<SimpleSelect, List<String>>newHashMap();
  ...
  private List<String> predicateStringList = CollectionLiterals.<String>newArrayList();
  ...
  private boolean prepareParsingSelect(final SimpleSelect select) {
  ...
  private CharSequence parseSelectWithoutPredicate(final SimpleSelect select) {
  ...
  private CharSequence parseSelectWithPredicate(final SimpleSelect stmt) {
    ...
    this.predicateStringList.clear();
    this.parsePredicate(predicates);
    String selectInput = this.buildInput2(stmt, operator1, operator2).toString();
    String predicate = this.parsePredicateString(this.predicateStringList);
    String select = "";
  ...
  private CharSequence buildPredicateString(final CharSequence sequence) {
   ...
   */
  private CharSequence parsePredicate(final Expression e) {
    ...
    if (_not) {
      ...
      if (e instanceof OrPredicate) {
        _matched=true;
        this.parsePredicate(((OrPredicate)e).getLeft());
        this.buildPredicateString("||");
        this.parsePredicate(((OrPredicate)e).getRight());
      }
      if (!_matched) {
        if (e instanceof AndPredicate) {
          _matched=true;
          this.parsePredicate(((AndPredicate)e).getLeft());
          this.buildPredicateString("&&");
          this.parsePredicate(((AndPredicate)e).getRight());
        }
      }
      if (!_matched) {
        if (e instanceof Equality) {
          _matched=true;
          this.parsePredicate(((Equality)e).getLeft());
          ...
          this.parsePredicate(((Equality)e).getRight());
        }
      }
      if (!_matched) {
        if (e instanceof Comparision) {
          _matched=true;
          this.parsePredicate(((Comparision)e).getLeft());
          this.buildPredicateString(((Comparision)e).getOp());
          this.parsePredicate(((Comparision)e).getRight());
        }
      }
      if (!_matched) {
        if (e instanceof Plus) {
          _matched=true;
          this.parsePredicate(((Plus)e).getLeft());
          this.buildPredicateString("+");
          this.parsePredicate(((Plus)e).getRight());
        }
      }
      if (!_matched) {
        if (e instanceof Minus) {
          _matched=true;
          this.parsePredicate(((Minus)e).getLeft());
          this.buildPredicateString("-");
          this.parsePredicate(((Minus)e).getRight());
        }
      }
      if (!_matched) {
        if (e instanceof MulOrDiv) {
          _matched=true;
          this.parsePredicate(((MulOrDiv)e).getLeft());
          this.buildPredicateString(((MulOrDiv)e).getOp());
          this.parsePredicate(((MulOrDiv)e).getRight());
        }
      }
      if (!_matched) {
        if (e instanceof NOT) {
          _matched=true;
          this.buildPredicateString("!");
          this.parsePredicate(((NOT)e).getExpression());
        }
      }
      if (!_matched) {
        if (e instanceof Bracket) {
          _matched=true;
          this.buildPredicateString("(");
          this.parsePredicate(((Bracket)e).getInner());
          this.buildPredicateString(")");
        }
      }
      if (!_matched) {
        if (e instanceof AttributeRef) {
          ...
          this.buildPredicateString(this.getAttributename(((Attribute) _value)));
        }
      }
      if (!_matched) {
        if (e instanceof ComplexPredicateRef) {
          ...
          if (((quantification = complexPredicate.getQuantification()) != null)) {
            ...
            this.prepareParsingSelect(select);
            String predicate = "";
            ExpressionsModel _predicates = select.getPredicates();
            boolean _tripleEquals = (_predicates == null);
            if (_tripleEquals) {
              this.parseSelectWithoutPredicate(select);
              List<String> _get = this.projectionAttributes.get(select);
              ...
              this.parseSelectWithPredicate(select);
              List<String> _get_1 = this.projectionAttributes.get(select);
            ...
            args.put("type", type);
            args.put("input", this.getLastOperator());
            Set<Map.Entry<String, List<String>>> _entrySet = this.queryAttributes.get(select).entrySet();
            ...
            this.registry_existenceOperators.add(args);
            ...
            if (((exists = complexPredicate.getExists()) != null)) {
              ...
              if (((in = complexPredicate.getIn()) != null)) {
                ...
                this.prepareParsingSelect(select_1);
                String predicate_1 = "";
                ExpressionsModel _predicates_1 = select_1.getPredicates();
                boolean _tripleEquals_1 = (_predicates_1 == null);
                if (_tripleEquals_1) {
                  this.parseSelectWithoutPredicate(select_1);
                  InputOutput.<String>println(this.projectionAttributes.get(select_1).toString());
                  List<String> _get_2 = this.projectionAttributes.get(select_1);
                  ...
                  this.parseSelectWithPredicate(select_1);
                  InputOutput.<String>println(this.projectionAttributes.get(select_1).toString());
                  List<String> _get_3 = this.projectionAttributes.get(select_1);
                ...
                Set<Map.Entry<String, List<String>>> _entrySet_1 = this.queryAttributes.get(select_1).entrySet();
                ...
                args_1.put("input", this.getLastOperator());
                args_1.put("predicate", predicate_1);
                this.registry_existenceOperators.add(args_1);
    ...
    InputOutput.<String>println(this.predicateString);
  ...
  private List<String> parseComplexPredicate(final ComplexPredicate complexPredicate, final String type) {
    List<String> _xblockexpression = null;
    {
      ...
      this.prepareParsingSelect(subQuery);
      this.parseSelectWithoutPredicate(subQuery);
      this.parsePredicate(complexPredicate.getSelect().getSelect().getPredicates().getElements().get(0));
      InputOutput.<String>println(("exists() -> " + this.predicateString));
      Set<Map.Entry<String, List<String>>> _entrySet = this.queryAttributes.get(subQuery).entrySet();
      for (final Map.Entry<String, List<String>> l : _entrySet) {
  ...
  private CharSequence parsePredicate(final List<Expression> expressions) {
    InputOutput.<String>println("parsePredicate(List<Expressions>)");
    this.predicateString = "";
    for (int i = 0; (i < (expressions.size() - 1)); i++) {
      Expression _get = expressions.get(i);
      if ((_get instanceof AttributeRef)) {
      } else {
        this.predicateString = this.parsePredicate(expressions.get(i)).toString();
        this.buildPredicateString("&&");
        InputOutput.<String>println(("parsePredicate()x -> " + this.predicateString));
      }
    }
    int _size = expressions.size();
    int _minus = (_size - 1);
    this.predicateString = this.parsePredicate(expressions.get(_minus)).toString();
    InputOutput.<String>println(("parsePredicate() -> " + this.predicateString));
    return this.predicateString;
  }
  ...
  private String buildInput2(final SimpleSelect select, final String... operators) {
    if ((operators != null)) {
      ...
      if (((mapOperator != null) && (aggregateOperator != null))) {
        ...
        return this.checkForGroupAttributes(aggregateOperator, select, this.buildJoin(new String[] { aggregateOperator, _buildJoin }));
      } else {
        if ((mapOperator != null)) {
          ...
          if ((aggregateOperator != null)) {
            ...
            if (_containsAll) {
              ...
              if ((((predicateAttributes != null) && (!predicateAttributes.isEmpty())) && this.registry_AggregationAttributes.containsAll(predicateAttributes))) {
                return aggregateOperator;
              } else {
                String _buildJoin_1 = this.buildJoin(select.getSources());
                return this.checkForGroupAttributes(aggregateOperator, select, this.buildJoin(new String[] { aggregateOperator, _buildJoin_1 }));
              }
            } else {
              InputOutput.<String>println("buildInput() -> aggregate : !(contains all project attributes)");
              String _buildJoin_2 = this.buildJoin(select.getSources());
              return this.checkForGroupAttributes(aggregateOperator, select, this.buildJoin(new String[] { aggregateOperator, _buildJoin_2 }));
            }
          }
        }
      }
   ...
   * 		{
   * 			var List<String> predicateAttributes = newArrayList
   * 			if(select.predicates !== null)
   * 				predicateAttributes = EcoreUtil2.getAllContentsOfType(select.predicates, Attribute).stream.map(e|e.name).collect(Collectors.toList)
   * 			println(predicateAttributes)
   ...
   * 			{
   * 				if(!predicateAttributes.empty && registry_Aggregations.equals(predicateAttributes))
   * 					input = aggregate
   * 				else
   * 					input = buildJoin(#[aggregate, buildJoin(sources).toString]).toString//rename if group by attribute
   * 			}
   * 			else
   * 			{
   * 				input = buildJoin(sources).toString//buildJoin(sources.stream.filter(e|e instanceof SimpleSource).map(e|(e as SimpleSource).name).collect(Collectors.toList)).toString
  ...
  public String getAttributenameFromAlias(final String alias) {
    ...
    if ((this.registry_AggregationAttributes.contains(alias) || this.registry_Expressions.keySet().contains(alias))) {
      return alias;
    }
    return null;
  }
