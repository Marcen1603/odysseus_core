...
class AttributeNameParser implements IAttributeNameParser {
	...
	@Inject
	new(IUtilityService utilityService, ICacheService cacheService) {
...
//				}
			...
			if (containedBySources.size == 1) {
				var aliases = utilityService.getSource(containedBySources.get(0).getName()).findByName(attribute).getAliases//getAliasFromAttributename(attribute, containedBySources.get(0).getName)
				if (!aliases.empty) {
