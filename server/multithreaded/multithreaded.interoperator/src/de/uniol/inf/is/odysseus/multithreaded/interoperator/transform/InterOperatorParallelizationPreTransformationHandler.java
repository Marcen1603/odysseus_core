			...
			if (handlerParameters.size() != PARAMETER_COUNT) {
				...
				for (Pair<String, String> pair : handlerParameters) {
					...
					case GLOBAL_DEGREE_OF_PARALLELIZATION:
						try {
							globalDegreeOfParallelization = Integer
									.parseInt(pair.getE2());
				...
				if (logicalPlan instanceof TopAO) {
					...
					if (validTypes != null) {
						...
						for (ILogicalOperator operatorForTransformation : collVisitor
								.getResult()) {
							// operator has no custom settings or no settings
							// are
							// available, then choose best strategy and
							// transform
							if (!operatorIds.isEmpty()
								...
								// operators should be transformed. If then the
								// id
								// of the given operator is null or not in the
								// set
