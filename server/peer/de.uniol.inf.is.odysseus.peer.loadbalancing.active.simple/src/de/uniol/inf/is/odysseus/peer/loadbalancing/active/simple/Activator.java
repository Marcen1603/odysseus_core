...
/**
 * The {@link BundleActivator} for the bundle <code>de.uniol.inf.is.odysseus.peer.loadbalancing.active.simple</code>.
...
public class Activator implements BundleActivator {
	...
	/**
	 * The {@link ISessionManagement}.
	 */
	private static ISessionManagement cSessionManagement;
	 ...
	 * Called by OSGI-DS.
	 * @param executor The implementation of {@link IExecutor} to be bound.
	 */
	public static void bindExecutor(IExecutor executor) {
		
		Preconditions.checkNotNull("The executor to be bound must be not null!");
		
		Activator.cExecutor = executor;
		LOG.debug("Bound {} as an implementation of {}", executor.getClass().getSimpleName(), IExecutor.class.getSimpleName());
	 ...
	 * Called by OSGI-DS.
	 * @param executor The implementation of {@link IExecutor} to be unbound.
	 */
	public static void unbindExecutor(IExecutor executor) {
		...
		if(executor != null && executor.equals(Activator.cExecutor)) {
			...
			LOG.debug("Unbound {} as an implementation of {}", executor.getClass().getSimpleName(), IExecutor.class.getSimpleName());
	...
	/**
	 * Gets the session.
	 * @return The {@link ISession}, if an {@link ISessionManagement} is bound.
	 */
	public static Optional<ISession> getSession() {
		
		if(Activator.cSessionManagement == null) {
			
			return Optional.absent();
			
		} else if(Activator.cSession == null || !Activator.cSession.isValid()) {
			
			Activator.cSession = Activator.cSessionManagement.loginSuperUser(null, UserManagementProvider.getDefaultTenant().getName());
			return Optional.of(Activator.cSession);
	...
	/**
	 * Binds a session management. <br />
	 * Called by OSGI-DS.
	 * @param management The implementation of {@link ISessionManagement} to be bound.
	 */
	public static void bindSessionManagement(ISessionManagement management) {
		
		Preconditions.checkNotNull("The session management to be bound must be not null!");
		
		Activator.cSessionManagement = management;
		LOG.debug("Bound {} as an implementation of {}", management.getClass().getSimpleName(), ISessionManagement.class.getSimpleName());
		
	}
	
	/**
	 * Removes the binding for a session management. <br />
	 * Called by OSGI-DS.
	 * @param management The implementation of {@link ISessionManagement} to be unbound.
	 */
	public static void unbindSessionManagement(ISessionManagement management) {
		
		if(management != null && management.equals(Activator.cSessionManagement)) {
			
			Activator.cSessionManagement = null;
			LOG.debug("Unbound {} as an implementation of {}", management.getClass().getSimpleName(), ISessionManagement.class.getSimpleName());
	 ...
	 * Called by OSGI-DS.
	 * @param manager The implementation of {@link IPeerResourceUsageManager} to be bound.
	 */
	public static void bindResourceManager(IPeerResourceUsageManager manager) {
		
		Preconditions.checkNotNull("The resource manager to be bound must be not null!");
		
		Activator.cResourceManager = manager;
		LOG.debug("Bound {} as an implementation of {}", manager.getClass().getSimpleName(), IPeerResourceUsageManager.class.getSimpleName());
	 ...
	 * Called by OSGI-DS.
	 * @param manager The implementation of {@link IPeerResourceUsageManager} to be unbound.
	 */
	public static void unbindResourceManager(IPeerResourceUsageManager manager) {
		...
		if(manager != null && manager.equals(Activator.cResourceManager)) {
			...
			LOG.debug("Unbound {} as an implementation of {}", manager.getClass().getSimpleName(), IPeerResourceUsageManager.class.getSimpleName());
	 ...
	 * Called by OSGI-DS.
	 * @param manager The implementation of {@link IP2PNetworkManager} to be bound.
	 */
	public static void bindNetworkManager(IP2PNetworkManager manager) {
		
		Preconditions.checkNotNull("The network manager to be bound must be not null!");
		
		Activator.cNetworkmanager = manager;
		LOG.debug("Bound {} as an implementation of {}", manager.getClass().getSimpleName(), IP2PNetworkManager.class.getSimpleName());
	 ...
	 * Called by OSGI-DS.
	 * @param manager The implementation of {@link IP2PNetworkManager} to be unbound.
	 */
	public static void unbindNetworkManager(IP2PNetworkManager manager) {
		...
		if(manager != null && manager.equals(Activator.cNetworkmanager)) {
			...
			LOG.debug("Unbound {} as an implementation of {}", manager.getClass().getSimpleName(), IP2PNetworkManager.class.getSimpleName());
