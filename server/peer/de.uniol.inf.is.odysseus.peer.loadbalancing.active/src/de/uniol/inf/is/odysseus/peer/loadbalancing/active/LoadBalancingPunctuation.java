...
/**
 * A {@link LoadBalancingPunctuation} will be send to mark, that the current stream is part of an active load balancing process. <br />
 * This means, that there are more than one stream in parallel, one being the original and the others being in a synchronize process with 
 * the first one. <br />
 * A {@link LoadBalancingPunctuation} can either mark the start or the end of a load balancing process. 
 * None of those markers must be set for all punctuations.
 * @author Michael
 *
 */
public class LoadBalancingPunctuation extends AbstractPunctuation {
	...
	/**
	 * Creates a new {@link LoadBalancingPunctuation} marking neither start nor stop point of load balancing.
	 * @param time The current time.
	 */
	...
	/**
	 * Creates a new {@link LoadBalancingPunctuation} as a copy of an existing one.
	 * @param punctuation The punctuation to copy.
	 */
	 ...
	 * @return True, if the punctuation marks the start of the load balancing process.
	 */
	 ...
	 * @return True, if the punctuation marks the stop of the load balancing process.
	 */
	...
	public String toString() {
		...
		StringBuffer strBuffer = new StringBuffer(this.getClass().getSimpleName() + " (" + hashCode() + ") | " + this.getTime());
		if(this.start)
