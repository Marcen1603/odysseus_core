...
public class MinCPULoadBalancer implements ILoadBalancer {

	private static final Logger LOG = LoggerFactory.getLogger(MinCPULoadBalancer.Usage.class);
	...
	public Map<ILogicalQueryPart, PeerID> balance(Map<PeerID, IResourceUsage> currentResourceUsageMap, Map<ILogicalQueryPart, OperatorCost<?>> partCosts) {
		...
		for (ILogicalQueryPart queryPart : partCosts.keySet()) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Allocated to {} with mem={} and cpu={}", new Object[] { optPeerName.isPresent() ? optPeerName.get() : "<unknown>", minUsage.mem, minUsage.cpu });
			}

			result.put(queryPart, minPeerID);

			minUsage.mem += partCosts.get(queryPart).getMemCost();
			minUsage.cpu += partCosts.get(queryPart).getCpuCost();
			
			if (LOG.isDebugEnabled()) {
				LOG.debug("Set usage of {} to mem={} and cpu={}", new Object[] { optPeerName.isPresent() ? optPeerName.get() : "<unknown>", minUsage.mem, minUsage.cpu });
			}
		}
