...
public class CostSummary {
	...
	public CostSummary(String operatorId, double cpuCost, double memCost, ILogicalOperator plan) {
	...
	public String toString() {
		return Objects.toStringHelper(this).addValue(this.operatorId).addValue(this.cpuCost).addValue(this.memCost).toString();
	}
	...
	public static CostSummary calcSum(Collection<CostSummary> costs) {
		...
		for (CostSummary cost : costs) {
			cpuCosts += cost.getCpuCost() * 100; // um fehler bei floatingpoints
													// zu vermeiden (ziemlich
													// dumm diese loesung)
