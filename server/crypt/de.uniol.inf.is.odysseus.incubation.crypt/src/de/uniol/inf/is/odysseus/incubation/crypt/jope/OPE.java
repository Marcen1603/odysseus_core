package de.uniol.inf.is.odysseus.incubation.crypt.jope;
...
/**
 * @author https://github.com/ssavvides/jope (17.10.16), Savvas Savvides
 *
 */
public class OPE {
	...
	public OPE(Key key) {
		...
		// this.inRange = new ValueRange(BigInteger.ZERO, new
		// BigInteger("100"));
		// this.outRange = new ValueRange(BigInteger.ZERO, new
		// BigInteger("200"));
	}
	...
	private BigInteger encryptRecursive(BigInteger ptxt, ValueRange inRange, ValueRange outRange) {
		...
		BigInteger m = new BigDecimal(outSize).divide(two, PRECISION, RoundingMode.CEILING).toBigInteger();
	...
	/**
	 * Uniformly select a number from the range using the bit list as a source
	 * of randomness
	 *
	 * @param outRange
	 * @param coins
	...
	private static BigInteger sampleHGD(ValueRange inRange, ValueRange outRange, BigInteger nSample, Coins coins) {
	...
	public static Long crypt(Long number, int mode, Key key) {
		BigInteger bigInt = BigInteger.valueOf(number.longValue());
		OPE ope = new OPE(key);

		if (mode == Cipher.ENCRYPT_MODE && bigInt != null) {
			bigInt = ope.encrypt(bigInt);
		} else if (mode == Cipher.DECRYPT_MODE) {
			bigInt = ope.decrypt(bigInt);
		}
