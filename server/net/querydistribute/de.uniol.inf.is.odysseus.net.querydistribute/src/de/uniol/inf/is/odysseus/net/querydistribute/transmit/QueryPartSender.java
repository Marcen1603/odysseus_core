...
public class QueryPartSender implements IOdysseusNodeCommunicatorListener {
	...
	public UUID transmit(Map<ILogicalQueryPart, IOdysseusNode> allocationMap, IServerExecutor serverExecutor, ISession caller, QueryBuildConfiguration config) throws QueryPartTransmissionException, QueryDistributionException {
		...
		try {
			callExecutorToAddLocalQueries(localQuery, sharedQueryID, serverExecutor, caller, config, determineSlavePeers(allocationMap, localNode));
		} catch (Throwable t) {
	...
	private static Collection<ILogicalQueryPart> determineLocalQueryParts(Map<ILogicalQueryPart, IOdysseusNode> allocationMap, IOdysseusNode localNode) {
	...
	private void distributeToRemotePeers(UUID sharedQueryID, IServerExecutor serverExecutor, ISession caller, Map<ILogicalQueryPart, IOdysseusNode> allocationMap, IOdysseusNode localNode, QueryBuildConfiguration parameters) throws QueryPartTransmissionException {
		...
		if (LOG.isDebugEnabled()) {
			...
			for (Integer qpid : sendResultMap.keySet()) {
				LOG.debug("\tQueryPartID {} (Peer {}) : {}", new Object[] { qpid, sendDestinationMap.get(qpid), sendResultMap.get(qpid) });
			}
		}
	...
	private static Collection<IOdysseusNode> determineSlavePeers(Map<ILogicalQueryPart, IOdysseusNode> allocationMap, IOdysseusNode localNode) {
	...
	private static void callExecutorToAddLocalQueries(ILogicalQuery query, UUID sharedQueryID, IServerExecutor serverExecutor, ISession caller, QueryBuildConfiguration config, Collection<IOdysseusNode> slaveNodes) throws QueryDistributionException {
		try {
			...
			Collection<Integer> queryIDs = serverExecutor.addQuery(scriptText, "OdysseusScript", QueryDistributionPlugIn.getActiveSession(), "Standard", Context.empty(), configuration);

			QueryPartController.getInstance().registerAsMaster(query, queryIDs.iterator().next(), sharedQueryID, slaveNodes);
