...
public class ContinuousDistributionPredicateEvaluator implements IPredicateEvaluator {
    ...
    public ProbabilisticTuple<? extends IProbabilistic> evaluate(ProbabilisticTuple<? extends IProbabilistic> input) {
        ...
        // if
        // (expression.getType().equals(SDFProbabilisticDatatype.PROBABILISTIC_DOUBLE))
        // {
        // final MultivariateMixtureDistribution newDistribution =
        // (MultivariateMixtureDistribution) expr;
        // int index = ((ProbabilisticDouble)
        // output.getAttribute(newDistribution.getAttribute(0))).getDistribution();
        // final MultivariateMixtureDistribution oldDistribution =
        // output.getDistribution(index);
        //
        // for (int i = 0; i < newDistribution.getSupport().length; i++) {
        // double newMu = 0.0;
        // double oldMu = 0.0;
        // for (Pair<Double, IMultivariateDistribution> component :
        // ((MultivariateMixtureDistribution) newDistribution).getComponents())
        // {
        // newMu += component.getKey() * component.getValue().getMean()[i];
        // }
        // for (Pair<Double, IMultivariateDistribution> component :
        // ((MultivariateMixtureDistribution) oldDistribution).getComponents())
        // {
        // oldMu += component.getKey() * component.getValue().getMean()[i];
        // }
        // oldDistribution.setSupport(i,
        // newDistribution.getSupport()[i].subtract(newMu - oldMu));
        //
        // }
        // jointProbability *= oldDistribution.getScale() /
        // newDistribution.getScale();
