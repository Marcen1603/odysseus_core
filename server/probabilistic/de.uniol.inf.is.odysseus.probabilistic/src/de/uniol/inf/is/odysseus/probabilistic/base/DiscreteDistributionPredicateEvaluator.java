...
public class DiscreteDistributionPredicateEvaluator implements IPredicateEvaluator {
    ...
    public ProbabilisticTuple<? extends IProbabilistic> evaluate(ProbabilisticTuple<? extends IProbabilistic> input) {
        ...
        // final double[] inSum = new
        // double[this.probabilisticAttributePos.length];
        // final double[] outSum = new
        // double[this.probabilisticAttributePos.length];
        //
        // for (int i = 0; i < this.probabilisticAttributePos.length; i++) {
        // ((AbstractProbabilisticValue<?>)
        // output.getAttribute(this.probabilisticAttributePos[i])).getValues().clear();
        // final AbstractProbabilisticValue<?> attribute =
        // (AbstractProbabilisticValue<?>)
        // input.getAttribute(this.probabilisticAttributePos[i]);
        // for (final Double probability : attribute.getValues().values()) {
        // inSum[i] += probability;
        // }
        // }
        //
        // final Object[][] worlds = ProbabilisticDiscreteUtils.getWorlds(input,
        // this.probabilisticAttributePos);
        //
        // // Evaluate each world and store the possible ones in the output
        // tuple
        ...
        // throw new
        // IllegalArgumentException("More than one distribution not supported inside predicate");
        // }
        ...
        // output.getAttribute(this.probabilisticAttributePos[i]);
        ...
        // output.getMetadata()).getExistence();
        // for (int i = 0; i < this.probabilisticAttributePos.length; i++) {
