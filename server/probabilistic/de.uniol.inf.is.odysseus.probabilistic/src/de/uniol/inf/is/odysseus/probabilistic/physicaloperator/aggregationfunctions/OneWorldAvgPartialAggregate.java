...
public class OneWorldAvgPartialAggregate<T> extends AbstractPartialAggregate<T> {
    ...
    private boolean checkCorollary24() {
        ...
        if (this.pBig[0] >= ((4 / this.epsilon) * Math.log(2 / this.epsilon))) {
            result = (((2 * Math.log(this.pBig[0])) / this.pBig[0]) <= ((2 * (Math.log(2) + Math.log(2 / this.epsilon) + Math.log(Math.log(2 / this.epsilon)))) / ((4 / this.epsilon) * Math
                    .log(2 / this.epsilon))))
    ...
    private boolean checkTheorem23(final double rho, final double mean) {
        ...
        if (this.pBig[0] >= Math.E) {
            ...
            if (!result) {
                OneWorldAvgPartialAggregate.LOG.error("Error at: " + (((rho * this.aBig[0]) / this.pBig[0]) * (1 - ((2 * Math.log(this.pBig[0])) / this.pBig[0]))) + " <= " + mean + " " + mean
