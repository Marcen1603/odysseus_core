...
public class TProbabilisticContinuousEquiJoinAOInsertLinearRegressionRule extends AbstractTransformationRule<JoinAO> {
    ...
    private void insertSampleAO(final ILogicalOperator operator, final SDFExpression expr) {
        ...
        if (leftStreamAttributes.size() > rightStreamAttributes.size()) {
            sampleAO.setAttributes(new ArrayList<SDFAttribute>(rightStreamAttributes));
            port = 1;
        }
        else {
            sampleAO.setAttributes(new ArrayList<SDFAttribute>(leftStreamAttributes));
            port = 0;
        }

        sampleAO.setName(operator.getName() + "_sample");

        RestructHelper.insertOperatorBefore(sampleAO, operator.getSubscribedToSource(port).getTarget());
        sampleAO.initialize();
        operator.getSubscribedToSource(port).setSchema(sampleAO.getOutputSchema());
