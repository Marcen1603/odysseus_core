            ...
            if (message.startsWith(SickConnectionImpl.SRA)) {
                ...
                if (SickConnectionImpl.LMD_SCANDATA.equalsIgnoreCase(data[1])) {
                    ...
                    if (data.length >= 6) {
                        ...
                        measurement.setStatus(Integer.parseInt(data[pos + 5]),
                                Integer.parseInt(data[pos + 5]));
                        ...
                        measurement.setMeasurementFrequency(Long.parseLong(data[pos + 17]) * 10);

                        measurement.setEncoders(Integer.parseInt(data[pos + 18], 16));
                        if (measurement.getEncoders() > 0) {
                            measurement.setEncoderPosition(Long.parseLong(data[pos + 19]));
                            ...
                            if ((name.equalsIgnoreCase(SickConnectionImpl.DIST1))
                                ...
                                for (int j = 0; j < samples; j++) {
                                    if (measurement.getSamples()[j] == null) {
                                        ...
                                        measurement.getSamples()[j].setAngle((j * angularStepWidth)
                                                + startingAngle);
                                    }
                                    try {
                                        final float value = Integer.parseInt(data[index + 6 + j],
                                                16) * scalingFactor + scalingOffset;
                                        if (name.equalsIgnoreCase(SickConnectionImpl.DIST1)) {
                                            measurement.getSamples()[j].setDist1(this
                                                    .substractBackground(j,
                                                            value <= 3 ? Float.MAX_VALUE : value));

                                        }
                                        else if (name.equalsIgnoreCase(SickConnectionImpl.DIST2)) {
                                            measurement.getSamples()[j].setDist2(this
                                                    .substractBackground(j,
                                                            value <= 3 ? Float.MAX_VALUE : value));
                                        }
                                        else if (name.equalsIgnoreCase(SickConnectionImpl.RSSI1)) {
                                            measurement.getSamples()[j].setRssi1(value);
                                        }
                                        else if (name.equalsIgnoreCase(SickConnectionImpl.RSSI2)) {
                                ...
                                SickConnectionImpl.LOG.warn(String.format("Error in package: %s",
                                        message));
                ...
                while (!Thread.currentThread().isInterrupted()) {
                    ...
                    while ((this.channel.read(buffer)) > 0) {
                        for (int i = 0; i < buffer.remaining(); i++) {
                            if (buffer.get(i) == SickConnectionImpl.START) {
                                beginIndex = i;
                            }
                            else if (buffer.get(i) == SickConnectionImpl.END) {
                                endIndex = i;
                                break;
                            }
                        }
                        ...
                        if ((beginIndex >= 0) && (endIndex >= 0) && (beginIndex < endIndex)) {
                            buffer.position(beginIndex + 1);
                            final CharBuffer charBuffer = decoder.decode(buffer);
                            message = charBuffer.subSequence(0, endIndex - 1).toString();
                            try {
