...
public class SICKDataHandler extends AbstractDataHandler<Tuple<?>> {
    private final static Logger   LOG                             = LoggerFactory.getLogger(SICKDataHandler.class);
            ...
            if (SICKConstants.LMD_SCANDATA.equalsIgnoreCase(data[1])) {
                if (data.length >= 19) {
                    ...
                    try {
                            ...
                            if ((name.equalsIgnoreCase(SICKConstants.DIST1))
                                ...
                                for (int j = 0; j < samples; j++) {
                                    if (measurement.getSamples()[j] == null) {
                                        ...
                                        measurement.getSamples()[j].setAngle((j * angularStepWidth) + startingAngle);
                                    }
                                    try {
                                        final float value = Integer.parseInt(data[pos++], 16) * scalingFactor
                                                + scalingOffset;
                                        if (name.equalsIgnoreCase(SICKConstants.DIST1)) {
                                            measurement.getSamples()[j].setDist1(value);

                                        }
                                        else if (name.equalsIgnoreCase(SICKConstants.DIST2)) {
                                            measurement.getSamples()[j].setDist2(value);
                                        }
                                        else if (name.equalsIgnoreCase(SICKConstants.RSSI1)) {
                                            measurement.getSamples()[j].setRssi1(value);
                                        }
                                        else if (name.equalsIgnoreCase(SICKConstants.RSSI2)) {
                        ...
                        if (hasPosition == 1) {
                            @SuppressWarnings("unused")
                            float xPosition = Float.intBitsToFloat(((Long) Long.parseLong(data[pos++], 16)).intValue());
                            @SuppressWarnings("unused")
                            float yPosition = Float.intBitsToFloat(((Long) Long.parseLong(data[pos++], 16)).intValue());
                            @SuppressWarnings("unused")
                            float zPosition = Float.intBitsToFloat(((Long) Long.parseLong(data[pos++], 16)).intValue());
                            @SuppressWarnings("unused")
                            float xRotation = Float.intBitsToFloat(((Long) Long.parseLong(data[pos++], 16)).intValue());
                            @SuppressWarnings("unused")
                            float yRotation = Float.intBitsToFloat(((Long) Long.parseLong(data[pos++], 16)).intValue());
                            @SuppressWarnings("unused")
                            float zRotation = Float.intBitsToFloat(((Long) Long.parseLong(data[pos++], 16)).intValue());
                            @SuppressWarnings("unused")
                        ...
                        if (hasEventInfo == 1) {
                            ...
                            @SuppressWarnings("unused")
                            int encoderPosition = Integer.parseInt(data[pos++], 16);
                            @SuppressWarnings("unused")
                            int eventTime = Integer.parseInt(data[pos++], 16);
                            @SuppressWarnings("unused")
                            int angularPosition = Integer.parseInt(data[pos++], 16);
                        }
                    }
                    catch (final Exception e) {
                        throw new SickReadErrorException(message);
                    ...
                    event.put(MESSAGE_COUNT_ATTRIBUTE, measurement.getMessageCount());
                    event.put(SCAN_COUNT_ATTRIBUTE, measurement.getScanCount());
                    event.put(POWERUP_DURATION_ATTRIBUTE, measurement.getPowerUpDuration());
                    event.put(TRANSMISSION_DURATION_ATTRIBUTE, measurement.getTransmissionDuration());
                    event.put(INPUT_STATUS_ATTRIBUTE, measurement.isInputStatus());
                    event.put(OUTPUT_STATUS_ATTRIBUTE, measurement.isOutputStatus());
                    ...
                    for (int i = 0; i < retObj.length; i++) {
                        retObj[i] = event.get(schema.get(i).getAttributeName().toUpperCase());
                    }
