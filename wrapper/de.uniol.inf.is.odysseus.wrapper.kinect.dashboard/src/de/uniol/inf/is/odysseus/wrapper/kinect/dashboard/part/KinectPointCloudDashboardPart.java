...
public class KinectPointCloudDashboardPart extends AbstractDashboardPart
    ...
    protected void renderPointCloud(GL2 gl) {
        ...
        for (int y = 0; y < pointCloud.getHeight(); y++) {
            for (int x = 0; x < pointCloud.getWidth(); x++) {
                ...
                Point3D p = KinectPointCloud.unproject(pointCloud, x, y, z);

                gl.glColor3f(Helper.byteToFloat(colorBytes[index * KinectColorMap.DEPTH]),
                		Helper.byteToFloat(colorBytes[index * KinectColorMap.DEPTH + 1]),
                		Helper.byteToFloat(colorBytes[index * KinectColorMap.DEPTH + 2]));
    ...
    public void renderSkeleton(KinectSkeleton skeleton, GL2 gl) {
        setColor(gl, skeleton.getUserId() - 1);
        renderLine(gl, skeleton.getHead(), skeleton.getNeck());

        renderLine(gl, skeleton.getLeftShoulder(), skeleton.getTorso());
        renderLine(gl, skeleton.getRightShoulder(), skeleton.getTorso());
        renderLine(gl, skeleton.getLeftShoulder(), skeleton.getNeck());
        renderLine(gl, skeleton.getRightShoulder(), skeleton.getNeck());

        renderLine(gl, skeleton.getLeftShoulder(), skeleton.getLeftElbow());
        renderLine(gl, skeleton.getLeftElbow(), skeleton.getLeftHand());

        renderLine(gl, skeleton.getRightShoulder(), skeleton.getRightElbow());
        renderLine(gl, skeleton.getRightElbow(), skeleton.getRightHand());

        renderLine(gl, skeleton.getLeftHip(), skeleton.getTorso());
        renderLine(gl, skeleton.getRightHip(), skeleton.getTorso());
        renderLine(gl, skeleton.getLeftHip(), skeleton.getRightHip());

        renderLine(gl, skeleton.getLeftHip(), skeleton.getLeftKnee());
        renderLine(gl, skeleton.getLeftKnee(), skeleton.getLeftFoot());

        renderLine(gl, skeleton.getRightHip(), skeleton.getRightKnee());
        renderLine(gl, skeleton.getRightKnee(), skeleton.getRightFoot());
    }    
