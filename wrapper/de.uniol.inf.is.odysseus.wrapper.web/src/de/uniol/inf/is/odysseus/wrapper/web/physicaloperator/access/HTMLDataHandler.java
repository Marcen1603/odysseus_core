...
/**
 * Data Handler for HTML documents
 * The schema attribute names define the XPath for the attribute values
 * 
 * @author Christian Kuka <christian@kuka.cc>
 */
public class HTMLDataHandler extends AbstractDataHandler<Tuple<?>> {

    Logger                        LOG     = LoggerFactory.getLogger(HTMLDataHandler.class);
 ...
 */
     ...
     * @param schema
     */
    private HTMLDataHandler(SDFSchema schema) {
        this.schema = schema;
    ...
    public Tuple<?> readData(String string) {
        ...
        try {
            this.parser.parse(string, fragment);
        }
        ...
        for (int i = 0; i < schema.size(); i++) {
            ...
            try {
                ...
                NodeList nodes = (NodeList) expr.evaluate(fragment, XPathConstants.NODESET);
    ...
    protected IDataHandler<Tuple<?>> getInstance(SDFSchema schema) {
        return new HTMLDataHandler(schema);
    }
