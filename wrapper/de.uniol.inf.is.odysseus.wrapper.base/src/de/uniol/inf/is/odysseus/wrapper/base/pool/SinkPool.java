...
public class SinkPool<T extends IMetaAttribute> {
	...
	public static void registerSink(final String adapterName,
			final SinkPO<?> sink, final Map<String, String> options) {
	...
	public static void transfer(final SinkPO<?> sink, final long timestamp,
			final Object[] data) {
	...
	private void _registerSink(final String adapterName, final SinkPO<?> sink,
		...
		if (adapter != null) {
			...
			SinkPool.LOG.info("Physical Sink {} registered for adapter {}",
					sink, adapterName);
		} else {
	...
	private void _registerAdapter(final SinkAdapter adapter) {
		((ConcurrentHashMap<String, SinkAdapter>) this.adapters).putIfAbsent(
				adapter.getName(), adapter);
		SinkPool.LOG.info("Adapter {} registered", adapter.getName());
	}

	private void _transfer(final SinkPO<?> sink, final long timestamp,
			final Object[] data) {
		if (this.sinks.containsKey(sink)) {
			final String adapterName = this.sinkAdapterMapping.get(sink
					.getName());
			try {
				if (this.adapters.containsKey(adapterName)) {
					this.adapters.get(adapterName).transfer(
							this.sinks.get(sink), timestamp, data);
				}
	...
	private void _unregisterSink(final SinkPO<?> sink) {
		if (this.sinkAdapterMapping.containsKey(sink.getName())) {
			final SinkAdapter adapter = this.adapters
					.get(this.sinkAdapterMapping.get(sink.getName()));
