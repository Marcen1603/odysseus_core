...
public class KafkaTransportHandler_old extends AbstractPushTransportHandler {
	...
	public KafkaTransportHandler_old() {
		super();
	}

	public KafkaTransportHandler_old(final IProtocolHandler<?> protocolHandler, OptionMap options) {
	...
	public ITransportHandler createInstance(final IProtocolHandler<?> protocolHandler, final OptionMap options) {
		final KafkaTransportHandler_old handler = new KafkaTransportHandler_old(protocolHandler, options);
		return handler;
	}
	...
	public void processOutOpen() throws IOException {
		if (this.shareProducer) {
			producer = KafkaProducerRegistry_old.getInstance().getSemanticallyEqualProducer(this);
			if (this.producer == null) {
				config = new ProducerConfig(props);
				producer = new Producer<String, String>(config);
			}
			KafkaProducerRegistry_old.getInstance().addProducer(this, producer);
		} else {
			config = new ProducerConfig(props);
			producer = new Producer<String, String>(config);
		}
	...
	public void processOutClose() throws IOException {
		if (this.shareProducer) {
			KafkaProducerRegistry_old.getInstance().removeProducer(this);
		} else {
			producer.close();
		}
	}
	...
	public boolean isSemanticallyEqualImpl(ITransportHandler other) {
		if (!(other instanceof KafkaTransportHandler_old))
			return false;
		KafkaTransportHandler_old handler = (KafkaTransportHandler_old) other;
