 ...
 * push byte array data to Kafka as well as to consume byte array data from
 * Kafka.
 *
 ...
 */
public class ByteArrayKafkaTransportHandler extends AbstractTransportHandler {

	/**
	 * The sample (list of elements to be as a sample).
	 */
	private List<KeyedMessage<String, byte[]>> sample = new ArrayList<>();
	 ...
	 */
	public ByteArrayKafkaTransportHandler(final IProtocolHandler<?> protocolHandler, final OptionMap options,
			final String topic, final int sampleSize, final ProducerConfig producerConfig) {
		super(protocolHandler, options);
		this.topic = topic;
		this.sampleSize = sampleSize;
		this.producer = new Producer<String, byte[]>(producerConfig);
	}
	...
	public void send(byte[] message) throws IOException {
		sample.add(new KeyedMessage<String, byte[]>(topic, message));
		if (sample.size() == sampleSize) {
			...
			if (KafkaTransportHandler.log.isDebugEnabled()) {
				...
				sample.stream().forEach(m -> KafkaTransportHandler.log.debug(new String(m.message())));
			}
			sample.clear();
		}
	}
